...
public class rovashared {
 ...
 // handler for received Intents for the "com.quintech.connect.REG_COMPLETE" event 
    ...
    public RegistrationInfo GetRegistrationInformation()
    {
    	...
    	registrationInfo.browsersettings = getBrowserSettingsInfo();
    ...
    BrowserSettingsInfo getBrowserSettingsInfo()
    {
    	
    	BrowserSettingsInfo BrowserSettings = db.getBrowserSettings();
    	
    	return BrowserSettings;
    	
    }
    
    public Boolean blockURL(String URLString)
    {
        Boolean bBlock = false;
        Boolean bHost = false;
        String bwValue = "";
        
        bwValue = Constants.getData().getSetting(Settings.SET_BrowserBlackWhite, Data.SafeSpace );
        
        if (bwValue.length() > 0)
        {
            bHost = Constants.getData().FindHost(URLString );
            
            if (bwValue.compareTo("1")==0 && (bHost))
                bBlock = true;
            else if (bwValue.compareTo("2")==0 && (!bHost))
                bBlock =true;
        }
