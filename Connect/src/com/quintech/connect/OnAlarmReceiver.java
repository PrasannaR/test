...
public class OnAlarmReceiver extends BroadcastReceiver 
{
	...
	{    
		try
		{
			...
			if (intent!=null)
			{
				bForce = intent.getBooleanExtra("ForceUpdate",false);
			}
		            ...
		            public void run() 
		            {
		            	try
		            	{
							...
							if (Constants.getData().getFlag(Flags.FLG_IsRegistered, Data.SafeSpace))
							{
								...
								if (Constants.getData().getFlag(Flags.FLG_PIMSettings_Updated,Data.SafeSpace))
								{
								...
								if (Constants.getData().getFlag(Flags.FLG_SecuritySettings_Updated,Data.SafeSpace))
								{
									Intent return_intent = new Intent("com.cognizant.trubox.event");
								...
								if (Constants.getData().getFlag(Flags.FLG_EmailSettings_Updated, Data.SafeSpace))
								{
									Intent return_intent = new Intent("com.cognizant.trubox.event");
