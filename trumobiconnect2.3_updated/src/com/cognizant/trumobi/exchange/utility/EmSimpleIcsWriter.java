 ...
 */
public class EmSimpleIcsWriter {
    ...
    /* package for testing */ void writeLine(String string) {
        int numBytes = 0;
        for (byte b : EmUtility.toUtf8(string)) {
            ...
            if (numBytes > (MAX_LINE_LENGTH - CHAR_MAX_BYTES_IN_UTF8)
                    && EmUtility.isFirstUtf8Byte(b)) { // Only wrappable if it's before the first byte
                mOut.write((byte) '\r');
                mOut.write((byte) '\n');
                mOut.write((byte) '\t');
                numBytes = 1; // for TAB
    ...
    public String toString() {
        return EmUtility.fromUtf8(getBytes());
    }
