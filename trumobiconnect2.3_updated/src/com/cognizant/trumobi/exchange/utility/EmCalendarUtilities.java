package com.cognizant.trumobi.exchange.utility;
...
import com.cognizant.trumobi.common.provider.Calendar.EventsEntity;
...
public class EmCalendarUtilities {
	// NOTE: Most definitions in this class are have package visibility for
	// testing purposes
	...
	// TimeZone cache; we parse/decode as little as possible, because the
	...
	static final String[] sTypeToFreq = new String[] { "DAILY", "WEEKLY",
			"MONTHLY", "MONTHLY", "", "YEARLY", "YEARLY" };

	static final String[] sDayTokens = new String[] { "SU", "MO", "TU", "WE",
			"TH", "FR", "SA" };

	static final String[] sTwoCharacterNumbers = new String[] { "00", "01",
	...
	static final String ICALENDAR_ATTENDEE_INVITE = ICALENDAR_ATTENDEE
			+ ";PARTSTAT=NEEDS-ACTION;RSVP=TRUE";
	static final String ICALENDAR_ATTENDEE_ACCEPT = ICALENDAR_ATTENDEE
			+ ";PARTSTAT=ACCEPTED";
	static final String ICALENDAR_ATTENDEE_DECLINE = ICALENDAR_ATTENDEE
			+ ";PARTSTAT=DECLINED";
	static final String ICALENDAR_ATTENDEE_TENTATIVE = ICALENDAR_ATTENDEE
			+ ";PARTSTAT=TENTATIVE";

	// Note that these constants apply to Calendar items
	// For future reference: MeetingRequest data can also include free/busy
	// information, but the
	// constants for these four options in MeetingRequest data have different
	// values!
	...
	static void putRuleIntoTimeZoneInformation(byte[] bytes, int offset,
			RRule rrule, int hour, int minute) {
		...
		setWord(bytes, offset + MSFT_SYSTEMTIME_DAY_OF_WEEK,
				rrule.dayOfWeek - 1);
		// 5 means "last" in MSFT land; for RRule, it's -1
		setWord(bytes, offset + MSFT_SYSTEMTIME_DAY, rrule.week < 0 ? 5
				: rrule.week);
	...
	// Write a transition time into SYSTEMTIME data (via an offset into a byte
	// array)
	static void putTransitionMillisIntoSystemTime(byte[] bytes, int offset,
			long millis) {
		...
		// Turn hours/minutes into ms from midnight (per TimeZone)
		setWord(bytes, offset + MSFT_SYSTEMTIME_HOUR,
				getTrueTransitionHour(cal));
		setWord(bytes, offset + MSFT_SYSTEMTIME_MINUTE,
				getTrueTransitionMinute(cal));
	}
	...
	/**
	 * Return a GregorianCalendar representing the first standard/daylight
	 * transition between a start time and an end time in the given time zone
	 * 
	 * @param tz
	 *            a TimeZone the time zone in which we're looking for
	 *            transitions
	 * @param startTime
	 *            the start time for the test
	 * @param endTime
	 *            the end time for the test
	 * @param startInDaylightTime
	 *            whether daylight time is in effect at the startTime
	...
	/**
	 * Return a Base64 representation of a MSFT TIME_ZONE_INFORMATION structure
	 * from a TimeZone that might be found in an Event; use cached result, if
	 * possible
	 * 
	 * @param tz
	 *            the TimeZone
	 * @return the Base64 String representing a Microsoft TIME_ZONE_INFORMATION
	 *         element
	 */
	static public String timeZoneToTziString(TimeZone tz) {
		...
		if (tziString != null) {
			if (EmEas.USER_LOG) {
				EmSyncManager.log(TAG, "TZI string for " + tz.getDisplayName()
						+ " found in cache.");
			}
			return tziString;
		}
	...
	/**
	 * A class for storing RRULE information. The RRULE members can be accessed
	 * individually or an RRULE string can be created with toString()
	 */
	static class RRule {
		 ...
		 *            the month (1 = JAN, 12 = DEC)
		 * @param _dayOfWeek
		 *            the day of the week (1 = SU, 7 = SA)
		 * @param _week
		 *            the week in the month (1-5 or -1 for last)
		 */
	...
	/**
	 * Generate an RRULE string for an array of GregorianCalendars, if possible.
	 * For now, we are only looking for rules based on the same date in a month
	 * or a specific instance of a day of the week in a month (e.g. 2nd Tuesday
	 * or last Friday). Indeed, these are the only kinds of rules used in the
	 * current tzinfo database.
	 * 
	 * @param calendars
	 *            an array of GregorianCalendar, set to a series of transition
	 *            times in consecutive years starting with the current year
	...
	static RRule inferRRuleFromCalendars(GregorianCalendar[] calendars) {
		...
		// sDayTokens is 0 based (SU = 0); Calendar days of week are 1 based (SU
		// = 1)
	...
	/**
	 * Generate an rfc2445 utcOffset from minutes offset from GMT These look
	 * like +0800 or -0100
	 * 
	 * @param offsetMinutes
	 *            minutes offset from GMT (east is positive, west is negative
	 * @return a utcOffset
	 */
	...
	/**
	 * Fill the passed in GregorianCalendars arrays with DST transition
	 * information for this and the following years (based on the length of the
	 * arrays)
	 * 
	 * @param tz
	 *            the time zone
	 * @param toDaylightCalendars
	 *            an array of GregorianCalendars, one for each year,
	 *            representing the transition to daylight time
	 * @param toStandardCalendars
	 *            an array of GregorianCalendars, one for each year,
	 *            representing the transition to standard time
	 * @return true if transitions could be found for all years, false otherwise
	 */
	static boolean getDSTCalendars(TimeZone tz,
			GregorianCalendar[] toDaylightCalendars,
			GregorianCalendar[] toStandardCalendars) {
		// We'll use the length of the arrays to determine how many years to
		// check
	 ...
	 * 
	 * @param writer
	 *            the SimpleIcsWriter we're using
	 * @param tz
	 *            the time zone
	 * @param offsetString
	 *            the offset string in VTIMEZONE format (e.g. +0800)
	 * @throws IOException
	 */
	static private void writeNoDST(EmSimpleIcsWriter writer, TimeZone tz,
	 ...
	 * 
	 * @param tz
	 *            the TimeZone to be used in the conversion
	 * @param writer
	 *            the SimpleIcsWriter to be used
	 * @throws IOException
	 */
	static void timeZoneToVTimezone(TimeZone tz, EmSimpleIcsWriter writer)
			throws IOException {
		// We'll use these regardless of whether there's DST in this time zone
		// or not
		...
		String daylightOffsetString = utcOffsetString(rawOffsetMinutes
				+ (tz.getDSTSavings() / MINUTES));
		...
		if (hasRule) {
			...
			for (int i = 1; i < maxYears; i++) {
				writer.writeTag(
						"RDATE",
						transitionMillisToVCalendarTime(
								toDaylightCalendars[i].getTimeInMillis(), tz,
								true));
			}
		}
		...
		if (hasRule) {
			...
			for (int i = 1; i < maxYears; i++) {
				writer.writeTag(
						"RDATE",
						transitionMillisToVCalendarTime(
								toStandardCalendars[i].getTimeInMillis(), tz,
								true));
			}
		}
		writer.writeTag("END", "STANDARD");
	 ...
	 * 
	 * @param transitions
	 *            calendars representing transitions to/from DST
	 * @return millis for the first transition after the current date/time
	 */
	static long findNextTransition(long startingMillis,
			GregorianCalendar[] transitions) {
	 ...
	 * structure from a TimeZone that might be found in an Event. Since the
	 * internal representation of the TimeZone is hidden from us we'll find the
	 * DST transitions and build the structure from that information
	 * 
	 * @param tz
	 *            the TimeZone
	 * @return the Base64 String representing a Microsoft TIME_ZONE_INFORMATION
	 *         element
	 */
	public static String timeZoneToTziStringImpl(TimeZone tz) {
		String tziString;
		...
		if (tz.useDaylightTime()) {
			...
			// See if we can get transitions for a few years; if not, we can't
			// generate DST info
			// for this time zone
			if (getDSTCalendars(tz, toDaylightCalendars, toStandardCalendars)) {
				...
				if ((daylightRule != null)
						&& (daylightRule.type == RRule.RRULE_DAY_WEEK)
						&& (standardRule != null)
						&& (standardRule.type == RRule.RRULE_DAY_WEEK)) {
					// We need both rules and they have to be DAY/WEEK type
					// Write month, day of week, week, hour, minute
					putRuleIntoTimeZoneInformation(tziBytes,
							MSFT_TIME_ZONE_STANDARD_DATE_OFFSET, standardRule,
							getTrueTransitionHour(toStandardCalendars[0]),
							getTrueTransitionMinute(toStandardCalendars[0]));
					putRuleIntoTimeZoneInformation(tziBytes,
							MSFT_TIME_ZONE_DAYLIGHT_DATE_OFFSET, daylightRule,
							getTrueTransitionHour(toDaylightCalendars[0]),
							getTrueTransitionMinute(toDaylightCalendars[0]));
				} else {
					// If there's no rule, we'll use the first transition to
					// standard/to daylight
					// And indicate that it's just for this year...
					long now = System.currentTimeMillis();
					long standardTransition = findNextTransition(now,
							toStandardCalendars);
					long daylightTransition = findNextTransition(now,
							toDaylightCalendars);
			...
			setLong(tziBytes, MSFT_TIME_ZONE_DAYLIGHT_BIAS_OFFSET, -dstOffset
					/ MINUTES);
		}
	...
	/**
	 * Given a String as directly read from EAS, returns a TimeZone
	 * corresponding to that String
	 * 
	 * @param timeZoneString
	 *            the String read from the server
	...
	static public TimeZone tziStringToTimeZone(String timeZoneString) {
		...
		if (timeZone != null) {
			if (EmEas.USER_LOG) {
				EmSyncManager.log(TAG,
						" Using cached TimeZone " + timeZone.getID());
			}
		} else {
			timeZone = tziStringToTimeZoneImpl(timeZoneString);
			if (timeZone == null) {
				// If we don't find a match, we just return the current
				// TimeZone. In theory, this
				// shouldn't be happening...
				EmSyncManager.alwaysLog("TimeZone not found using default: "
						+ timeZoneString);
	...
	/**
	 * Given a String as directly read from EAS, tries to find a TimeZone in the
	 * database of all time zones that corresponds to that String.
	 * 
	 * @param timeZoneString
	 *            the String read from the server
	...
	static TimeZone tziStringToTimeZoneImpl(String timeZoneString) {
		...
		// Then, we get the bias (similar to a rawOffset); for TimeZone, we need
		// ms
		// but EAS gives us minutes, so do the conversion. Note that EAS is the
		// bias that's added
		// to the time zone to reach UTC; our library uses the time from UTC to
		// our time zone, so
		...
		if (zoneIds.length > 0) {
			...
			TimeZoneDate dstEnd = getTimeZoneDateFromSystemTime(timeZoneBytes,
					MSFT_TIME_ZONE_STANDARD_DATE_OFFSET);
			if (dstEnd == null) {
				// If the default time zone is a match
				TimeZone defaultTimeZone = TimeZone.getDefault();
				if (!defaultTimeZone.useDaylightTime()
						&& EmArrayUtils.contains(zoneIds,
								defaultTimeZone.getID())) {
					if (EmEas.USER_LOG) {
						EmSyncManager.log(TAG,
								"TimeZone without DST found to be default: "
										+ defaultTimeZone.getID());
					}
					return defaultTimeZone;
				}
				// In this case, there is no daylight savings time, so the only
				// interesting data
				...
				for (String zoneId : zoneIds) {
					...
					if (!timeZone.useDaylightTime()) {
						if (EmEas.USER_LOG) {
							EmSyncManager.log(TAG,
									"TimeZone without DST found by offset: "
											+ timeZone.getID());
						}
						return timeZone;
					}
				...
				long dstSavings = -1
						* getLong(timeZoneBytes,
								MSFT_TIME_ZONE_DAYLIGHT_BIAS_OFFSET) * MINUTES;

				// We'll go through each time zone to find one with the same DST
				// transitions and
				// savings length
				for (String zoneId : zoneIds) {
					...
					// Our strategy here is to check just before and just after
					// the transitions
					// and see whether the check for daylight time matches the
					// expectation
					// If both transitions match, then we have a match for the
					// offset and start/end
					// of dst. That's the best we can do for now, since there's
					// no other info
					// provided by EAS (i.e. we can't get dynamic transitions,
					// etc.)
					...
					millisAtTransition = getMillisAtTimeZoneDateTransition(
							timeZone, dstEnd);
					// Note that we need to subtract an extra hour here, because
					// we end up with
				...
				// In this case, there is no daylight savings time, so the only
				// interesting data
				// is the offset, and we know that all of the zoneId's match;
				// we'll take the first
	...
	static public String convertEmailDateTimeToCalendarDateTime(String date) {
		...
		return date.substring(0, 4) + date.substring(5, 7)
				+ date.substring(8, 13) + date.substring(14, 16)
				+ date.substring(17, 19) + 'Z';
	}
	...
	/**
	 * Generate an EAS formatted date/time string based on GMT. See below for
	 * details.
	 */
	...
	/**
	 * Generate an EAS formatted local date/time string from a time and a time
	 * zone. If the final argument is false, only a date will be returned (e.g.
	 * 20100331)
	 * 
	 * @param millis
	 ...
	 *            a time zone
	 * @param withTime
	 *            if the time is to be included in the string
	 * @return an EAS formatted string indicating the date (and time) in the
	 *         given time zone
	 */
	...
	/**
	 * Return the true minute at which a transition occurs Our transition time
	 * should be the in the minute BEFORE the transition If this minute is 59,
	 * set minute to 0 and increment the hour NOTE: We don't want to add a
	 * minute and retrieve minute/hour from the Calendar, because Calendar time
	 * will itself be influenced by the transition! So adding 1 minute to 01:59
	 * (assume PST->PDT) will become 03:00, which isn't what we want (we want
	 * 02:00)
	 * 
	 * @param calendar
	 *            the calendar holding the transition date/time
	...
	/**
	 * Return the true hour at which a transition occurs See description for
	 * getTrueTransitionMinute, above
	 * 
	 * @param calendar
	 *            the calendar holding the transition date/time
	...
	/**
	 * Generate a date/time string suitable for VTIMEZONE from a transition time
	 * in millis The format is YYYYMMDDTHHMMSS
	 * 
	 * @param millis
	 *            a transition time in milliseconds
	 * @param tz
	 *            a time zone
	 * @param dst
	 *            whether we're entering daylight time
	 */
	static String transitionMillisToVCalendarTime(long millis, TimeZone tz,
			boolean dst) {
		StringBuilder sb = new StringBuilder();
		GregorianCalendar cal = new GregorianCalendar(tz);
		cal.setTimeInMillis(millis);
	...
	/**
	 * Returns a UTC calendar with year/month/day from local calendar and
	 * h/m/s/ms = 0
	 * 
	 * @param time
	 *            the time in seconds of an all-day event in local time
	 * @return the time in seconds in UTC
	 */
	static public long getUtcAllDayCalendarTime(long time,
			TimeZone localTimeZone) {
		return transposeAllDayTime(time, localTimeZone, UTC_TIMEZONE);
	}

	/**
	 * Returns a local calendar with year/month/day from UTC calendar and
	 * h/m/s/ms = 0
	 * 
	 * @param time
	 *            the time in seconds of an all-day event in UTC
	 * @return the time in seconds in local time
	 */
	static public long getLocalAllDayCalendarTime(long time,
			TimeZone localTimeZone) {
	...
	static private long transposeAllDayTime(long time, TimeZone fromTimeZone,
		...
		// Set this calendar with correct year, month, and day, but zero hour,
		// minute, and seconds
	...
	public static String getDaysFromDow(int dow, int wom) {
	...
	/**
	 * Generate the String version of the EAS integer for a given BYDAY value in
	 * an rrule
	 * 
	 * @param dow
	 *            the BYDAY value of the rrule
	 * @return the String version of the EAS value of these days
	 */
	public static String generateEasDayOfWeek(String dow) {
		int bits = 0;
		int bit = 1;
		for (String token : sDayTokens) {
			// If we can find the day in the dow String, add the bit to our bits
			// value
	 ...
	 *            an RRULE string
	 * @param token
	 *            a token to look for in the RRULE
	 ...
	 */
	static void addUntil(String rrule, EmSerializer s) throws IOException {
		String until = tokenFromRrule(rrule, "UNTIL=");
		if (until != null) {
			s.data(EmTags.CALENDAR_RECURRENCE_UNTIL, recurrenceUntilToEasUntil(until));
		}
	}

	/**
	 * Write recurrence information to EAS based on the RRULE in
	 * CalendarProvider
	 * 
	 * @param rrule
	 *            the RRULE, from CalendarProvider
	 * @param startTime
	 *            , the DTSTART of this Event
	 * @param s
	 *            the Serializer we're using to write WBXML data
	 * @throws IOException
	 */
	// NOTE: For the moment, we're only parsing recurrence types that are
	// supported by the
	...
	static public void recurrenceFromRrule(String rrule, long startTime,
			EmSerializer s) throws IOException {
		if (EmEas.USER_LOG) {
			EmSyncManager.log(TAG, "RRULE: " + rrule);
		}
		String freq = tokenFromRrule(rrule, "FREQ=");
		// If there's no FREQ=X, then we don't write a recurrence
		// Note that we duplicate s.start(Tags.CALENDAR_RECURRENCE); s.end(); to
		// prevent the
		// possibility of writing out a partial recurrence stanza
		if (freq != null) {
			if (freq.equals("DAILY")) {
				s.start(EmTags.CALENDAR_RECURRENCE);
				s.data(EmTags.CALENDAR_RECURRENCE_TYPE, "0");
				s.data(EmTags.CALENDAR_RECURRENCE_INTERVAL, "1");
				addUntil(rrule, s);
				s.end();
			} else if (freq.equals("WEEKLY")) {
				s.start(EmTags.CALENDAR_RECURRENCE);
				s.data(EmTags.CALENDAR_RECURRENCE_TYPE, "1");
				s.data(EmTags.CALENDAR_RECURRENCE_INTERVAL, "1");
				// Requires a day of week (whereas RRULE does not)
				String byDay = tokenFromRrule(rrule, "BYDAY=");
				if (byDay != null) {
					s.data(EmTags.CALENDAR_RECURRENCE_DAYOFWEEK,
							generateEasDayOfWeek(byDay));
				}
				addUntil(rrule, s);
				s.end();
				...
				if (byMonthDay != null) {
					...
					s.start(EmTags.CALENDAR_RECURRENCE);
					s.data(EmTags.CALENDAR_RECURRENCE_TYPE, "2");
					s.data(EmTags.CALENDAR_RECURRENCE_DAYOFMONTH, byMonthDay);
					addUntil(rrule, s);
					s.end();
				} else {
					String byDay = tokenFromRrule(rrule, "BYDAY=");
					...
					if (byDay != null) {
						...
						if (wom == '-') {
							// -1 is the only legal case (last week) Use "5" for
							// EAS
							wom = 5;
						...
						s.start(EmTags.CALENDAR_RECURRENCE);
						s.data(EmTags.CALENDAR_RECURRENCE_TYPE, "3");
						s.data(EmTags.CALENDAR_RECURRENCE_WEEKOFMONTH,
								Integer.toString(wom));
						s.data(EmTags.CALENDAR_RECURRENCE_DAYOFWEEK,
								generateEasDayOfWeek(bareByDay));
						addUntil(rrule, s);
						s.end();
					}
				}
				...
				if (byMonth == null || byMonthDay == null) {
					...
					byMonthDay = Integer.toString(cal
							.get(Calendar.DAY_OF_MONTH));
				}
				s.start(EmTags.CALENDAR_RECURRENCE);
				s.data(EmTags.CALENDAR_RECURRENCE_TYPE, "5");
				s.data(EmTags.CALENDAR_RECURRENCE_DAYOFMONTH, byMonthDay);
				s.data(EmTags.CALENDAR_RECURRENCE_MONTHOFYEAR, byMonth);
				addUntil(rrule, s);
				s.end();
			}
		}
	 ...
	 * @param occurrences
	 *            how many recurrences (instances)
	 * @param interval
	 ...
	 *            month of the year
	 * @param until
	 *            the last recurrence time
	 * @return a valid RRULE String
	 */
	static public String rruleFromRecurrence(int type, int occurrences,
			int interval, int dow, int dom, int wom, int moy, String until) {
		...
		case 1: // WEEKLY
			if (dow > 0)
				addByDay(rrule, dow, -1);
			break;
		case 2: // MONTHLY
			if (dom > 0)
				addByMonthDay(rrule, dom);
			break;
		case 3: // MONTHLY (on the nth day)
			if (dow > 0)
				addByDay(rrule, dow, wom);
			break;
	...
	/**
	 * Create a Calendar in CalendarProvider to which synced Events will be
	 * linked
	 * 
	 * @param service
	 *            the sync service requesting Calendar creation
	 * @param account
	 *            the account being synced
	 * @param mailbox
	 *            the Exchange mailbox for the calendar
	 * @return the unique id of the Calendar
	 */
	static public long createCalendar(EmEasSyncService service,
			Account account, Mailbox mailbox) {
		// Create a Calendar object
		ContentValues cv = new ContentValues();
		// TODO How will this change if the user changes his account display
		// name?
		cv.put(Calendars.DISPLAY_NAME, account.mDisplayName);
		cv.put(Calendars._SYNC_ACCOUNT, account.mEmailAddress);
		cv.put(Calendars._SYNC_ACCOUNT_TYPE,
				Email.EXCHANGE_ACCOUNT_MANAGER_TYPE);
		cv.put(Calendars.SYNC_EVENTS, 1);
		cv.put(Calendars.SELECTED, 1);
		// cv.put(Calendars.HIDDEN, 0); //NaGa
	...
	static public byte[] getICSBytesForEvent(Event event, Account account) {
		int messageFlag = Message.FLAG_OUTGOING_MEETING_INVITE;
		...
		try {
			...
			ics.writeTag(
					"DTSTART" + vCalendarDateSuffix,
					millisToEasDateTime(event.dtstart, vCalendarTimeZone,
							!allDayEvent));
			...
			//
			//
			ics.writeTag("LOCATION", event.eventLocation);
			//
			//
			//
			// Resources resources = context.getResources();
			//
			ics.writeTag("SUMMARY", event.title);

			ics.writeTag("DESCRIPTION", event.description);

			//
			// // And store the message text

			String organizerName = CalendarDatabaseHelper.getEmail_ID();
			String organizerEmail = CalendarDatabaseHelper.getEmail_ID();
			ArrayList<EmAddress> toList = new ArrayList<EmAddress>();
			if (event.attendees != null && !event.attendees.isEmpty()) {
				for (Attendee ncv : event.attendees) {

					// if(!ncv.attendeeEmail.equalsIgnoreCase(account.mEmailAddress))
					// {
					addAttendeeToMessage(ics, toList, ncv.attendeeName,
							ncv.attendeeEmail, messageFlag, account);
					// }
					// addAttendeeToMessage(ics,
					// toList,"aishwarya@truboxmdmdev.com",
					// "aishwarya@truboxmdmdev.com", messageFlag,
					// account);
				}
			...
			if (organizerEmail != null) {
				...
				// We should be able to find this, assuming the CognizantEmail
				// is the user's email
				// TODO Find this in the account
				if (organizerName != null) {
					icalTag += ";CN="
							+ EmSimpleIcsWriter.quoteParamValue(organizerName);
				}
			...
			ics.writeTag(
					"STATUS",
					(messageFlag == Message.FLAG_OUTGOING_MEETING_CANCEL) ? "CANCELLED"
							: "CONFIRMED");
			ics.writeTag("TRANSP", "OPAQUE"); // What Exchange uses
			ics.writeTag("PRIORITY", "5"); // 1 to 9, 5 = medium
			ics.writeTag("SEQUENCE", "0");
			ics.writeTag("UID", event.calendarUID);
			ics.writeTag("CLASS", "PUBLIC");
			ics.writeTag("X-MICROSOFT-CDO-APPT-SEQUENCE", "0");
			ics.writeTag("X-MICROSOFT-CDO-BUSYSTATUS", "TENTATIVE");
			ics.writeTag("X-MICROSOFT-CDO-INTENDEDSTATUS", "BUSY");
			ics.writeTag("X-MICROSOFT-CDO-ALLDAYEVENT", "FALSE");
			ics.writeTag("X-MICROSOFT-CDO-IMPORTANCE", "1");
			ics.writeTag("X-MICROSOFT-CDO-INSTTYPE", "0");
			ics.writeTag("X-MICROSOFT-DISALLOW-COUNTER", "FALSE");
			ics.writeTag("END", "VEVENT");
			ics.writeTag("END", "VCALENDAR");
			return ics.getBytes();

		} catch (Exception e) {
			Log.w(TAG, "Exception in createMessageForEntity " + e.toString());
			return null;
		}
	...
	static public EmEmailContent.Message createMessageForEvent(Event event,
			Account account, int messageFlag, boolean isException) {

		EmEmailContent.Message msg = new EmEmailContent.Message();
		msg.mFlags = messageFlag;
		msg.mTimeStamp = System.currentTimeMillis();
		msg.mMessageId = EmUtility.generateMessageId();
		...
		try {
			...
			if (event.allDay == 1) {
				...
				vCalendarDateSuffix = ";VALUE=DATE";
				allDayEvent = true;
				if (!Event.EVENT_TIMEZONE.equalsIgnoreCase("")) {
					vCalendarTimeZone = CalendarDatabaseHelper
							.getTimeZoneFromString(Event.EVENT_TIMEZONE);
				}
			}

			// If we're inviting people and the meeting is recurring, we need to
			// send our time zone
			// information and make sure to send DTSTART/DTEND in local time
			// (unless, of course,
			// this is an all-day event). Recurring, for this purpose, includes
			// exceptions to
			// recurring events
			if (!allDayEvent
					&& ((event.rrule != null && !event.rrule.isEmpty()) || isException)) {
			...
			ics.writeTag(
					"DTEND" + vCalendarDateSuffix,
					millisToEasDateTime(event.dtend, vCalendarTimeZone,
							!allDayEvent));
			...
			//
			//
			 Resources resources = Email.getAppContext().getResources();
			//
			ics.writeTag("SUMMARY", event.title);
			//
			msg.mSubject = event.title;
			//
			//
			// // Build the text for the message, starting with an initial line
			// describing the
			// // exception (if this is one)
			StringBuilder sb = new StringBuilder();
			if (isException) {
				long originalTime = event.dtstart;
				ics.writeTag(
						"RECURRENCE-ID" + vCalendarDateSuffix,
						millisToEasDateTime(originalTime, vCalendarTimeZone,
								!allDayEvent));
			}
			if (isException) {
				// Add the line, depending on whether this is a cancellation or
				// update
				Date date = new Date(event.dtstart);
				String dateString = DateFormat.getDateInstance().format(date);

				sb.append(resources.getString(R.string.exception_updated,
						dateString));
			...
			//
			// // And store the message text
			msg.mText = buildMessageTextFromEventValues(Email.getAppContext(),
					event, sb);
			// // if (!isReply) {
			// // if (entityValues.containsKey(Events.ALL_DAY)) {
			// // Integer ade = entityValues.getAsInteger(Events.ALL_DAY);
			// // ics.writeTag("X-MICROSOFT-CDO-ALLDAYEVENT", ade == 0 ? "FALSE"
			// : "TRUE");
			// // }
			// //
			...
			// // // If we decide to send alarm information in the meeting
			// request ics file,
			// // // handle it here by looping through the subvalues
			// // }
			//
			// // Handle attendee data here; determine "to" list and add
			// ATTENDEE tags to ics
			String organizerName = CalendarDatabaseHelper.getEmail_ID();
			String organizerEmail = CalendarDatabaseHelper.getEmail_ID();
			ArrayList<EmAddress> toList = new ArrayList<EmAddress>();
			if (event.attendees != null && !event.attendees.isEmpty()) {
				CalendarLog.d(CalendarConstants.Tag, "attendees not empty ");
				for (Attendee ncv : event.attendees) {

					if (!ncv.attendeeEmail
							.equalsIgnoreCase(account.mEmailAddress)) {
						CalendarLog.d(CalendarConstants.Tag,
								"attendee email which is added is "
										+ ncv.attendeeEmail);
						addAttendeeToMessage(ics, toList, ncv.attendeeName,
								ncv.attendeeEmail, messageFlag, account);
					}
			...
			if (toList.isEmpty())
				CalendarLog.d(CalendarConstants.Tag, "to list is null ");

			// Write out the "to" list
			EmAddress[] toArray = new EmAddress[toList.size()];
			int i = 0;
			for (EmAddress address : toList) {
				toArray[i++] = address;
			}
			msg.mTo = EmAddress.pack(toArray);
			CalendarLog.d(CalendarConstants.Tag, "msg to " + msg.mTo);
			msg.mFrom = CalendarDatabaseHelper.getEmail_ID();
			if(event.accessibility==1)
				ics.writeTag("CLASS", "PRIVATE");
			else
			ics.writeTag("CLASS", "PUBLIC");
			...
			CalendarLog.d(CalendarConstants.Tag, "Attach "
					+ att.toContentValues().toString());
			// Add the attachment to the message
			msg.mAttachments = new ArrayList<Attachment>();
			msg.mFlagAttachment = true;
			msg.mAttachments.add(att);
			CalendarLog.d(CalendarConstants.Tag, msg.toContentValues()
					.toString());
		} catch (Exception e) {
			Log.w(TAG, "Exception in createMessageForEntity " + e.toString());
			return null;
	 ...
	 * 
	 * @param globalObjId
	 *            the base64 encoded String provided by EAS
	...
	static public String getUidFromGlobalObjId(String globalObjId) {
		...
		try {
			...
			// If the base64 decoded string contains the magic substring:
			// "vCal-Uid", then
			// the actual uid is hidden within; the magic substring is never at
			// the start of the
			// decoded base64
			int index = idString.indexOf("vCal-Uid");
			if (index > 0) {
				// The uid starts after "vCal-Uidxxxx", where xxxx are padding
				// characters. And it ends before the last character, which is
				// ascii 0
				return idString.substring(index + 12, idString.length() - 1);
			} else {
				// This is an EAS uid. Go through the bytes and write out the
				// hex
				// values as characters; this is what we'll need to pass back to
				// EAS
				// when responding to the invitation
				for (byte b : idBytes) {
					EmUtility.byteToHex(sb, b);
				}
				return sb.toString();
			}
		} catch (RuntimeException e) {
			// In the worst of cases (bad format, etc.), we can always return
			// the input
			return globalObjId;
		}
	}
	...
	/**
	 * Get a selfAttendeeStatus from a busy status The default here is NONE
	 * (i.e. we don't know the status) Note that a busy status of FREE must mean
	 * NONE as well, since it can't mean declined (there would be no event)
	 * 
	 * @param busyStatus
	 *            the busy status, from EAS
	...
	static public int busyStatusFromAvailability(int availability) {
		int busyStatus;
		switch (availability) {
		case Event.AVAILABILITY_BUSY:
			busyStatus = BUSY_STATUS_BUSY;
			break;
		case Event.AVAILABILITY_FREE:
			busyStatus = BUSY_STATUS_FREE;
			break;
		case Event.AVAILABILITY_TENTATIVE:
	...
	static public int availabilityFromBusyStatus(int busyStatus) {
		...
		switch (busyStatus) {
		case BUSY_STATUS_BUSY:
			availability = Event.AVAILABILITY_BUSY;
			break;
		case BUSY_STATUS_FREE:
			availability = Event.AVAILABILITY_FREE;
			break;
		case BUSY_STATUS_TENTATIVE:
			availability = Event.AVAILABILITY_TENTATIVE;
			break;
		default:
			availability = Event.AVAILABILITY_AWAY;
			break;
		}
		return availability;
	}
	 ...
	 * 
	 * @param selfAttendeeStatus
	 *            from CalendarProvider2
	...
	static public String buildMessageTextFromEventValues(Context context,
		...
		// What we're missing is the duration and any recurrence information. So
		// this should be
		// more like... When: Tuesdays, starting March 5th from 2:00pm - 3:00pm
		// This would require code to build complex strings, and it will have to
		// wait
		...
		if (event.rrule != null
				&& (event.rrule.equalsIgnoreCase("") || event.rrule
						.equalsIgnoreCase("0")))
			recurringEvent = false;

		else
			recurringEvent = true;
		CalendarLog.d(CalendarConstants.Tag, "recurreing is true? "
				+ recurringEvent + " " + event.rrule);
		String dateTimeString;
		int res;
		long startTime = event.dtstart;
	...
	static public String buildMessageTextFromEntityValues(Context context,
		...
		if (!entityValues.containsKey(Events.ORIGINAL_EVENT)
				&& entityValues.containsKey(Events.RRULE)) {
			sb.append(resources.getString(R.string.meeting_recurring,
					dateTimeString));
		} else {
			sb.append(resources
					.getString(R.string.meeting_when, dateTimeString));
		}
		String location = null;
	...
	/**
	 * Add an attendee to the ics attachment and the to list of the Message
	 * being composed
	 * 
	 * @param ics
	 *            the ics attachment writer
	 * @param toList
	 *            the list of addressees for this email
	 * @param attendeeName
	 *            the name of the attendee
	 * @param attendeeEmail
	 *            the email address of the attendee
	 * @param messageFlag
	 *            the flag indicating the action to be indicated by the message
	 * @param account
	 *            the sending account of the email
	 */
	static private void addAttendeeToMessage(EmSimpleIcsWriter ics,
			ArrayList<EmAddress> toList, String attendeeName,
			String attendeeEmail, int messageFlag, Account account) {
		CalendarLog.d(CalendarConstants.Tag, "message flag is " + messageFlag);
		if ((messageFlag & Message.FLAG_OUTGOING_MEETING_REQUEST_MASK) != 0) {
			CalendarLog.d(CalendarConstants.Tag, "message is request ");
			...
			if (attendeeName != null) {
				icalTag += ";CN="
						+ EmSimpleIcsWriter.quoteParamValue(attendeeName);
			}
			ics.writeTag(icalTag, "MAILTO:" + attendeeEmail);
			toList.add(attendeeName == null ? new EmAddress(attendeeEmail)
					: new EmAddress(attendeeEmail, attendeeName));
	 ...
	 * 
	 * @param entity
	 *            the Entity for the Event (as might be retrieved by
	 *            CalendarProvider)
	 * @param messageFlag
	 *            the Message.FLAG_XXX constant indicating the type of email to
	 *            be sent
	 * @param the
	 *            unique id of this Event, or null if it can be retrieved from
	 *            the Event
	 * @param the
	 *            user's account
	 * @return a Message with many fields pre-filled (more later)
	 */
	static public EmEmailContent.Message createMessageForEntity(
			Context context, Entity entity, int messageFlag, String uid,
			Account account) {
		return createMessageForEntity(context, entity, messageFlag, uid,
				account, null /* specifiedAttendee */);
	}

	static public EmEmailContent.Message createMessageForEntity(
			Context context, Entity entity, int messageFlag, String uid,
			Account account, String specifiedAttendee) {
		...
		String method;
		if ((messageFlag & EmEmailContent.Message.FLAG_OUTGOING_MEETING_INVITE) != 0) {
			method = "REQUEST";
		} else if ((messageFlag & EmEmailContent.Message.FLAG_OUTGOING_MEETING_CANCEL) != 0) {
			method = "CANCEL";
		} else {
			method = "REPLY";
			isReply = true;
		...
		try {
			...
			if (!isReply
					&& !allDayEvent
					&& (entityValues.containsKey(Events.RRULE) || entityValues
							.containsKey(Events.ORIGINAL_EVENT))) {
				vCalendarTimeZone = TimeZone.getDefault();
				// Write the VTIMEZONE block to the writer
				timeZoneToVTimezone(vCalendarTimeZone, ics);
				// Example: DTSTART;TZID=US/Pacific:20100331T124500
			...
			// If this is an Exception, we send the recurrence-id, which is just
			// the original
			// instance time
			if (isException) {
			...
			if (!entityValues.containsKey(Events.DURATION)) {
				...
				ics.writeTag(
						"DTEND" + vCalendarDateSuffix,
						millisToEasDateTime(startTime + durationMillis,
								vCalendarTimeZone, !allDayEvent));
			}
			...
			if (isException && !isReply) {
				...
				// update
				Date date = new Date(
						entityValues.getAsLong(Events.ORIGINAL_INSTANCE_TIME));
				String dateString = DateFormat.getDateInstance().format(date);
				if (titleId == R.string.meeting_canceled) {
					sb.append(resources.getString(R.string.exception_cancel,
							dateString));
				} else {
					sb.append(resources.getString(R.string.exception_updated,
							dateString));
			...
			String text = EmCalendarUtilities.buildMessageTextFromEntityValues(
					context, entityValues, sb);
			...
			if (!isReply) {
				if (entityValues.containsKey(Events.ALL_DAY)) {
					Integer ade = entityValues.getAsInteger(Events.ALL_DAY);
					ics.writeTag("X-MICROSOFT-CDO-ALLDAYEVENT",
							ade == 0 ? "FALSE" : "TRUE");
				}
				...
				// If we decide to send alarm information in the meeting request
				// ics file,
				// handle it here by looping through the subvalues
			}

			// Handle attendee data here; determine "to" list and add ATTENDEE
			// tags to ics
			...
			for (NamedContentValues ncv : subValues) {
				...
				if (ncvUri.equals(Attendees.CONTENT_URI)) {
					...
					if (relationship != null
						...
						addAttendeeToMessage(ics, toList, attendeeName,
								attendeeEmail, messageFlag, account);
					}
				}
			}

			// Manually add the specifiedAttendee if he wasn't added in the
			// Attendees loop
			if (toList.isEmpty() && (specifiedAttendee != null)) {
				addAttendeeToMessage(ics, toList, null, specifiedAttendee,
						messageFlag, account);
			}
			...
			if (organizerEmail != null) {
				String icalTag = "ORGANIZER";
				// We should be able to find this, assuming the Email is the
				// user's email
				// TODO Find this in the account
				if (organizerName != null) {
					icalTag += ";CN="
				...
				ics.writeTag(icalTag, "MAILTO:" + organizerEmail);
				if (isReply) {
					toList.add(organizerName == null ? new EmAddress(
							organizerEmail) : new EmAddress(organizerEmail,
							organizerName));
				}
			}
	 ...
	 * 
	 * @param cr
	 *            a content resolver that can be used to query for the Event
	 * @param eventId
	 ...
	 * @param the
	 *            user's account
	 * @param requireAddressees
	 *            if true (the default), no Message is returned if there aren't
	 *            any addressees; if false, return the Message regardless
	 *            (addressees will be filled in later)
	 * @return a Message with many fields pre-filled (more later)
	 * @throws RemoteException
	 *             if there is an issue retrieving the Event from
	 *             CalendarProvider
	 */
	static public EmEmailContent.Message createMessageForEventId(
			Context context, long eventId, int messageFlag, String uid,
			Account account) throws RemoteException {
		return createMessageForEventId(context, eventId, messageFlag, uid,
				account, null /* specifiedAttendee */);
	}

	static public EmEmailContent.Message createMessageForEventId(
			Context context, long eventId, int messageFlag, String uid,
			Account account, String specifiedAttendee) throws RemoteException {
		ContentResolver cr = context.getContentResolver();
		EntityIterator eventIterator = EventsEntity
				.newEntityIterator(
						cr.query(
								ContentUris
										.withAppendedId(
												Events.CONTENT_URI
														...
														.appendQueryParameter(
																com.cognizant.trumobi.common.provider.Calendar.CALLER_IS_SYNCADAPTER,
																"true").build(),
		...
		try {
			while (eventIterator.hasNext()) {
				...
				return createMessageForEntity(context, entity, messageFlag,
						uid, account, specifiedAttendee);
			}
		} finally {
			eventIterator.close();
		}
	 ...
	 * @param columnName
	 *            the name of a column to be found in the ContentValues
	 * @return a boolean representation of the value of columnName in values;
	 *         null and 0 = false, other integers = true
	 */
	static public boolean getIntegerValueAsBoolean(ContentValues values,
			String columnName) {
