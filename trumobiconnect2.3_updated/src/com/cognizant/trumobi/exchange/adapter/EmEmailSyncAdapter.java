 ...
 *
 */
public class EmEmailSyncAdapter extends EmAbstractSyncAdapter {
    ...
    public EmEmailSyncAdapter(Mailbox mailbox, EmEasSyncService service) {
    ...
    public class EasEmailSyncParser extends EmAbstractSyncParser {
        ...
        public EasEmailSyncParser(InputStream in, EmEmailSyncAdapter adapter) throws IOException {
            super(in, adapter);
        ...
        public void addData (Message msg) throws IOException {
            ...
            while (nextTag(EmTags.SYNC_APPLICATION_DATA) != END) {
                switch (tag) {
                    case EmTags.EMAIL_ATTACHMENTS:
                    case EmTags.BASE_ATTACHMENTS: // BASE_ATTACHMENTS is used in EAS 12.0 and up
                        attachmentsParser(atts, msg);
                        break;
                    case EmTags.EMAIL_TO:
                        msg.mTo = EmAddress.pack(EmAddress.parse(getValue()));
                        break;
                    case EmTags.EMAIL_FROM:
                        EmAddress[] froms = EmAddress.parse(getValue());
                        if (froms != null && froms.length > 0) {
                          msg.mDisplayName = froms[0].toFriendly();
                        }
                        msg.mFrom = EmAddress.pack(froms);
                        break;
                    case EmTags.EMAIL_CC:
                        msg.mCc = EmAddress.pack(EmAddress.parse(getValue()));
                        break;
                    case EmTags.EMAIL_REPLY_TO:
                        msg.mReplyTo = EmAddress.pack(EmAddress.parse(getValue()));
                        break;
                    case EmTags.EMAIL_DATE_RECEIVED:
                        msg.mTimeStamp = EmUtility.parseEmailDateTimeToMillis(getValue());
                        break;
                    case EmTags.EMAIL_SUBJECT:
                        msg.mSubject = getValue();
                        break;
                    case EmTags.EMAIL_READ:
                        msg.mFlagRead = getValueInt() == 1;
                        break;
                    case EmTags.BASE_BODY:
                        bodyParser(msg);
                        break;
                    case EmTags.EMAIL_FLAG:
                        msg.mFlagFavorite = flagParser();
                        break;
                    case EmTags.EMAIL_BODY:
                        String text = getValue();
                        msg.mText = text;
                        break;
                    case EmTags.EMAIL_MESSAGE_CLASS:
                        ...
                        break;
                    case EmTags.EMAIL_MEETING_REQUEST:
        ...
        private void meetingRequestParser(Message msg) throws IOException {
            EmPackedString.Builder packedString = new EmPackedString.Builder(); 
            while (nextTag(EmTags.EMAIL_MEETING_REQUEST) != END) {
                switch (tag) {
                    case EmTags.EMAIL_DTSTAMP:
                        packedString.put(EmMeetingInfo.MEETING_DTSTAMP, getValue());
                        break;
                    case EmTags.EMAIL_START_TIME:
                        packedString.put(EmMeetingInfo.MEETING_DTSTART, getValue());
                        break;
                    case EmTags.EMAIL_END_TIME:
                        packedString.put(EmMeetingInfo.MEETING_DTEND, getValue());
                        break;
                    case EmTags.EMAIL_ORGANIZER:
                        packedString.put(EmMeetingInfo.MEETING_ORGANIZER_EMAIL, getValue());
                        break;
                    case EmTags.EMAIL_LOCATION:
                        packedString.put(EmMeetingInfo.MEETING_LOCATION, getValue());
                        break;
                    case EmTags.EMAIL_GLOBAL_OBJID:
                        packedString.put(EmMeetingInfo.MEETING_UID,
                                EmCalendarUtilities.getUidFromGlobalObjId(getValue()));
                        break;
                    case EmTags.EMAIL_CATEGORIES:
                        nullParser();
                        break;
                    case EmTags.EMAIL_RECURRENCES:
                        recurrencesParser();
                        break;
                    case EmTags.EMAIL_RESPONSE_REQUESTED:	//NaGa
                        packedString.put(EmMeetingInfo.MEETING_RESPONSE_REQUESTED, getValue());
                        break;
                    case EmTags.EMAIL_ALL_DAY_EVENT:
                        if (getValueInt() == 1) {
                            packedString.put(EmMeetingInfo.MEETING_ALL_DAY, "1");
                        }
                        break;
                    default:
                        skipTag();
            ...
            if (msg.mSubject != null) {
                packedString.put(EmMeetingInfo.MEETING_TITLE, msg.mSubject);
            }
            msg.mMeetingInfo = packedString.toString();
        }

        private void nullParser() throws IOException {
            while (nextTag(EmTags.EMAIL_CATEGORIES) != END) {
                skipTag();
            }
        }

        private void recurrencesParser() throws IOException {
            while (nextTag(EmTags.EMAIL_RECURRENCES) != END) {
                switch (tag) {
                    case EmTags.EMAIL_RECURRENCE:
                        nullParser();
                        break;
                    default:
                        skipTag();
        ...
        private Boolean flagParser() throws IOException {
            Boolean state = false;
            while (nextTag(EmTags.EMAIL_FLAG) != END) {
                switch (tag) {
                    case EmTags.EMAIL_FLAG_STATUS:
                        state = getValueInt() == 2;
                        break;
                    default:
                        skipTag();
        ...
        private void bodyParser(Message msg) throws IOException {
            String bodyType = EmEas.BODY_PREFERENCE_TEXT;
            String body = "";
            while (nextTag(EmTags.EMAIL_BODY) != END) {
                switch (tag) {
                    case EmTags.BASE_TYPE:
                        bodyType = getValue();
                        break;
            ...
            if (bodyType.equals(EmEas.BODY_PREFERENCE_HTML)) {
                msg.mHtml = body;
            } else {
                msg.mText = body;
            }
        ...
        private void attachmentsParser(ArrayList<Attachment> atts, Message msg) throws IOException {
            while (nextTag(EmTags.EMAIL_ATTACHMENTS) != END) {
                switch (tag) {
                    case EmTags.EMAIL_ATTACHMENT:
                    case EmTags.BASE_ATTACHMENT:  // BASE_ATTACHMENT is used in EAS 12.0 and up
        ...
        private void attachmentParser(ArrayList<Attachment> atts, Message msg) throws IOException {
            ...
            while (nextTag(EmTags.EMAIL_ATTACHMENT) != END) {
                switch (tag) {
                    // We handle both EAS 2.5 and 12.0+ attachments here
                    case EmTags.EMAIL_DISPLAY_NAME:
                    case EmTags.BASE_DISPLAY_NAME:
                        fileName = getValue();
                        break;
                    case EmTags.EMAIL_ATT_NAME:
                    case EmTags.BASE_FILE_REFERENCE:
                        location = getValue();
                        break;
                    case EmTags.EMAIL_ATT_SIZE:
                    case EmTags.BASE_ESTIMATED_DATA_SIZE:
                        length = getValue();
                        break;
                    default:
                        skipTag();
            ...
            if ((fileName != null) && (length != null) && (location != null)) {
                ...
                /*Log.v("att.mSize", "att.mSize "+att.mSize);
                Log.v("att.mFileName", "att.mFileName "+att.mFileName);
                Log.v("att.mLocation", "att.mLocation "+att.mLocation);*/
            }
        ...
        public void commandsParser() throws IOException {
            while (nextTag(EmTags.SYNC_COMMANDS) != END) {
                ...
                } else if (tag == EmTags.SYNC_DELETE || tag == EmTags.SYNC_SOFT_DELETE) {
        ...
        public void commit() {
            ...
            for (Long id : deletedEmails) {
                ...
                EmAttachmentProvider.deleteAllAttachmentFiles(mContext, mAccount.mId, id);
            }
            ...
            if (notifyCount > 0) {
                ...
                ContentValues cv = new ContentValues();
                cv.put(EmEmailContent.FIELD_COLUMN_NAME, AccountColumns.NEW_MESSAGE_COUNT);
                cv.put(EmEmailContent.ADD_COLUMN_NAME, notifyCount);
                Uri uri = ContentUris.withAppendedId(Account.ADD_TO_FIELD_URI, mAccount.mId);
                mContentResolver.update(uri, cv, null, null);
                EmMailService.actionNotifyNewMessages(mContext, mAccount.mId);
            }
        }
    }
        ...
        if (!mDeletedIdList.isEmpty() || !mUpdatedIdList.isEmpty()) {
            ...
            try {
                mContext.getContentResolver()
                    .applyBatch(EmEmailProvider.EMAIL_AUTHORITY, ops);
            } catch (RemoteException e) {
                // There is nothing to be done here; fail by returning null
            } catch (OperationApplicationException e) {
                // There is nothing to be done here; fail by returning null
     ...
     * @throws IOException
     */
    boolean sendDeletedItems(EmSerializer s, ArrayList<Long> deletedIds, boolean first)
            throws IOException {
        ...
        try {
            while (c.moveToNext()) {
                ...
                s.start(EmTags.SYNC_DELETE).data(EmTags.SYNC_SERVER_ID, serverId).end();
        ...
        try {
            while (c.moveToNext()) {
                ...
                try {
                    ...
                    s.start(EmTags.SYNC_CHANGE)
                        .data(EmTags.SYNC_SERVER_ID, c.getString(Message.LIST_SERVER_ID_COLUMN))
                        .start(EmTags.SYNC_APPLICATION_DATA);
                    if (readChange) {
                        s.data(EmTags.EMAIL_READ, Integer.toString(read));
                    }
                    ...
                    if (flagChange) {
                        if (flag != 0) {
                            // Status 2 = set flag
                            s.start(EmTags.EMAIL_FLAG).data(EmTags.EMAIL_FLAG_STATUS, "2");
                            // "FollowUp" is the standard type
                            s.data(EmTags.EMAIL_FLAG_TYPE, "FollowUp");
                            ...
                            s.data(EmTags.TASK_START_DATE, utc).data(EmTags.TASK_UTC_START_DATE, utc);
                            ...
                            s.data(EmTags.TASK_DUE_DATE, utc).data(EmTags.TASK_UTC_DUE_DATE, utc);
                            s.end();
                        } else {
                            s.tag(EmTags.EMAIL_FLAG);
                        }
                    }
