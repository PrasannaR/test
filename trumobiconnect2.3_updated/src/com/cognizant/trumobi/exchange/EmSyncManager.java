// Setting persona Pref value false, on deleting Account
...
import com.cognizant.trumobi.em.mail.transport.EmSSLUtils;
...
import com.cognizant.trumobi.em.utility.EmTrustManagerFactory;
import com.cognizant.trumobi.em.utility.EmTrustedSocketFactory;
 ...
 * PWD_CHANGE_IMPL		367712		Warn user when Account password changed on Server
 *
 */

/**
 * The SyncManager handles all aspects of starting, maintaining, and stopping
 * the various sync adapters used by Exchange. However, it is capable of handing
 * any kind of email sync, and it would be appropriate to use for IMAP push,
 * when that functionality is added to the Email application.
 * 
 * The Email application communicates with EAS sync adapters via SyncManager's
 * binder interface, which exposes UI-related functionality to the application
 * (see the definitions below)
 * 
 * SyncManager uses ContentObservers to detect changes to accounts, mailboxes,
 * and messages in order to maintain proper 2-way syncing of data. (More
 * documentation to follow)
 * 
 */
public class EmSyncManager extends Service implements Runnable {
	...
	private static final int CONNECTIVITY_WAIT_TIME = 10 * SECONDS;
	...
	private static final String WHERE_PUSH_OR_PING_NOT_ACCOUNT_MAILBOX = MailboxColumns.ACCOUNT_KEY
			+ "=? and "
			+ MailboxColumns.TYPE
			+ "!="
			...
			+ ')';
	protected static final String WHERE_IN_ACCOUNT_AND_PUSHABLE = MailboxColumns.ACCOUNT_KEY
			+ "=? and type in ("
			+ Mailbox.TYPE_INBOX
			+ ','
			+ Mailbox.TYPE_EAS_ACCOUNT_MAILBOX
	...
	protected static final String WHERE_IN_ACCOUNT_AND_TYPE_INBOX = MailboxColumns.ACCOUNT_KEY
			+ "=? and type = " + Mailbox.TYPE_INBOX;
	...
	private static final String WHERE_PROTOCOL_EAS = HostAuthColumns.PROTOCOL
			+ "=\"" + EmAbstractSyncService.EAS_PROTOCOL + "\"";
	...
	private static final String ACCOUNT_KEY_IN = MailboxColumns.ACCOUNT_KEY
			+ " in (";
	private static final String WHERE_CALENDAR_ID = Events.CALENDAR_ID + "=?";

	// Offsets into the syncStatus data for EAS that indicate type, exit status,
	// and change count
	...
	// We synchronize on this for all actions affecting the service and error
	// maps
	...
	private HashMap<Long, EmAbstractSyncService> mServiceMap = new HashMap<Long, EmAbstractSyncService>();
	// Keeps track of services whose last sync ended with an error (by mailbox
	// id)
	...
	private ConcurrentHashMap<Long, CalendarObserver> mCalendarObservers = new ConcurrentHashMap<Long, CalendarObserver>();

	private ContentResolver mResolver;

	// The singleton SyncManager object, with its thread and stop flag
	protected static EmSyncManager INSTANCE;
	...
	private DataConnectivityReceiver mBackgroundDataSettingReceiver = null;
	private volatile boolean mBackgroundData = true;
	private static EmExchangeData mExchangeData;
	// The callback sent in from the UI using setCallback
	private IEmailServiceCallback mCallback;
	private RemoteCallbackList<IEmailServiceCallback> mCallbackList = new RemoteCallbackList<IEmailServiceCallback>();

	// New Dialer DB Implementation

	private CallLogObserver callLogObserver = null;
	private DBThread callLogThread = new DBThread();

	/**
	 * Proxy that can be used by various sync adapters to tie into SyncManager's
	 * callback system. Used this way:
	 * SyncManager.callback().callbackMethod(args...); The proxy wraps checking
	 * for existence of a SyncManager instance and an active callback. Failures
	 * of these callbacks can be safely ignored.
	 */
	static private final IEmailServiceCallback.Stub sCallbackProxy = new IEmailServiceCallback.Stub() {

		public void loadAttachmentStatus(long messageId, long attachmentId,
				int statusCode, int progress) throws RemoteException {
			IEmailServiceCallback cb = INSTANCE == null ? null
					: INSTANCE.mCallback;
			if (cb != null) {
				cb.loadAttachmentStatus(messageId, attachmentId, statusCode,
						progress);
			}
		}

		public void sendMessageStatus(long messageId, String subject,
				int statusCode, int progress) throws RemoteException {
			IEmailServiceCallback cb = INSTANCE == null ? null
					: INSTANCE.mCallback;
			if (cb != null) {
		...
		public void syncMailboxListStatus(long accountId, int statusCode,
		...
		public void syncMailboxStatus(long mailboxId, int statusCode,
				int progress) throws RemoteException {
			IEmailServiceCallback cb = INSTANCE == null ? null
					: INSTANCE.mCallback;
			if (cb != null) {
	...
	private final IEmailService.Stub mBinder = new IEmailService.Stub() {
		...
		public int validate(String protocol, String host, String userName,
				String password, int port, boolean ssl,
				boolean trustCertificates) throws RemoteException {
			try {
				EmAbstractSyncService.validate(EmEasSyncService.class, host,
						userName, password, port, ssl, trustCertificates,
						EmSyncManager.this);
				return EmMessagingException.NO_ERROR;
			} catch (EmMessagingException e) {
				e.printStackTrace();
				EmailLog.e("secureeeeeeeeeeeeeeeeeee",
		...
		public Bundle autoDiscover(String userName, String password)
			...
			return new EmEasSyncService().tryAutodiscover(userName, password);
		}

		public void startSync(long mailboxId) throws RemoteException {
			EmSyncManager syncManager = INSTANCE;
			if (syncManager == null)
				return;
			...
			if (m.mType == Mailbox.TYPE_OUTBOX) {
				// We're using SERVER_ID to indicate an error condition (it has
				// no other use for
				// sent mail) Upon request to sync the Outbox, we clear this so
				// that all messages
				// are candidates for sending.
				ContentValues cv = new ContentValues();
				cv.put(SyncColumns.SERVER_ID, 0);
				syncManager.getContentResolver().update(Message.CONTENT_URI,
						cv, WHERE_MAILBOX_KEY,
						new String[] { Long.toString(mailboxId) });
				// Clear the error state; the Outbox sync will be started from
				// checkMailboxes
			...
			startManualSync(mailboxId, EmSyncManager.SYNC_SERVICE_START_SYNC,
					null);
		}
		...
		public void loadAttachment(long attachmentId, String destinationFile,
			...
			Attachment att = Attachment.restoreAttachmentWithId(
					EmSyncManager.this, attachmentId);
			sendMessageRequest(new EmPartRequest(att, destinationFile,
					contentUriString));
		}

		public void updateFolderList(long accountId) throws RemoteException {
			reloadFolderList(EmSyncManager.this, accountId, false);
		}
		...
		public void hostChanged(long accountId) throws RemoteException {
			...
			synchronized (sSyncLock) {
				...
				for (long mailboxId : syncErrorMap.keySet()) {
					...
					Mailbox m = Mailbox.restoreMailboxWithId(syncManager,
							mailboxId);
					// If it's for the account whose host has changed, clear the
					// error
					// If the mailbox is no longer around, remove the entry in
					// the map
					if (m == null) {
		...
		public void setLogging(int on) throws RemoteException {
			EmEas.setUserDebug(on);
		}

		public void sendMeetingResponse(long messageId, int response)
				throws RemoteException {
			sendMessageRequest(new EmMeetingResponseRequest(messageId, response));
		}
		...
		public boolean sendOofSettings(long accountId, boolean enable,
			...
			try {
				return EmAbstractSyncService.sendOof(EmEasSyncService.class,
						accountId, enable, replyMessage, startDate, endDate,
						EmSyncManager.this);
			} catch (EmMessagingException e) {
		...
		public boolean getOofSettings(long accountId) throws RemoteException {
			// TODO Auto-generated method stub
			try {
				return EmAbstractSyncService.getOof(EmEasSyncService.class,
						accountId, EmSyncManager.this);
			} catch (EmMessagingException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	class AccountObserver extends ContentObserver {
		...
		/**
		 * Returns a String suitable for appending to a where clause that
		 * selects for all syncable mailboxes in all eas accounts
		 * 
		...
		/**
		 * Returns a String suitable for appending to a where clause that
		 * selects for all eas accounts.
		 * 
		 * @return a String in the form "accountKey in (a, b, c...)" that is not
		 *         to be cached
		 */
		...
		private void onAccountChanged() {
			...
			// A change to the list requires us to scan for deletions (stop
			// running syncs)
				...
				synchronized (mAccountList) {
					for (Account account : mAccountList) {
						...
						if ((account.mFlags & Account.FLAGS_INCOMPLETE) != 0) {
							...
							// This is a deletion; shut down any account-related
							// syncs
							...
							AccountManager.get(EmSyncManager.this)
									.removeAccount(acct, null, null);

							new SharedPreferences(Email.getAppContext())
									.edit()
									.putBoolean("isEmailAccountConfigured",
											false).commit();
							...
							if (account.mSyncInterval != updatedAccount.mSyncInterval
								...
								getContentResolver().update(
										...
										new String[] { Long
												.toString(account.mId) });
								// Stop all current syncs; the appropriate ones
								// will restart
								log("Account " + account.mDisplayName
										+ " changed; stop syncs");
								stopAccountSyncs(account.mId, true);
							}
							...
							if (onSecurityHold(account)
									&& !onSecurityHold(updatedAccount)) {
								releaseSyncHolds(
										EmSyncManager.this,
										EmAbstractSyncService.EXIT_SECURITY_FAILURE,
										account);
							}
					...
					for (Account account : currentAccounts) {
						if (!mAccountList.contains(account.mId)) {
							...
							HostAuth ha = HostAuth.restoreHostAuthWithId(
									getContext(), account.mHostAuthKeyRecv);
							if (ha == null)
								continue;
							account.mHostAuthRecv = ha;
							// This is an addition; create our magic hidden
							// mailbox...
							log("Account observer found new account: "
		...
		private void addAccountMailbox(long acctId) {
			...
			main.mDisplayName = EmEas.ACCOUNT_MAILBOX_PREFIX;
			main.mServerId = EmEas.ACCOUNT_MAILBOX_PREFIX + System.nanoTime();
	...
	/**
	 * Register a specific Calendar's data observer; we need to recognize when
	 * the SYNC_EVENTS column has changed (when sync has turned off or on)
	 * 
	 * @param account
	 *            the Account whose Calendar we're observing
	 */
	private void registerCalendarObserver(Account account) {
		...
		// // If we find the Calendar (and we'd better) register it and store it
		// in the map
		// mCalendarObservers.put(account.mId, observer);
		// mResolver.registerContentObserver(
		// ContentUris.withAppendedId(Event.CONTENT_URI, observer.mCalendarId),
		// false,
		// observer);
		// }
	}
	...
	static public void unregisterCalendarObservers() {
		...
		// for (CalendarObserver observer:
		// syncManager.mCalendarObservers.values()) {
	...
	/**
	 * Return the syncable state of an account's calendar, as determined by the
	 * sync_events column of our Calendar (from CalendarProvider2) Note that the
	 * current state of sync_events is cached in our CalendarObserver
	 * 
	 * @param accountId
	 *            the id of the account whose calendar we are checking
	...
	private boolean isCalendarEnabled(long accountId) {
		...
		// If there's no observer, there's no Calendar in CalendarProvider2, so
		// we return true
	...
	private class CalendarObserver extends ContentObserver {
		...
		public CalendarObserver(Handler handler, Account account) {
			...
			Cursor c = mResolver.query(Calendars.CONTENT_URI, new String[] {
					Calendars._ID, Calendars.SYNC_EVENTS },
					EmCalendarSyncAdapter.CALENDAR_SELECTION, new String[] {
							account.mEmailAddress,
							Email.EXCHANGE_ACCOUNT_MANAGER_TYPE }, null);
			if (c != null) {
			...
			if (!selfChange) {
				new Thread(new Runnable() {
					public void run() {
						...
						if (c == null)
							return;
						// Get its sync events; if it's changed, we've got work
						// to do
						try {
							if (c.moveToFirst()) {
								long newSyncEvents = c.getLong(0);
								if (newSyncEvents != mSyncEvents) {
									log("_sync_events changed for calendar in "
											+ mAccountName);
									Mailbox mailbox = Mailbox.restoreMailboxOfType(
											INSTANCE, mAccountId,
											Mailbox.TYPE_CALENDAR);
									...
									if (mailbox == null)
										return;
									if (newSyncEvents == 0) {
										// When sync is disabled, we're supposed
										// to delete
										...
										EmEasSyncService service = new EmEasSyncService(
												INSTANCE, mailbox);
										EmCalendarSyncAdapter adapter = new EmCalendarSyncAdapter(
												mailbox, service);
										try {
										...
										// parameter so that the deletion is
										// only local
										Uri eventsAsSyncAdapter = Events.CONTENT_URI
												.buildUpon()
												.appendQueryParameter(
														Calendar.CALLER_IS_SYNCADAPTER,
														"true").build();
										mResolver.delete(
												eventsAsSyncAdapter,
												WHERE_CALENDAR_ID,
												new String[] { Long
														.toString(mCalendarId) });
									} else {
										// If we're in a ping, stop it so that
										// calendar sync can
										// start right away
										stopPing(mAccountId);
	...
	private class SyncedMessageObserver extends ContentObserver {
		Intent syncAlarmIntent = new Intent(INSTANCE,
				EmEmailSyncAlarmReceiver.class);
		PendingIntent syncAlarmPendingIntent = PendingIntent.getBroadcast(
				INSTANCE, 0, syncAlarmIntent, 0);
	...
	private class MessageObserver extends ContentObserver {
		...
		@Override
		public void onChange(boolean selfChange) {
			// A rather blunt instrument here. But we don't have information
			// about the URI that
	...
	/***
	 * 
	 * @author 361572 New Dialer DB Implementation
	 * 
	 */
	...
	public class CallLogObserver extends ContentObserver {
		...
		public void onChange(boolean selfChange) {
			...
			Log.w("Dialer Call Log",
					" ********** CallLog Observer *************");
			if (callLogThread != null)
				new Thread(callLogThread).start();
	...
	private class DBThread implements Runnable {
		...
		public void run() {
			synchronized (this) {				
				DialerParentActivity.insertCallLogDB();
			}
	...
	private void logSyncHolds() {
		if (EmEas.USER_LOG && !mSyncErrorMap.isEmpty()) {
			log("Sync holds:");
			...
			synchronized (sSyncLock) {
				for (long mailboxId : mSyncErrorMap.keySet()) {
					...
					if (m == null) {
						...
						log("Mailbox " + m.mDisplayName + ", error = "
								+ error.reason + ", fatal = " + error.fatal);
						if (error.holdEndTime > 0) {
							log("Hold ends in "
									+ ((error.holdEndTime - time) / 1000) + "s");
						}
					}
				}
			}
	 ...
	 * 
	 * @param account
	 *            the account whose Mailboxes should be released from security
	 *            hold
	 */
	static public void releaseSecurityHold(Account account) {
		EmSyncManager syncManager = INSTANCE;
		if (syncManager != null) {
			syncManager.releaseSyncHolds(INSTANCE,
					EmAbstractSyncService.EXIT_SECURITY_FAILURE, account);
		}
	}

	/**
	 * Release a specific type of hold (the reason) for the specified Account;
	 * if the account is null, mailboxes from all accounts with the specified
	 * hold will be released
	 * 
	 * @param reason
	 *            the reason for the SyncError (AbstractSyncService.EXIT_XXX)
	 * @param account
	 *            an Account whose mailboxes should be released (or all if null)
	 */
	/* package */void releaseSyncHolds(Context context, int reason,
			Account account) {
		releaseSyncHoldsImpl(context, reason, account);
		kick("security release");
	}

	private void releaseSyncHoldsImpl(Context context, int reason,
			Account account) {
		synchronized (sSyncLock) {
			...
			for (long mailboxId : mSyncErrorMap.keySet()) {
				if (account != null) {
					Mailbox m = Mailbox
							.restoreMailboxWithId(context, mailboxId);
					if (m == null) {
	...
	public class EasSyncStatusObserver implements SyncStatusObserver {
		public void onStatusChanged(int which) {
			// We ignore the argument (we can only get called in one case - when
			// settings change)
			if (INSTANCE != null) {
	...
	/**
	 * The reconciler (which is called from this listener) can make blocking
	 * calls back into the account manager. So, in this callback we spin up a
	 * worker thread to call the reconciler.
	 */
	...
	/**
	 * Non-blocking call to run the account reconciler. Launches a worker
	 * thread, so it may be called from UI thread.
	 */
	private void runAccountReconciler() {
		final EmSyncManager syncManager = this;
		new Thread() {
			@Override
			public void run() {
				android.accounts.Account[] accountMgrList = AccountManager.get(
						syncManager).getAccountsByType(
						Email.EXCHANGE_ACCOUNT_MANAGER_TYPE);
				synchronized (mAccountList) {
					// Make sure we have an up-to-date sAccountList. If not (for
					// example, if the
					// service has been destroyed), we would be reconciling
					// against an empty account
					// list, which would cause the deletion of all of our
					// accounts
	 ...
	 * create it as droid<n> where <n> is system time. This would work on a real
	 * device as well, but it would be better to use the "real" id if it's
	 * available
	 */
	...
	/*
	 * static public synchronized String getDeviceId(Context context) throws
	 * IOException { if (sDeviceId == null) { sDeviceId =
	 * getDeviceIdInternal(context); } return sDeviceId; }
	 */
	...
	static public synchronized String getDeviceId(Context context)
		...
		if (sDeviceId == null) {
			...
			sDeviceId = EmUtility.getConsistentDeviceId(context);
			if (sDeviceId != null) {
				// Use different prefix from random IDs.
				sDeviceId = "androidc" + sDeviceId;
			} else {
				sDeviceId = "android" + System.currentTimeMillis();
			}
	...
	static private String getDeviceIdInternal(Context context)
		...
		try {
			...
			if (f.exists() && f.canRead()) {
				...
				final String consistentDeviceId = EmUtility
	...
	static public synchronized ClientConnectionManager getClientConnectionManager() {
		Context thisContext = Email.getAppContext();
		if (sClientConnectionManager == null ||
				(EmTrustManagerFactory.getHostAddress() 
				!= new SharedPreferences(thisContext).getString("server", ""))) {
			...
			registry.register(new Scheme("http", PlainSocketFactory
					.getSocketFactory(), 80));
			/*
			 * registry.register(new Scheme("https", EmSSLSocketFactory
			 * .getSocketFactory(), 443));
			 */
			// Use "insecure" socket factory.

			EmSSLSocketFactory sf = new EmSSLSocketFactory(
					EmSSLUtils.getSSLSocketFactory(true));
			sf.setHostnameVerifier(EmSSLSocketFactory.STRICT_HOSTNAME_VERIFIER);
			// (EmWelcome.app ==
														// null)?
														// getContext():EmWelcome.app;
			try {
				if (!EmWelcome.useCertBasedSetup) {
					...
					registry.register(new Scheme("https",
							new EmTrustedSocketFactory(getServerAddress(),
									true, thisContext), 443));
					registry.register(new Scheme("httpts",
							new EmTrustedSocketFactory(getServerAddress(),
									true, thisContext), 443));
				} else {
					setupCertData(thisContext);
					// try {
					registry.register(new Scheme("https",
							new EmTrustedSocketFactory(mExchangeData
									.getServer(), true, thisContext), 443));
				}
	...
	private static void setupCertData(Context ctx) {
		mExchangeData = EmExchangeData.getInstance(ctx);
				...
				if (box != null) {
					if (box.mAccountKey == acctId) {
						if (!includeAccountMailbox
							...
							EmAbstractSyncService svc = mServiceMap.get(mid);
							if (svc != null) {
								svc.stop();
							}
							continue;
	...
	static private void reloadFolderListFailed(long accountId) {
		try {
			callback().syncMailboxListStatus(accountId,
					EmEmailServiceStatus.ACCOUNT_UNINITIALIZED, 0);
	...
	static public void reloadFolderList(Context context, long accountId,
		...
		Cursor c = context
				.getContentResolver()
				.query(Mailbox.CONTENT_URI,
						Mailbox.CONTENT_PROJECTION,
						MailboxColumns.ACCOUNT_KEY + "=? AND "
								+ MailboxColumns.TYPE + "=?",
						new String[] { Long.toString(accountId),
								Long.toString(Mailbox.TYPE_EAS_ACCOUNT_MAILBOX) },
						null);
		try {
			if (c.moveToFirst()) {
				synchronized (sSyncLock) {
					Mailbox m = new Mailbox().restore(c);
					Account acct = Account.restoreAccountWithId(context,
							accountId);
					if (acct == null) {
					...
					context.getContentResolver().update(Mailbox.CONTENT_URI,
							cv, WHERE_PUSH_OR_PING_NOT_ACCOUNT_MAILBOX,
							new String[] { Long.toString(accountId) });
					log("Set push/ping boxes to push/hold");

					long id = m.mId;
					EmAbstractSyncService svc = syncManager.mServiceMap.get(id);
	...
	/**
	 * Informs SyncManager that an account has a new folder list; as a result,
	 * any existing folder might have become invalid. Therefore, we act as if
	 * the account has been deleted, and then we reinitialize it.
	 * 
	 * @param acctId
	 */
	...
	static public String alarmOwner(long id) {
		if (id == SYNC_MANAGER_ID) {
			...
			if (EmEas.USER_LOG && INSTANCE != null) {
	...
	private void setAlarm(long id, long millis) {
		synchronized (mPendingIntents) {
			PendingIntent pi = mPendingIntents.get(id);
			if (pi == null) {
				Intent i = new Intent(this, EmMailboxAlarmReceiver.class);
				...
				// log("+Alarm set for " + alarmOwner(id) + ", " + millis/1000 +
				// "s");
			}
		}
	}
	...
	static public void wakelockRelease(long accountId) { // Battery Drain fix
		EmSyncManager syncManager = INSTANCE;
		if (syncManager != null) {
			// exchangeService.setAlarm(id);
			syncManager.releaseWakeLock(accountId);
		}
	}
	...
	static public void alert(Context context, final long id) {
		final EmSyncManager syncManager = INSTANCE;
		checkSyncManagerServiceRunning();
		if (id < 0) {
			...
			final EmAbstractSyncService service = syncManager.mServiceMap
					.get(id);
			if (service != null) {
				// Handle alerts in a background thread, as we are typically
				// called from a
				// broadcast receiver, and are therefore running in the UI
				// thread
				...
				new Thread(new Runnable() {
					public void run() {
						Mailbox m = Mailbox.restoreMailboxWithId(syncManager,
								id);
						if (m != null) {
							// We ignore drafts completely (doesn't sync).
							// Changes in Outbox are
							// handled in the checkMailboxes loop, so we can
							// ignore these pings.
							if (m.mType == Mailbox.TYPE_DRAFTS
									|| m.mType == Mailbox.TYPE_OUTBOX) {
								String[] args = new String[] { Long.toString(m.mId) };
								ContentResolver resolver = INSTANCE.mResolver;
								resolver.delete(Message.DELETED_CONTENT_URI,
										WHERE_MAILBOX_KEY, args);
								resolver.delete(Message.UPDATED_CONTENT_URI,
										WHERE_MAILBOX_KEY, args);
								return;
							}
							service.mAccount = Account.restoreAccountWithId(
									INSTANCE, m.mAccountKey);
							service.mMailbox = m;
							// Send the alarm to the sync service
							if (!service.alarm()) {
								// A false return means that we were forced to
								// interrupt the thread
								// In this case, we release the mailbox so that
								...
								// Shutdown the connection manager; this should
								// close all of our
								// sockets and generate IOExceptions all around.
								EmSyncManager.shutdownConnectionManager();
							}
						}
					}
				}, threadName).start();
	...
	/**
	 * See if we need to change the syncInterval for any of our PIM mailboxes
	 * based on changes to settings in the AccountManager (sync settings). This
	 * code is called 1) when SyncManager starts, and 2) when SyncManager is
	 * running and there are changes made (this is detected via a
	 * SyncStatusObserver)
	 */
	private void updatePIMSyncSettings(Account providerAccount,
			int mailboxType, String authority) {
		ContentValues cv = new ContentValues();
		long mailboxId = Mailbox.findMailboxOfType(this, providerAccount.mId,
				mailboxType);
		// Presumably there is one, but if not, it's ok. Just move on...
		if (mailboxId != Mailbox.NO_MAILBOX) {
			// Create an AccountManager style Account
			android.accounts.Account acct = new android.accounts.Account(
					providerAccount.mEmailAddress,
					Email.EXCHANGE_ACCOUNT_MANAGER_TYPE);
			...
			if (ContentResolver.getIsSyncable(acct, authority) > 0) {
				// If we're supposed to sync automatically (push), set to push
				// if it's not
				if (ContentResolver.getSyncAutomatically(acct, authority)) {
					if (syncInterval == Mailbox.CHECK_INTERVAL_NEVER
							|| syncInterval > 0) {
						log("Sync for " + mailbox.mDisplayName + " in "
								+ acct.name + ": push");
						cv.put(MailboxColumns.SYNC_INTERVAL,
								Mailbox.CHECK_INTERVAL_PUSH);
					}
					// If we're NOT supposed to push, and we're not set up that
					// way, change it
				} else if (syncInterval != Mailbox.CHECK_INTERVAL_NEVER) {
					log("Sync for " + mailbox.mDisplayName + " in " + acct.name
							+ ": manual");
					cv.put(MailboxColumns.SYNC_INTERVAL,
							Mailbox.CHECK_INTERVAL_NEVER);
				}
			...
			if (cv.containsKey(MailboxColumns.SYNC_INTERVAL)) {
				mResolver.update(ContentUris.withAppendedId(
						Mailbox.CONTENT_URI, mailboxId), cv, null, null);
	...
	private void checkPIMSyncSettings() {
		synchronized (mAccountList) {
			for (Account account : mAccountList) {
				updatePIMSyncSettings(account, Mailbox.TYPE_CONTACTS,
						ContactsConsts.AUTHORITY);
				updatePIMSyncSettings(account, Mailbox.TYPE_CALENDAR,
	 ...
	 * orphan, as these must remain in sync.
	 * 
	 * Note that the duplication of account information is caused by the Email
	 * application's incomplete integration with AccountManager.
	 * 
	 * This function may not be called from the main/UI thread, because it makes
	 * blocking calls into the account manager.
	 * 
	 * @param context
	 *            The context in which to operate
	 * @param cachedEasAccounts
	 *            the exchange provider accounts to work from
	 * @param accountManagerAccounts
	 *            The account manager accounts to work from
	 * @param blockExternalChanges
	 *            FOR TESTING ONLY - block backups, security changes, etc.
	 * @param resolver
	 *            the content resolver for making provider updates (injected for
	 *            testability)
	 */
	/* package */static void reconcileAccountsWithAccountManager(
			Context context, List<Account> cachedEasAccounts,
			android.accounts.Account[] accountManagerAccounts,
			boolean blockExternalChanges, ContentResolver resolver) {
		// First, look through our cached EAS Accounts (from EmailProvider) to
		// make sure there's a
		...
		for (Account providerAccount : cachedEasAccounts) {
			...
			for (android.accounts.Account accountManagerAccount : accountManagerAccounts) {
				if (accountManagerAccount.name
						.equalsIgnoreCase(providerAccountName)) {
					found = true;
					break;
				}
			}
			if (!found) {
				...
				alwaysLog("Account deleted in AccountManager; deleting from provider: "
						+ providerAccountName);
				// TODO This will orphan downloaded attachments; need to handle
				// this
				resolver.delete(ContentUris.withAppendedId(Account.CONTENT_URI, // Naga,
																				// to
																				// remove
																				// account,
																				// after
		...
		for (android.accounts.Account accountManagerAccount : accountManagerAccounts) {
			...
			for (Account cachedEasAccount : cachedEasAccounts) {
				if (cachedEasAccount.mEmailAddress
						.equalsIgnoreCase(accountManagerAccountName)) {
					found = true;
				}
			}
			if (!found) {
				...
				// Delete the account
				AccountManagerFuture<Boolean> blockingResult = AccountManager
						.get(context).removeAccount(accountManagerAccount,
								null, null);
				try {
					// Note: All of the potential errors from removeAccount()
					// are simply logged
		...
		// If we changed the list of accounts, refresh the backup & security
		// settings
	...
	public class DataConnectivityReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(
				...
				ConnectivityManager cm = (ConnectivityManager) EmSyncManager.this
				...
				if (mBackgroundData) {
					...
					synchronized (mAccountList) {
						for (Account account : mAccountList)
							EmSyncManager.stopAccountSyncs(account.mId);
					}
				}
			}
		}
	...
	/**
	 * Starts a service thread and enters it into the service map This is the
	 * point of instantiation of all sync threads
	 * 
	 * @param service
	 *            the service to start
	 * @param m
	 *            the Mailbox on which the service will operate
	 */
	private void startServiceThread(EmAbstractSyncService service, Mailbox m) {
		if (m == null)
			return;
		synchronized (sSyncLock) {
			...
			Thread thread = new Thread(service, mailboxName + "(" + accountName
					+ ")");
			log("Starting thread for " + mailboxName + " in account "
					+ accountName);
			thread.start();
			mServiceMap.put(m.mId, service);
			runAwake(m.mId);
			if ((m.mServerId != null)
					&& !m.mServerId.startsWith(EmEas.ACCOUNT_MAILBOX_PREFIX)) {
			...
			for (long mailboxId : mServiceMap.keySet()) {
				...
				if (m != null) {
					...
					if (m.mAccountKey == accountId
							&& serverId != null
							&& serverId
									.startsWith(EmEas.ACCOUNT_MAILBOX_PREFIX)) {
						// Here's our account mailbox; reset him (stopping
						// pings)
						EmAbstractSyncService svc = mServiceMap.get(mailboxId);
						svc.reset();
					}
				}
			}
	...
	private void requestSync(Mailbox m, int reason, EmRequest req) {
		...
		synchronized (sSyncLock) {
			...
			if (acct != null) {
				// Always make sure there's not a running instance of this
				// service
				EmAbstractSyncService service = mServiceMap.get(m.mId);
				if (service == null) {
					service = new EmEasSyncService(this, m);
					if (!((EmEasSyncService) service).mIsValid)
						return;
	...
	private void stopServiceThreads() {
		synchronized (sSyncLock) {
			...
			for (Long mailboxId : toStop) {
				EmAbstractSyncService svc = mServiceMap.get(mailboxId);
				if (svc != null) {
					log("Stopping " + svc.mAccount.mDisplayName + '/'
							+ svc.mMailbox.mDisplayName);
					svc.stop();
					if (svc.mThread != null) {
	...
	static EmMessageList.ConnectivityCallback mConnct;

	public static void setnetworkCallBack(
			EmMessageList.ConnectivityCallback mCallBack) {
		mConnct = mCallBack;
	}
		...
		while (!sStop) {
			...
			if (info != null) {
				...
				if (waiting) {
					...
					releaseSyncHolds(this, EmAbstractSyncService.EXIT_IO_ERROR,
							null);
				...
				// If this is our first time through the loop, shut down running
				// service threads
				if (!waiting) {
					waiting = true;
					stopServiceThreads();
				}
				// Wait until a network is connected (or 10 mins), but let the
				// device sleep
				// We'll set an alarm just in case we don't get notified (bugs
				// happen)
				synchronized (sConnectivityLock) {
					runAsleep(SYNC_MANAGER_ID, CONNECTIVITY_WAIT_TIME + 5
							* SECONDS);
					try {
	 ...
	 * 
	 * 1) as a background service instantiated via startService (which happens
	 * on boot, when the first EAS account is created, etc), in which case the
	 * service thread is spun up, mailboxes sync, etc. and 2) to execute an RPC
	 * call from the UI, in which case the background service will already be
	 * running most of the time (unless we're creating a first EAS account)
	 * 
	 * If the running background service detects that there are no EAS accounts
	 * (on boot, if none were created, or afterward if the last remaining EAS
	 * account is deleted), it will call stopSelf() to terminate operation.
	 * 
	 * The goal is to ensure that the background service is running at all times
	 * when there is at least one EAS account in existence
	 * 
	 * Because there are edge cases in which our process can crash (typically,
	 * this has been seen in UI crashes, ANR's, etc.), it's possible for the UI
	 * to start up again without the background service having been started. We
	 * explicitly try to start the service in Welcome (to handle the case of the
	 * app having been reloaded). We also start the service on any startSync
	...
	public void onCreate() {
		synchronized (sSyncLock) {
			...
			// Run the reconciler and clean up any mismatched accounts - if we
			// weren't running when
			// accounts were deleted, it won't have been called.
			runAccountReconciler();
			// Work around framework bug in which a call to onStartCommand is
			// not guaranteed unless
			// there is a posted Intent
			startService(new Intent(this, EmSyncManager.class));
		}
	}
	...
	void maybeStartSyncManagerThread() {
		...
		if (EmEmailContent.count(this, HostAuth.CONTENT_URI,
	...
	/**
	 * Start up the SyncManager service if it's not already running This is a
	 * stopgap for cases in which SyncManager died (due to a crash somewhere in
	 * com.cognizant.trumobi.em) and hasn't been restarted. See the comment for
	 * onCreate for details
	 */
	static void checkSyncManagerServiceRunning() {
		...
		if (sServiceThread == null) {
			...
			syncManager.startService(new Intent(syncManager,
					EmSyncManager.class));
		}
	}
	...
	public void run() {
		...
		// Synchronize here to prevent a shutdown from happening while we
		...
		synchronized (sSyncLock) {
			if (INSTANCE != null) {
				...
				// Set up our observers; we need them to know when to start/stop
				// various syncs based
				// on the insert/delete/update of mailboxes and accounts
				// We also observe synced messages to trigger upsyncs at the
				// appropriate time
				mAccountObserver = new AccountObserver(mHandler);
				mResolver.registerContentObserver(Account.CONTENT_URI, true,
						mAccountObserver);
				mMailboxObserver = new MailboxObserver(mHandler);
				mResolver.registerContentObserver(Mailbox.CONTENT_URI, false,
						mMailboxObserver);
				mSyncedMessageObserver = new SyncedMessageObserver(mHandler);
				mResolver.registerContentObserver(Message.SYNCED_CONTENT_URI,
						true, mSyncedMessageObserver);
				mMessageObserver = new MessageObserver(mHandler);
				mResolver.registerContentObserver(Message.CONTENT_URI, true,
						mMessageObserver);

				callLogObserver = new CallLogObserver(mHandler);
				mResolver.registerContentObserver(
				...
				AccountManager.get(getApplication())
						.addOnAccountsUpdatedListener(mAccountsUpdatedListener,
								mHandler, true);

				// unregisterReceivers();

				if (mConnectivityReceiver == null) {
					// Set up receivers for connectivity and background data
					// setting
				...
				if (mBackgroundDataSettingReceiver == null) {
					mBackgroundDataSettingReceiver = new DataConnectivityReceiver();
					registerReceiver(
							mBackgroundDataSettingReceiver,
							new IntentFilter(
									ConnectivityManager.ACTION_BACKGROUND_DATA_SETTING_CHANGED));
				}
				// Save away the current background data setting; we'll keep
				// track of it with the
		...
		try {
			...
			while (!sStop) {
				...
				// Log.e("SyncManager", "Battery Level : " + getBatteryLevel());

				/*
				 * if (getBatteryLevel() < 50) { if (nextWait < 300 * SECONDS)
				 * nextWait = 300 * SECONDS; }
				 */

				Log.e("SyncManager", "nextWaitnextWaitnextWait : " + nextWait);
					...
					synchronized (this) {
						if (!mKicked) {
							...
							if (nextWait > 10 * SECONDS) {
								log("Next awake in " + nextWait / 1000 + "s: "
										+ mNextWaitReason);
								runAsleep(SYNC_MANAGER_ID, nextWait
										+ (3 * SECONDS));
							}
							wait(nextWait);
						}
	...
	private void unregisterReceivers() {
		try {
			...
			if (mConnectivityReceiver != null) {
				...
				mConnectivityReceiver = null; // 367712, crash on 5 times wrong
												// PIN
			}
			if (mBackgroundDataSettingReceiver != null) {
				unregisterReceiver(mBackgroundDataSettingReceiver);
				mBackgroundDataSettingReceiver = null; // 367712, crash on 5
														// times wrong PIN
			}
		} catch (Exception e) {
		}
	...
	private void shutdown() {
		synchronized (sSyncLock) {
			...
			if (INSTANCE != null) {
				...
				// if (mConnectivityReceiver != null) {
				// unregisterReceiver(mConnectivityReceiver);
				// mConnectivityReceiver = null; //367712, crash on 5 times
				// wrong PIN
				// }
				// if (mBackgroundDataSettingReceiver != null) {
				// unregisterReceiver(mBackgroundDataSettingReceiver);
				...
				// Remove the sync status change listener (and null out the
				// observer)
				if (mStatusChangeListener != null) {
					ContentResolver
							.removeStatusChangeListener(mStatusChangeListener);
	...
	private long checkMailboxes() {
		...
		// Start up threads that need it; use a query which finds eas mailboxes
		// where the
		// the sync interval is not "never". This is the set of mailboxes that
		// we control
		if (mAccountObserver == null) {
		...
		try {
			while (c.moveToNext()) {
				...
				EmAbstractSyncService service = null;
				synchronized (sSyncLock) {
				...
				if (service == null) {
					...
					// Manual syncs are initiated elsewhere, so they will
					...
					if (type == Mailbox.TYPE_CONTACTS
							|| type == Mailbox.TYPE_CALENDAR) {
						// We don't sync these automatically if master auto sync
						// is off
						if (!masterAutoSync) {
							continue;
						}
						...
						String authority;
						Account account = getAccountById(c
								.getInt(Mailbox.CONTENT_ACCOUNT_KEY_COLUMN));
						if (account != null) {
							if (type == Mailbox.TYPE_CONTACTS) {
								authority = ContactsConsts.AUTHORITY;
							} else {
								authority = CalendarConstants.AUTHORITY;
								if (!mCalendarObservers
										.containsKey(account.mId)) {
									// Make sure we have an observer for this
									// Calendar, as
									// we need to be able to detect sync state
									// changes, sigh
									registerCalendarObserver(account);
								}
							}
							android.accounts.Account a = new android.accounts.Account(
									account.mEmailAddress,
									Email.EXCHANGE_ACCOUNT_MANAGER_TYPE);
					...
					long interval = c
							.getInt(Mailbox.CONTENT_SYNC_INTERVAL_COLUMN);
					if (interval == Mailbox.CHECK_INTERVAL_PUSH) {
						Mailbox m = EmEmailContent.getContent(c, Mailbox.class);
						requestSync(m, SYNC_PUSH, null);
					} else if (type == Mailbox.TYPE_OUTBOX) {
						int cnt = EmEmailContent
								.count(this,
										Message.CONTENT_URI,
										EmEasOutboxService.MAILBOX_KEY_AND_NOT_SEND_FAILED,
										new String[] { Long.toString(mid) });
						if (cnt > 0) {
							Mailbox m = EmEmailContent.getContent(c,
									Mailbox.class);
							startServiceThread(new EmEasOutboxService(this, m),
									m);
						}
					} else if (interval > 0 && interval <= ONE_DAY_MINUTES) {
						long lastSync = c
								.getLong(Mailbox.CONTENT_SYNC_TIME_COLUMN);
						long sinceLastSync = now - lastSync;
						if (sinceLastSync < 0) {
							log("WHOA! lastSync in the future for mailbox: "
									+ mid);
						...
						if (toNextSync <= 0) {
							...
							nextWait = toNextSync;
							if (EmEas.USER_LOG) {
								log("Next sync for " + name + " in " + nextWait
										/ 1000 + "s");
							}
							mNextWaitReason = "Scheduled sync, " + name;
						} else if (EmEas.USER_LOG) {
							log("Next sync for " + name + " in " + toNextSync
									/ 1000 + "s");
						}
					}
				} else {
					Thread thread = service.mThread;
					// Look for threads that have died and remove them from the
					// map
						...
						if (requestTime > 0) {
							...
							if (timeToRequest <= 0) {
								...
								service.alarm();
									...
									&& timeToRequest < nextWait) {
								if (timeToRequest < 11 * MINUTES) {
									nextWait = timeToRequest < 250 ? 250
											: timeToRequest;
									mNextWaitReason = "Sync data change";
								} else {
									log("Illegal timeToRequest: "
											+ timeToRequest);
								}
							}
						}
	...
	static public void serviceRequest(long mailboxId, long ms, int reason) {
		...
		if (m == null || m.mType == Mailbox.TYPE_DRAFTS
				|| m.mType == Mailbox.TYPE_OUTBOX) {
			log("Ignoring serviceRequest for drafts/outbox/null mailbox");
			return;
		}
		try {
			EmAbstractSyncService service = syncManager.mServiceMap
					.get(mailboxId);
			if (service != null) {
	...
	static public void serviceRequestImmediate(long mailboxId) {
		...
		if (syncManager == null)
			return;
		EmAbstractSyncService service = syncManager.mServiceMap.get(mailboxId);
		if (service != null) {
			service.mRequestTime = System.currentTimeMillis();
			Mailbox m = Mailbox.restoreMailboxWithId(syncManager, mailboxId);
			if (m != null) {
				service.mAccount = Account.restoreAccountWithId(syncManager,
						m.mAccountKey);
				service.mMailbox = m;
	...
	static public void sendMessageRequest(EmRequest req) {
		EmSyncManager syncManager = INSTANCE;
		if (syncManager == null)
			return;
	...
	/**
	 * Determine whether a given Mailbox can be synced, i.e. is not already
	 * syncing and is not in an error state
	 * 
	 * @param mailboxId
	 * @return whether or not the Mailbox is available for syncing (i.e. is a
	 *         valid push target)
	 */
	...
	static public EmAbstractSyncService startManualSync(long mailboxId,
			int reason, EmRequest req) {
		EmSyncManager syncManager = INSTANCE;
		if (syncManager == null)
			return null;
		synchronized (sSyncLock) {
			if (syncManager.mServiceMap.get(mailboxId) == null) {
				syncManager.mSyncErrorMap.remove(mailboxId);
				Mailbox m = Mailbox
						.restoreMailboxWithId(syncManager, mailboxId);
				if (m != null) {
	...
	static private void stopManualSync(long mailboxId) {
		...
		if (syncManager == null)
			return;
		synchronized (sSyncLock) {
			EmAbstractSyncService svc = syncManager.mServiceMap.get(mailboxId);
			if (svc != null) {
	...
	static public void accountUpdated(long acctId) {
		...
		if (syncManager == null)
			return;
		synchronized (sSyncLock) {
			for (EmAbstractSyncService svc : syncManager.mServiceMap.values()) {
				if (svc.mAccount.mId == acctId) {
					svc.mAccount = Account.restoreAccountWithId(syncManager,
							acctId);
				}
			}
		}
	...
	/**
	 * Tell SyncManager to remove the mailbox from the map of mailboxes with
	 * sync errors
	 * 
	 * @param mailboxId
	...
	/**
	 * Sent by services indicating that their thread is finished; action depends
	 * on the exitStatus of the service.
	 * 
	 * @param svc
	 *            the service that is finished
	 */
	static public void done(EmAbstractSyncService svc) {
		EmSyncManager syncManager = INSTANCE;
		if (syncManager == null)
			return;
		synchronized (sSyncLock) {
			...
			case EmAbstractSyncService.EXIT_IO_ERROR:
				...
				if (syncError != null) {
					syncError.escalate();
					log(m.mDisplayName + " held for " + syncError.holdDelay
							+ "ms");
				} else {
					errorMap.put(mailboxId, syncManager.new SyncError(
							exitStatus, false));
					log(m.mDisplayName + " added to syncErrorMap, hold for 15s");
				}
				break;
			// These errors are not retried automatically
			case EmAbstractSyncService.EXIT_SECURITY_FAILURE:
//			case EmAbstractSyncService.EXIT_LOGIN_FAILURE:	//PWD_CHANGE_IMPL
			case EmAbstractSyncService.EXIT_EXCEPTION:
				errorMap.put(mailboxId, syncManager.new SyncError(exitStatus,
						true));
				break;
			
			//PWD_CHANGE_IMPL, -->
			case EmAbstractSyncService.EXIT_LOGIN_FAILURE:
				EmMailService.actionNotifyLoginFailed(getContext(), svc.mAccount.mId);
				break;
			////PWD_CHANGE_IMPL, <--
	...
	/**
	 * Given the status string from a Mailbox, return the type code for the last
	 * sync
	 * 
	 * @param status
	 *            the syncStatus column of a Mailbox
	 * @return
	 */
	...
	/**
	 * Given the status string from a Mailbox, return the change count for the
	 * last sync The change count is the number of adds + deletes + changes in
	 * the last sync
	 * 
	 * @param status
	 *            the syncStatus column of a Mailbox
	...
	public static String getServerAddress() {
		...
		return new SharedPreferences(Email.getAppContext()).getString("server",
				"");
	}
