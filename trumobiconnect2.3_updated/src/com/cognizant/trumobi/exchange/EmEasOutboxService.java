...
public class EmEasOutboxService extends EmEasSyncService {
    ...
    public EmEasOutboxService(Context _context, Mailbox _mailbox) {
        super(_context, _mailbox);
    }

    private void sendCallback(long msgId, String subject, int status) {
        try {
            EmSyncManager.callback().sendMessageStatus(msgId, subject, status, 0);
     ...
     * @throws IOException
     */
    int sendMessage(File cacheDir, long msgId) throws IOException, EmMessagingException {
        int result;
        sendCallback(msgId, null, EmEmailServiceStatus.IN_PROGRESS);
        ...
        try {
            ...
            EmRfc822Output.writeTo(mContext, msgId, fileStream, !smartSend, true);
            fileStream.close();
            ...
            if (code == HttpStatus.SC_OK) {
                ...
                sendCallback(-1, subject, EmEmailServiceStatus.SUCCESS);
            } else {
    ...
    public void run() {
        ...
        try {
             ...
             try {
                while (c.moveToNext()) {
                    ...
                    if (msgId != 0) {
                        ...
                        if (result == EmEmailServiceStatus.LOGIN_FAILED) {
                            mExitStatus = EXIT_LOGIN_FAILURE;
                            return;
                        } else if (result == EmEmailServiceStatus.REMOTE_EXCEPTION) {
            ...
            EmSyncManager.wakelockRelease(mAccount.mId);	//Battery Drain fix
        }
    }
