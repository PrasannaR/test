...
//	public static final String EVENT_SETTING_INFO_COMP =  "Setting_Information_Complete_Event";
	...
	public static void pimConfigurationUpdate_EventHandler(Context mContext, Intent intent){
		...
		ExternalAdapterLog.d("personalocalauthentication", "---------- pim settings updated notification received --------");
		ExternalAdapterLog.d("personalocalauthentication", "---------- pim settings updated notification received --------");
		ExternalAdapterLog.d("personalocalauthentication", "---------- pim settings updated notification received --------");
		Intent pimUpdate = new Intent("PimConfigurationUpdated");
		mContext.sendBroadcast(pimUpdate);
	}
...
/*		Intent localBroadCast = new Intent("com.cognizant.trubox.event.trumobireceiver");
		...
		PersonaLog.d("PersonaLocalAuthentication","----- login complete receiver called in externalpolicy util------");
	...
	public static void settingInformationCompleten_EventHandler(Context mContext, Intent intent){
		...
		throwIntentToSettingsScreenIfRegistered(mContext);
	...
	private static void throwIntentToSettingsScreenIfRegistered(Context context) {
		Intent settingsUpdated = new Intent("UpdatedSettingsInfoRetrived");
		context.sendBroadcast(settingsUpdated);
	}
	...
	private static void sendThisUIUpdate(Context mContext, boolean isSettingsInfoAvailable) {
		...
		//called when first time login initiatefetchinfo  and emaaslogin initiatefetchinfo
		if(!isPinSaved(mContext)) {
		Intent updateUI = new Intent("UpdateLoginScreenUI");
		updateUI.putExtra("safe_method_failed",safeMethodfailed);
		mContext.sendBroadcast(updateUI);
		 PersonaLog.d("Personalocalauthentication",  "------ sendThisUIUpdate--------- isPinSaved false");
		}
		else {
		Intent updateUiAfterEmaasLoginInitiateFetch = new Intent("LoginActionComplete");
		updateUiAfterEmaasLoginInitiateFetch.putExtra("safe_method_failed",safeMethodfailed);
		updateUiAfterEmaasLoginInitiateFetch.putExtra("settingsInfoFetchComplete",true);
		mContext.sendBroadcast(updateUiAfterEmaasLoginInitiateFetch);
		 PersonaLog.d("Personalocalauthentication",  "------ sendThisUIUpdate--------- isPinSaved true");
		}
		
	}

	private static boolean isPinSaved(Context mContext) {
		String mPS;
		boolean isPinSaved = false;
		Cursor c = mContext.getContentResolver().query(	PersonaDatabaseProvider.ROUTINE_CHECK_CONTENT_URI, null, null,
				null, null);
		try {
			if (c.getCount() > 0) {
				c.moveToFirst();
				mPS = c.getString(c.getColumnIndex(PersonaExternalDBHelper.COL5));
