...
public class PersonaMyLauncherSettings extends PreferenceActivity implements OnPreferenceChangeListener {
	...
	private static final String PREF_BACKUP_FILENAME = "ME_settings.xml";
	private static final String CONFIG_BACKUP_FILENAME = "ME_launcher.db";
	private static final String NAMESPACE = "org.hcli.launcher";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(R.xml.pr_launcher_settings);
		PersonaDialogSeekBarPreference columnsDesktop= (PersonaDialogSeekBarPreference) findPreference("desktopColumns");
		columnsDesktop.setMin(3);
		PersonaDialogSeekBarPreference rowsDesktop= (PersonaDialogSeekBarPreference) findPreference("desktopRows");
		rowsDesktop.setMin(3);
		PersonaDialogSeekBarPreference columnsPortrait= (PersonaDialogSeekBarPreference) findPreference("drawerColumnsPortrait");
		columnsPortrait.setMin(1);
		PersonaDialogSeekBarPreference rowsPortrait= (PersonaDialogSeekBarPreference) findPreference("drawerRowsPortrait");
		rowsPortrait.setMin(1);
		PersonaDialogSeekBarPreference columnsLandscape= (PersonaDialogSeekBarPreference) findPreference("drawerColumnsLandscape");
		columnsLandscape.setMin(1);
		PersonaDialogSeekBarPreference rowsLandscape= (PersonaDialogSeekBarPreference) findPreference("drawerRowsLandscape");
		rowsLandscape.setMin(1);
		PersonaDialogSeekBarPreference zoomSpeed= (PersonaDialogSeekBarPreference) findPreference("zoomSpeed");
		zoomSpeed.setMin(300);
		PersonaDialogSeekBarPreference uiScaleAB= (PersonaDialogSeekBarPreference) findPreference("uiScaleAB");
		uiScaleAB.setMin(1);
		...
		ListPreference audio_setting1 = (ListPreference) findPreference("audio_setting");
		audio_setting1.setOnPreferenceChangeListener(this);

		ListPreference workspace_settings = (ListPreference) findPreference("workspacescreen");
		workspace_settings.setOnPreferenceChangeListener(this);
		...
		if(PersonaAlmostNexusSettingsHelper.getSystemPersistent(this)){
			orientations.setEnabled(false);
		}else{
			orientations.setEnabled(true);
		}
		PersonaDialogSeekBarPreference notif_size= (PersonaDialogSeekBarPreference) findPreference("notif_size");
		...
		if(val==PersonaLauncher.DOCK_STYLE_5 || val==PersonaLauncher.DOCK_STYLE_NONE){
		...
		if (IsDebugVersion) {
			//	Debugging options
			addPreferencesFromResource(R.xml.pr_debugging_settings);
		}
		...
		adw_version.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			public boolean onPreferenceClick(Preference preference) {
				try {
				/*	AlertDialog builder = PersonaAlmostNexusSettingsHelper.ChangelogDialogBuilder.create(mContext);
					builder.show();*/
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		final String themePackage=sp.getString("themePackageName", PersonaLauncher.THEME_DEFAULT);
		ListPreference lp = (ListPreference)findPreference("themePackageName");
		lp.setOnPreferenceChangeListener(this);


		Intent intent=new Intent("com.cognizant.trumobi.THEMES");
		intent.addCategory("android.intent.category.DEFAULT");

		Intent adwThemeIntent=new Intent("org.adw.launcher.THEMES");
		intent.addCategory("android.intent.category.DEFAULT");
		...
		List<ResolveInfo> adwTheme=pm.queryIntentActivities(adwThemeIntent, 0);

		int temp = adwTheme.size();
		String[] entries = new String[temp + themes.size()+1];
		String[] values = new String[temp + themes.size()+1];


		entries[0]=PersonaLauncher.THEME_DEFAULT;
		values[0]=PersonaLauncher.THEME_DEFAULT;
		...
		for(j=i;j<(themes.size()+temp);j++){
			String appPackageName=(adwTheme.get(j-i)).activityInfo.packageName.toString();
			String themeName=(adwTheme.get(j-i)).loadLabel(pm).toString();
			entries[j+1]=themeName;
			values[j+1]=appPackageName;
		}

		lp.setEntries(entries);
		lp.setEntryValues(values);
		PersonaPreviewPreference themePreview=(PersonaPreviewPreference) findPreference("themePreview");
	...
	public void applyTheme(View v){
		...
		if(!packageName.equals(PersonaLauncher.THEME_DEFAULT)){
			...
			if(themeResources!=null){
				...
				if(dock_styleId!=0){
					...
					if(Integer.valueOf(dock_style)==PersonaLauncher.DOCK_STYLE_5 || Integer.valueOf(dock_style)==PersonaLauncher.DOCK_STYLE_NONE)editor.putBoolean("uiDots", false);
				}
				...
				if(wallpaperId!=0){
					...
					try {
						wallpaper=BitmapFactory.decodeResource(getResources(),R.drawable.pr_app_background);
	...
	protected void onPause(){
		if(shouldRestart){
			if(Build.VERSION.SDK_INT<=7){
				Intent intent = new Intent(getApplicationContext(), PersonaLauncher.class);
				...
				acm.restartPackage("com.cognizant.trumobi");
			}else{
	...
	public boolean onPreferenceChange(Preference preference, Object newValue) {
		...
		}else if(preference.getKey().equals("swipedownActions"))
		{
			...
			if (newValue.equals(String.valueOf(PersonaLauncher.BIND_APP_LAUNCHER)))
			{
		...
		else if(preference.getKey().equals("workspacescreen")) {
			try
			{
				getPreferenceManager().getSharedPreferences().edit().putString("workspacescreen",newValue.toString()).commit();
				
				//trumobiEdits
				setWallpaper(BitmapFactory.decodeResource(getResources(),R.drawable.pr_app_background));
				
				
				/*if(PersonaLauncher.wsettings==0)
				{
					setWallpaper(BitmapFactory.decodeResource(getResources(),R.drawable.app_background));
				}
				else if(PersonaLauncher.wsettings==1)
				{
					setWallpaper(BitmapFactory.decodeResource(getResources(),R.drawable.business));
				}
				else if(PersonaLauncher.wsettings==2)
				{
					setWallpaper(BitmapFactory.decodeResource(getResources(),R.drawable.kids));
				}
				else if(PersonaLauncher.wsettings==3)
				{
					setWallpaper(BitmapFactory.decodeResource(getResources(),R.drawable.vernaculor));
				}
*/
		...
		else if(preference.getKey().equals("audio_setting")) {

			getPreferenceManager().getSharedPreferences().edit().putString("audio_setting",newValue.toString()).commit();
		}		
	...
	// wjax: Get the App chosen as to be launched upon gesture completion. And store it in SharedPreferences via PersonaAlmostNexusSettingsHelper!!!
	@Override
	protected void onActivityResult (int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			switch (requestCode) {
			case REQUEST_SWIPE_DOWN_APP_CHOOSER:
				PersonaAlmostNexusSettingsHelper.setSwipeDownAppToLaunch(this, infoFromApplicationIntent(this, data));
				break;
			case REQUEST_HOME_BINDING_APP_CHOOSER:
				PersonaAlmostNexusSettingsHelper.setHomeBindingAppToLaunch(this, infoFromApplicationIntent(this, data));
				break;
			case REQUEST_SWIPE_UP_APP_CHOOSER:
				PersonaAlmostNexusSettingsHelper.setSwipeUpAppToLaunch(this, infoFromApplicationIntent(this, data));
				break;
			}
		}
	...
	public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
		if (preference.getKey().equals("highlights_color")) {
			PersonaColorPickerDialog cp = new PersonaColorPickerDialog(this,mHighlightsColorListener,readHighlightsColor());
			cp.show();
		}else if(preference.getKey().equals("highlights_color_focus")) {
			PersonaColorPickerDialog cp = new PersonaColorPickerDialog(this,mHighlightsColorFocusListener,readHighlightsColorFocus());
			cp.show();
		}else if(preference.getKey().equals("drawer_color")) {
			PersonaColorPickerDialog cp = new PersonaColorPickerDialog(this,mDrawerColorListener,readDrawerColor());
			cp.show();
		}else if(preference.getKey().equals("uiABTintColor")) {
			PersonaColorPickerDialog cp = new PersonaColorPickerDialog(this,mABTintColorListener,PersonaAlmostNexusSettingsHelper.getUIABTintColor(this));
			cp.show();
		}
		return false;
	}
	private int readHighlightsColor() {
		return PersonaAlmostNexusSettingsHelper.getHighlightsColor(this);
	}

	PersonaColorPickerDialog.OnColorChangedListener mHighlightsColorListener =
			new PersonaColorPickerDialog.OnColorChangedListener() {
	...
	private int readHighlightsColorFocus() {
		return PersonaAlmostNexusSettingsHelper.getHighlightsColorFocus(this);
	}

	PersonaColorPickerDialog.OnColorChangedListener mHighlightsColorFocusListener =
			new PersonaColorPickerDialog.OnColorChangedListener() {
		public void colorChanged(int color) {
	...
	private int readDrawerColor() {
		return PersonaAlmostNexusSettingsHelper.getDrawerColor(this);
	}

	PersonaColorPickerDialog.OnColorChangedListener mDrawerColorListener =
			new PersonaColorPickerDialog.OnColorChangedListener() {
		public void colorChanged(int color) {
			getPreferenceManager().getSharedPreferences().edit().putInt("drawer_color", color).commit();
		}
	};
	PersonaColorPickerDialog.OnColorChangedListener mABTintColorListener =
			new PersonaColorPickerDialog.OnColorChangedListener() {
		public void colorChanged(int color) {
	...
	public void getThemes(View v){
		...
		try {
			...
			Toast.makeText(this, R.string.activity_not_found, Toast.LENGTH_SHORT).show();
			PersonaLog.e("ADW", "PersonaLauncher does not have the permission to launch " + marketIntent +
