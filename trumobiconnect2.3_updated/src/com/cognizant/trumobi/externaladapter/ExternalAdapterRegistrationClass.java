    ...
    * 
	...
	private void getPIMSettingsInfo(){
		...
		// Commented out this check to initializeROVA everytime ,till we get proper working realtime values.
			...
			ExternalAdapterLog.d(TAG,"----------- rovaPIMSettingsInfo.nPasswordExpiry --------" + rovaPIMSettingsInfo.PasswordExpires.getValue());
			...
			extPIMSettInfo.sEmailSignature = rovaPIMSettingsInfo.sEmailSignature; // ROVA_POLICY_CHECK - 26Dec2013 , added emailsignature for PIMsettings
				...
				exceptionSetDefaultPIMSettingsInfo(extPIMSettInfo);
			}
...
//		extDeviceInfo.bJailBroken=rovaDeviceInfo.bJailBroken;
	 ...
	 *  
	 */
	private void exceptionSetDefaultPIMSettingsInfo(ExternalPIMSettingsInfo extPIMSettInfo){
		
		//ExternalPIMSettingsInfo extPIMSettInfo = new ExternalPIMSettingsInfo();
		
	
		extPIMSettInfo.nMaxAttachmentSize = 0;
		extPIMSettInfo.nMaxPasswordTries = 2;
		...
		extPIMSettInfo.PasswordExpires = ExternalPasswordExpiration.fromInt(ExternalPasswordExpiration.Week.getValue());
		extPIMSettInfo.PasswordFailLockout = ExternalPasswordLockTime.fromInt(ExternalPasswordLockTime.Minute.getValue());
		extPIMSettInfo.Passwordtype = ExternalPasswordType.fromInt(ExternalPasswordType.Numeric.getValue());
		ExternalAdapterLog.d(TAG, "----------- extPIMSettInfo.Passwordtype ----------" +extPIMSettInfo.Passwordtype );
		...
		extPIMSettInfo.bPasswordPIN = false;
		extPIMSettInfo.bCopyPaste = true;
		extPIMSettInfo.bEmailDownloadAttachments = true;
		extPIMSettInfo.bPasswordSimpleAllowed = false;
		extPIMSettInfo.bShortcuts =true;
		extPIMSettInfo.bShowTXTMessage = true;
		extPIMSettInfo.bWidgets = true;
		extPIMSettInfo.nMaxAttachmentSize = 5;
	}
	...
	//added by 290767 on 30/12/2013
    /**
     *   isBlockURL(String URLString)
     *   
     *   This method is invoked to check whether a URL is blocked or not
     *   
     */
     
     public boolean isBlockURL(String URLString){
            boolean bBlock = false;

            if(mRova != null)
                   bBlock = mRova.blockURL(URLString);
