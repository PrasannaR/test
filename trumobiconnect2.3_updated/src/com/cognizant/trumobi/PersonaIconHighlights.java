...
public class PersonaIconHighlights {
	...
	private static Drawable newSelector(Context context){
		...
		int selectedColor=PersonaAlmostNexusSettingsHelper.getHighlightsColorFocus(context);
		int pressedColor=PersonaAlmostNexusSettingsHelper.getHighlightsColor(context);
		...
		if(themeResources!=null){
			...
			if(type==TYPE_DRAWER){
				...
				if(resource_id!=0){
					...
					if(type==TYPE_DOCKBAR){
						drawable=themeResources.getDrawable(R.drawable.pr_dockbar_selector);
					}else{
						drawable=themeResources.getDrawable(R.drawable.pr_shortcut_selector);
					}
				}
	...
	public static Drawable getDrawable(Context context, int type){
		if(PersonaAlmostNexusSettingsHelper.getUINewSelectors(context)){
