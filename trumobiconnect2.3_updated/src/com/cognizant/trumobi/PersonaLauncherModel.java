...
public class PersonaLauncherModel {
	...
	private synchronized void stopAndWaitForApplicationsLoader() {
		if (mApplicationsLoader != null && mApplicationsLoader.isRunning()) {
			...
			try {
				...
				PersonaLog.e(LOG_TAG, "mApplicationsLoaderThread didn't exit in time");
			}
		}
	}
	...
	private synchronized void startApplicationsLoader(
		...
		if (DEBUG_LOADERS)
			PersonaLog.d(LOG_TAG, "  --> starting applications loader unlocked");
	...
	private static void updateApplicationInfoTitleAndIcon(
		...
		if (info.activityInfo.name
			...
			application.icon = context.getResources().getDrawable(
				...
				.contains("com.cognizant.trumobi.contacts.ContactsParentActivity")) { 
		...
		else if(info.activityInfo.name.contains("com.cognizant.trumobi.securebrowser.SB_MainActivity")){
	        	
        	 application.title = "Browser";
          	  application.icon=context.getResources().getDrawable(R.drawable.pr_browser_icon);
        }
		else if (info.activityInfo.name
			...
			application.icon = context.getResources().getDrawable(R.drawable.pr_settings_app_icon);
		}
		else{
		
			application.title = "Safe Space";
	...
	private class ApplicationsLoader implements Runnable {
		...
		public void run() {
			if (DEBUG_LOADERS)
				PersonaLog.d(LOG_TAG, "  ----> running applications loader (" + mId + ")");
			...
			if (apps != null && !mStopped) {
                ...
                boolean showConnect = mExtAdapReg.getExternalPIMSettingsInfo().bShowEmail;
                showConnect = true;
				for (int i = 0; i < count && !mStopped; i++) {
					...
					if (showConnect
						...
						info = apps.get(i);
						PersonaLog.e(LOG_TAG, "acitivity name : "
								+ apps.get(i).activityInfo.name);
			...
			if (mStopped) {
				if (DEBUG_LOADERS)
					PersonaLog.d(LOG_TAG, "  ----> applications loader stopped (" + mId
							+ ")");
			}
			mRunning = false;
		}
	...
	void loadUserItems(boolean isLaunching, PersonaLauncher personaLauncher,
		...
		if (isLaunching && isDesktopLoaded()) {
			...
			if (DEBUG_LOADERS)
				PersonaLog.d(LOG_TAG, "  --> loading from cache: " + mDesktopItems.size()
		...
		if (mDesktopItemsLoader != null && mDesktopItemsLoader.isRunning()) {
			...
			try {
				...
				PersonaLog.e(LOG_TAG, "mDesktopLoaderThread didn't exit in time");
			}
	...
	private class DesktopItemsLoader implements Runnable {
		...
		void stop() {
			PersonaLog.d(LOG_TAG, "  ----> workspace loader " + mId + " stopped from "
					+ Thread.currentThread().toString());
			mStopped = true;
		}
		...
		private void load_workspace() {
			if (DEBUG_LOADERS)
				PersonaLog.d(LOG_TAG, "  ----> running workspace loader (" + mId + ")");
					...
					try {
						...
						case PersonaLauncherSettings.Favorites.ITEM_TYPE_WIDGET_SEARCH:
							...
							if (container != PersonaLauncherSettings.Favorites.CONTAINER_DESKTOP) {
								PersonaLog.e(PersonaLauncher.LOG_TAG,
			...
			if (DEBUG_LOADERS) {
				...
				PersonaLog.d(LOG_TAG, "  ----> worskpace items=" + desktopItems.size());
			...
			synchronized (PersonaLauncherModel.this) {
				if (!mStopped) {
					if (DEBUG_LOADERS) {
						PersonaLog.d(LOG_TAG, "  --> done loading workspace; not stopped");
					}
					...
					if (!mStopped) {
						PersonaLog.d(LOG_TAG, "  ----> items cloned, ready to refresh UI");
						personaLauncher.runOnUiThread(new Runnable() {
							public void run() {
								if (DEBUG_LOADERS)
									PersonaLog.d(LOG_TAG, "  ----> onDesktopItemsLoaded()");
					...
					if (DEBUG_LOADERS)
						PersonaLog.d(LOG_TAG, "  ----> worskpace loader was stopped");
				}
			}
		}
		...
		private void updateShortcutLabels(ContentResolver resolver,
			...
			final Cursor c = resolver.query(
					PersonaLauncherSettings.Favorites.CONTENT_URI,
	...
	private void unbindAppDrawables(
		...
		if (applications != null) {
			...
			for (int i = 0; i < count; i++) {
				//PIMSettings
				if(applications.get(i) != null && applications.get(i).icon != null) 
