package com.cognizant.trumobi.persona.multitouch;

/**
 * PersonaMultiTouchController.java
 * 
 ...
 *
 *       private PersonaMultiTouchController<PinchWidgetType> multiTouchController = new PersonaMultiTouchController<PinchWidgetType>(this);
 *  
 ...
 * - Yuan Chin's fork of ADW PersonaLauncher to support multitouch
 ...
 */
public class PersonaMultiTouchController<T> {
	...
	/** Generate tons of PersonaLog entries for debugging */
	...
	public PersonaMultiTouchController(MultiTouchObjectCanvas<T> objectCanvas) {
		this(objectCanvas, true);
	}

	/** Full constructor */
	public PersonaMultiTouchController(MultiTouchObjectCanvas<T> objectCanvas, boolean handleSingleTouchEvents) {
	...
	static {
		...
		try {
			...
			PersonaLog.e("PersonaMultiTouchController", "static initializer failed", e);
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		try {
			...
			if (DEBUG)
				PersonaLog.i("MultiTouch", "Got here 1 - " + multiTouchSupported + " " + mMode + " " + handleSingleTouchEvents + " " + pointerCount);
			if (mMode == MODE_NOTHING && !handleSingleTouchEvents && pointerCount == 1)
				// Not handling initial single touch events, just pass them on
				return false;
			if (DEBUG)
				PersonaLog.i("MultiTouch", "Got here 2");
			...
			for (int histIdx = 0; histIdx <= histLen; histIdx++) {
				...
				if (!multiTouchSupported || pointerCount == 1) {
					...
					if (DEBUG)
						PersonaLog.i("MultiTouch", "Got here 3");
					...
					if (DEBUG)
						PersonaLog.i("MultiTouch", "Got here 4");
			...
			PersonaLog.e("PersonaMultiTouchController", "onTouchEvent() failed", e);
			return false;
		}
	}
	...
	private void decodeTouchEvent(int pointerCount, float[] x, float[] y, float[] pressure, int[] pointerIdxs, int action, boolean down,
			long eventTime) {
		if (DEBUG)
			PersonaLog.i("MultiTouch", "Got here 5 - " + pointerCount + " " + action + " " + down);
	...
	private void multiTouchController() {
		if (DEBUG)
			PersonaLog.i("MultiTouch", "Got here 6 - " + mMode + " " + mCurrPt.getNumTouchPoints() + " " + mCurrPt.isDown() + mCurrPt.isMultiTouch());
		...
		if (DEBUG)
			PersonaLog.i("MultiTouch", "Got here 7 - " + mMode + " " + mCurrPt.getNumTouchPoints() + " " + mCurrPt.isDown() + mCurrPt.isMultiTouch());
	}
	...
	public static class PointInfo {
		...
		private void set(int numPoints, float[] x, float[] y, float[] pressure, int[] pointerIdxs, int action, boolean isDown, long eventTime) {
			if (DEBUG)
				PersonaLog.i("MultiTouch", "Got here 8 - " + +numPoints + " " + x[0] + " " + y[0] + " " + (numPoints > 1 ? x[1] : x[0]) + " "
