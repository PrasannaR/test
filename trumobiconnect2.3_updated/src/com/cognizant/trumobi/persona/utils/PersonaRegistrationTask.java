...
public class PersonaRegistrationTask extends AsyncTask<String, Integer, Integer> {
	...
	protected Integer doInBackground(String... params) {
		...
		default: {
			...
			mSerialId=mCommonfunctions.getSerialNumber();
			mImei=mCommonfunctions.getIMEI();
			mMacAddress=mCommonfunctions.getMacAddress();
			mMake=mCommonfunctions.getDeviceMake();
			mModel=mCommonfunctions.getDeviceModel();
	...
	protected void onPostExecute(Integer result) {
		...
		if (result == PersonaConstants.RESPONSEOK) {
			...
			if(remoteCallCode!=1003){
				
				JSONObject appstore_info =jsonPolicy.getJSONObject("appstore_information");
				int app_id=appstore_info.getInt("app_detail_id");
				PersonaLog.e("=====================APP id=============", ""+app_id);
				new SharedPreferences(mContext).edit().putInt("app_id", app_id).commit();
			}
	...
	private JSONObject appendRegistrationJSON() {
		...
		try {
			...
			mJsonObj.put("os_version",mCommonfunctions.getOSVersion());
			mJsonObj.put("model_number", mModel);
			mJsonObj.put("device_type", mCommonfunctions.getDeviceType(mContext));
			...
			mJsonObj.put("ip_address",mSerialId );
			mJsonObj.put("device_token", "1221323243454354");
			mJsonObj.put("persona_key",mCommonfunctions.getPersonaKey());
			mJsonObj.put("app_version",mCommonfunctions.getAppVersion());
			mJsonObj.put("bundle_identifier",mContext.getApplicationInfo().packageName );
			
			boolean isCrsRejected= new SharedPreferences(mContext).getBoolean("crs_rejected", false);			
			mJsonObj.put("crs_rejected",isCrsRejected);
