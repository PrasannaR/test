...
import com.cognizant.trumobi.persona.widgets.PersonaWidgetListAdapter.ViewHolder;
 ...
 *
 */
public abstract class PersonaWidgetSpace extends ViewGroup {

	private static final String TAG = "PersonaWidgetSpace";
	...
	public PersonaWidgetSpace(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
	}

	public PersonaWidgetSpace(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public PersonaWidgetSpace(Context context) {
		super(context);
	}
	...
	BroadcastReceiver mAnimationProvider = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			PersonaLog.i("AnimationProvider", "" + intent);

			int widgetId = intent.getIntExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, -1);
			if (widgetId < 0)
				widgetId = intent.getIntExtra(PersonaLauncherIntent.Extra.EXTRA_APPWIDGET_ID, -1);
			if (widgetId < 0) {
				PersonaLog.e(TAG, "Scroll Provider cannot get a legal widget id");
				return;
			}
			...
			try {
				...
				if (PersonaLauncherIntent.Action.ACTION_START_FRAME_ANIMATION.equals(action)) {
					actFrameAnimation(widgetView, intent.getIntExtra(
							PersonaLauncherIntent.Extra.EXTRA_IMAGEVIEW_ID, 0), intent, true);
				} else if (PersonaLauncherIntent.Action.ACTION_STOP_FRAME_ANIMATION.equals(action)) {
					actFrameAnimation(widgetView, intent.getIntExtra(
							PersonaLauncherIntent.Extra.EXTRA_IMAGEVIEW_ID, 0), intent, false);
				} else if (PersonaLauncherIntent.Action.ACTION_START_TWEEN_ANIMATION.equals(action)) {
					startTweenAnimation(widgetView, intent.getIntExtra(
							PersonaLauncherIntent.Extra.EXTRA_VIEW_ID, 0), intent);
				}
				...
				getContext().sendBroadcast(
						intent.setAction(ae.mAction).putExtra(
								PersonaLauncherIntent.Extra.EXTRA_ERROR_MESSAGE, ae.getMessage()));
			...
			try {
				...
				if (start) {
					...
					.sendBroadcast(
							...
							.setAction(
									PersonaLauncherIntent.Notification.NOTIFICATION_FRAME_ANIMATION_STARTED));
				} else {
					// Stop the animation
					ad.stop();
					getContext()
					.sendBroadcast(
							...
							.setComponent(widgetView.getAppWidgetInfo().provider)
							.setAction(
									PersonaLauncherIntent.Notification.NOTIFICATION_FRAME_ANIMATION_STOPPED));
				}
			} catch (Exception e) {
		...
		void startTweenAnimation(AppWidgetHostView widgetView, int viewId, Intent intent)
			...
			int animId = intent.getIntExtra(PersonaLauncherIntent.Extra.EXTRA_ANIMATION_ID, -1);
			...
			try {
				...
				long animStartTime = intent.getLongExtra(
						PersonaLauncherIntent.Extra.EXTRA_ANIMATION_STARTTIME, -1);
				if (animStartTime > 0) {
		...
		class TweenAnimListener implements AnimationListener {
			...
			public void onAnimationEnd(Animation animation) {
				...
				.sendBroadcast(
						mIntent
						.setAction(PersonaLauncherIntent.Notification.NOTIFICATION_TWEEN_ANIMATION_ENDED));
				mIntent = null;
			...
			public void onAnimationRepeat(Animation animation) {
				...
				.sendBroadcast(
						mIntent
						.setAction(PersonaLauncherIntent.Notification.NOTIFICATION_TWEEN_ANIMATION_REPEATED));
			}
			...
			public void onAnimationStart(Animation animation) {
				...
				.sendBroadcast(
						mIntent
						.setAction(PersonaLauncherIntent.Notification.NOTIFICATION_TWEEN_ANIMATION_STARTED));
			}
	...
	class ScrollViewInfos {
		...
		ContentObserver obs;
		Handler obsHandler;
		PersonaScrollableBaseAdapter lvAdapter;
		CharSequence key;
	}
	...
	public synchronized boolean unbindWidgetScrollable() {
		for (ScrollViewInfos item : mScrollViewCursorInfos.values()) {
			if (item.lv != null) {
				if (CLEAR_DATA_CACHE) {
					...
					if (adapter != null) {
						if (adapter instanceof PersonaWidgetListAdapter)
							((PersonaWidgetListAdapter)adapter).clearDataCache();
						else if (adapter instanceof PersonaWidgetRemoteViewsListAdapter)
							((PersonaWidgetRemoteViewsListAdapter)adapter).dropCache();
					}
				}
		...
		PersonaListViewImageManager.getInstance().unbindDrawables();

		if (CLEAR_DATA_CACHE) {
			PersonaListViewImageManager.getInstance().clearCache();
		}
	...
	public synchronized boolean unbindWidgetScrollableId(int widgetId) {
		PersonaLog.d(TAG, "trying to completely unallocate widget ID="+widgetId);
		CharSequence keyToDelete=null;
		for (ScrollViewInfos item : mScrollViewCursorInfos.values()) {
			PersonaLog.d(TAG, "Comparing widget ID="+item.widgetId);
			if (item.widgetId == widgetId){
				PersonaLog.d(TAG, "trying to completely unallocate widget stuff...");
				AppWidgetHostView widgetView = findWidget(widgetId);
				PersonaLog.d(TAG, "PersonaWidget view to KILL:"+widgetView);
				...
				if (widgetView != null) {
					...
					if (info != null && info.provider != null) {
						...
						try {
							...
							PersonaLog.e(TAG, "couldn't find widget id:"+widgetId);
						}
					} else
						remoteContext = null;
				}
				...
				if(item.lv!=null){
					PersonaLog.d(TAG, "Trying to KILL the ListView...");
				...
				item=null;
				PersonaListViewImageManager.getInstance().clearCacheForWidget(getContext(), widgetId);
			}
		}
		...
		PersonaLog.d(TAG, "AFTER REMOVING, Our Scrollable widgets array contains:"+mScrollViewCursorInfos.size());
		return false;
	}
	...
	class ScrollViewProvider extends BroadcastReceiver implements OnScrollListener {
		...
		public void onReceive(Context context, Intent intent) {
			String action = intent.getAction();
			PersonaLog.i("PersonaWidgetSpace - onReceive: ", "" + intent);
			...
			if (widgetView == null) {
				getContext().sendBroadcast(
						intent.setAction(PersonaLauncherIntent.Error.ERROR_SCROLL_CURSOR).putExtra(
								PersonaLauncherIntent.Extra.EXTRA_ERROR_MESSAGE,
			...
			if (TextUtils.equals(action, PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_START)) {
				error = makeScrollable(context, intent, widgetView);

			} else if (TextUtils.equals(action,
					PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_SELECT_ITEM)) {
				error = setSelection(context, intent, widgetView);
			} else if (TextUtils.equals(action, PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_CLOSE)) {
				error = releaseScrollable(context, intent, widgetView);
			} else if (TextUtils.equals(action, PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_CLEAR_IMAGE_CACHE)) {
				error = PersonaListViewImageManager.getInstance().clearCacheForWidget(context, widgetId);
			}
			if (error == null) {
				// send finish signal
				intent.setComponent(appWidgetProvider);
				getContext().sendBroadcast(intent.setAction(PersonaLauncherIntent.Action.ACTION_FINISH));
			} else {
				// send error message
				intent.setComponent(appWidgetProvider);
				getContext().sendBroadcast(
						intent.setAction(PersonaLauncherIntent.Error.ERROR_SCROLL_CURSOR).putExtra(
								PersonaLauncherIntent.Extra.EXTRA_ERROR_MESSAGE, error));
			}
		}
		...
		private synchronized String makeScrollable(Context context, Intent intent,
			...
			final int dummyViewId = intent.getIntExtra(PersonaLauncherIntent.Extra.EXTRA_VIEW_ID, -1);
			if (dummyViewId <= 0)
			...
			try {
				...
				if (dummyView instanceof AbsListView)
					...
					dummyView = null;
					if (intent.hasExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_LISTVIEW_REMOTEVIEWS)) {
						PersonaSimpleRemoteViews rvs = (PersonaSimpleRemoteViews)intent.getParcelableExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_LISTVIEW_REMOTEVIEWS);
						...
						final int listViewResId = intent.getIntExtra(
								PersonaLauncherIntent.Extra.Scroll.EXTRA_LISTVIEW_LAYOUT_ID, -1);
						if (listViewResId <= 0) {
				...
				String cursorDataUriString = intent
						.getStringExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_DATA_URI);
				...
				if (newListView) {
					...
					final PersonaScrollableBaseAdapter lvAdapter;
					if (intent.hasExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_ITEM_LAYOUT_REMOTEVIEWS))
						lvAdapter = new PersonaWidgetRemoteViewsListAdapter(remoteContext, intent,
								appWidgetProvider, appWidgetId, dummyViewId);
					else
						lvAdapter = new PersonaWidgetListAdapter(remoteContext,
								intent, appWidgetProvider, appWidgetId, dummyViewId);
					// create listener for content Provider data modification
					PersonaWidgetDataChangeListener personaWidgetDataChangeListener = new PersonaWidgetDataChangeListener() {
						@Override
					...
					listViewInfos.obs = new PersonaWidgetContentObserver(listViewInfos.obsHandler,
							personaWidgetDataChangeListener);
					...
					if (LOGD)
						PersonaLog.d(TAG, "makeScrollable : recreate listview adapter");
				} else {
					if (LOGD)
						PersonaLog.d(TAG, "makeScrollable : restore listview adapter");
					if (listViewInfos.lvAdapter instanceof PersonaWidgetRemoteViewsListAdapter)
						((PersonaWidgetRemoteViewsListAdapter)listViewInfos.lvAdapter).updateFromIntent(intent);
				}

				lv.setAdapter(listViewInfos.lvAdapter);

				// finish listview configuration
				if ((listViewInfos.lvAdapter instanceof PersonaWidgetListAdapter) &&
						!((PersonaWidgetListAdapter)listViewInfos.lvAdapter).mItemChildrenClickable)
				...
				int position = intent.getIntExtra(
						PersonaLauncherIntent.Extra.Scroll.EXTRA_LISTVIEW_POSITION, -1);
				if (position >= 0)
					lv.setSelection(position);

				if (!newListView && CLEAR_DATA_CACHE &&
						(listViewInfos.lvAdapter instanceof PersonaWidgetListAdapter)) {
				...
				PersonaLog.d(TAG, "AFTER ADDING, Our Scrollable widgets array contains:"+mScrollViewCursorInfos.size());
				return null;
			} catch (Exception e) {
				return e.getMessage();
			}
		...
		private String setSelection(Context context, Intent intent, AppWidgetHostView widgetView) {
			...
			try {
				...
				String cursorDataUriString = intent
						.getStringExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_DATA_URI);
				int position = intent.getIntExtra(
						PersonaLauncherIntent.Extra.Scroll.EXTRA_LISTVIEW_POSITION, 0);
			...
			public void onItemClick(AdapterView<?> arg0, View view, int pos, long arg3) {
				try {
					...
					if (tag != null && tag instanceof String) {
						...
						Intent intent = new Intent(PersonaLauncherIntent.Action.ACTION_ITEM_CLICK);
						intent.setComponent(mAppWidgetProvider);
						intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mAppWidgetId)
						.putExtra(PersonaLauncherIntent.Extra.EXTRA_APPWIDGET_ID, mAppWidgetId);
						intent.putExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_LISTVIEW_ID, mListViewId);
						intent.putExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_ITEM_POS, pos);
						...
						intent.putExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_SOURCE_BOUNDS, srcRect);
	...
	public void registerProvider() {
		...
		filter.addAction(PersonaLauncherIntent.Action.ACTION_START_FRAME_ANIMATION);
		filter.addAction(PersonaLauncherIntent.Action.ACTION_STOP_FRAME_ANIMATION);
		filter.addAction(PersonaLauncherIntent.Action.ACTION_START_TWEEN_ANIMATION);
		context.registerReceiver(mAnimationProvider, filter);

		IntentFilter scrollFilter = new IntentFilter();
		scrollFilter.addAction(PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_START);
		scrollFilter.addAction(PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_CLOSE);
		scrollFilter.addAction(PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_CLEAR_IMAGE_CACHE);
		scrollFilter.addAction(PersonaLauncherIntent.Action.ACTION_SCROLL_WIDGET_SELECT_ITEM);
	...
	class FrameAnimationException extends AnimationException {
		public FrameAnimationException(String msg) {
			super(PersonaLauncherIntent.Error.ERROR_FRAME_ANIMATION, msg);
		}
	...
	class TweenAnimationException extends AnimationException {
		...
		public TweenAnimationException(String msg) {
			super(PersonaLauncherIntent.Error.ERROR_TWEEN_ANIMATION, msg);
		}
