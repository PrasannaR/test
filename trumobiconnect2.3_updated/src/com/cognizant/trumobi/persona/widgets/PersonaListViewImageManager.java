...
public class PersonaListViewImageManager {

	private static final String TAG = "PersonaListViewImageManager";

	private static final boolean LOGD = false;

	private static PersonaListViewImageManager instance;

	public static PersonaListViewImageManager getInstance() {
		if (instance == null)
			instance = new PersonaListViewImageManager();
		return instance;
	}
	...
	public Drawable getImageFromUri(Context mContext, int widgetId, String imgUri) {
		...
		if (LOGD)
			if (d != null)
				PersonaLog.d(TAG, "image URI restored (width = " + d.getMinimumWidth() + " / weight = " + d.getMinimumHeight()
						+ ")");
		...
		if (d == null) {
			...
			if (ContentResolver.SCHEME_ANDROID_RESOURCE.equals(scheme)) {
				...
				// } catch (Exception e) {
				// PersonaLog.w("ImageView", "Unable to open content: " + mUri, e);
				// }
				PersonaLog.w("ImageView", "Unable to open content: " + mUri);
			...
			if (LOGD)
				PersonaLog.d(TAG, "image URI decoded (width = " + d.getMinimumWidth() + " / weight = " + d.getMinimumHeight()
						+ ")");
	...
	public Drawable getImageFromId(Context ctx, int widgetId, int imgId) {
		...
		if (mCacheForImageById.containsKey(imgId) && mCacheForImageById.get(imgId) != null) {
			...
			if (ref != null) {
				...
				if (LOGD)
					PersonaLog.d(TAG, "image ID missing !!!!!!!!!!");
			}
		}

		if (LOGD)
			if (drawable != null)
				PersonaLog.d(TAG, "image ID restored");

		if (drawable == null) {
			if (LOGD)
				PersonaLog.d(TAG, "image ID decoded");
		...
		if (listByUri != null) {
			for (String imgUri : listByUri) {
				if (mCacheForImageByUri.containsKey(imgUri) && mCacheForImageByUri.get(imgUri) != null) {
					...
					PersonaLog.d(TAG, "image URI removed from cache : " + imgUri);
				}
			}
		}
		...
		if (listById != null) {
			for (Integer imgId : listById) {
				if (mCacheForImageById.containsKey(imgId) && mCacheForImageById.get(imgId) != null) {
					...
					PersonaLog.d(TAG, "image ID removed from cache : " + imgId);
				}
			}
		}
