 ...
 *
 */
public class PersonaWidgetListAdapter extends PersonaScrollableBaseAdapter {
	...
	static PersonaListViewImageManager mImageManager = PersonaListViewImageManager.getInstance();
	...
	final Runnable mGenerateDataCacheRunnable = new Runnable() {
		public void run() {
			if (LOGD)
				PersonaLog.d(LOG_TAG, "mGenerateDataCacheRunnable start");
			generateDataCache();
			if (LOGD)
				PersonaLog.d(LOG_TAG, "mGenerateDataCacheRunnable end");
		}
	};
	...
	public void clearDataCache() {
		...
		if (LOGD)
			PersonaLog.d(LOG_TAG, "clearDataCache");
	}
	 ...
	 * @param listViewId
	 */
    public PersonaWidgetListAdapter(Context context, Intent intent, ComponentName provider,
        ...
        mAllowRequery = intent.getBooleanExtra(
                PersonaLauncherIntent.Extra.Scroll.EXTRA_DATA_PROVIDER_ALLOW_REQUERY, false);

        // Get the layout if for items
        mItemLayoutId = intent.getIntExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_ITEM_LAYOUT_ID, -1);
        if (mItemLayoutId <= 0)
            throw (new IllegalArgumentException("The passed layout id is illegal"));

        mItemChildrenClickable = intent.getBooleanExtra(
                PersonaLauncherIntent.Extra.Scroll.EXTRA_ITEM_CHILDREN_CLICKABLE, false);

        mItemActionUriIndex = intent.getIntExtra(
                PersonaLauncherIntent.Extra.Scroll.EXTRA_ITEM_ACTION_VIEW_URI_INDEX, -1);
	...
	private void generateItemMapping(Intent intent) {
        ...
        int[] viewTypes = intent
                .getIntArrayExtra(PersonaLauncherIntent.Extra.Scroll.Mapping.EXTRA_VIEW_TYPES);
        int[] viewIds = intent.getIntArrayExtra(PersonaLauncherIntent.Extra.Scroll.Mapping.EXTRA_VIEW_IDS);
        int[] cursorIndices = intent
                .getIntArrayExtra(PersonaLauncherIntent.Extra.Scroll.Mapping.EXTRA_CURSOR_INDICES);
        int[] defaultResources = intent
                .getIntArrayExtra(PersonaLauncherIntent.Extra.Scroll.Mapping.EXTRA_DEFAULT_RESOURCES);
        boolean[] viewClickable = intent
                .getBooleanArrayExtra(PersonaLauncherIntent.Extra.Scroll.Mapping.EXTRA_VIEW_CLICKABLE);
	...
	private void generateDataCache() {
        if (mItemMappings == null)
            return;
      PersonaLog.d("LAUNCHER","API v1 START QUERY");
        mAsyncQuery.startQuery(1, "cookie",
                Uri.parse(mIntent.getStringExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_DATA_URI)) ,
                mIntent.getStringArrayExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_PROJECTION),
                mIntent.getStringExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_SELECTION),
                mIntent.getStringArrayExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_SELECTION_ARGUMENTS),
                mIntent.getStringExtra(PersonaLauncherIntent.Extra.Scroll.EXTRA_SORT_ORDER));
	}
	...
	public void bindView(ViewHolder holder, View view, Context context, int itemPosition) {
		...
		try {
			...
			for (int i = size - 1; i >= 0; i--) {
				...
				case PersonaLauncherIntent.Extra.Scroll.Types.TEXTVIEW:
					...
					break;
				case PersonaLauncherIntent.Extra.Scroll.Types.TEXTVIEWHTML:
					if (!(child instanceof TextView))
						break;
					if (rowElement.data != null)
						...
						((TextView) child).setText(itemMapping.defaultResource);
					break;
				case PersonaLauncherIntent.Extra.Scroll.Types.IMAGEBLOB:
					...
					break;
				case PersonaLauncherIntent.Extra.Scroll.Types.IMAGEURI:
					if (!(child instanceof ImageView))
						break;
					iv = (ImageView) child;
                   if ((rowElement.data != null) && (!rowElement.data.equals(""))) {
                        ...
                        iv.setImageDrawable(null);
                    break;
                case PersonaLauncherIntent.Extra.Scroll.Types.IMAGERESOURCE:
                    if (!(child instanceof ImageView))
                        break;
                    iv = (ImageView) child;
            ...
            PersonaLog.d(LOG_TAG, "****** freeMemory = " + Runtime.getRuntime().freeMemory() / 1000
                    + " Kb");
		...
		// if (LOGD)
		// PersonaLog.d(LOG_TAG, "freeMemory = " + Runtime.getRuntime().freeMemory() /
		// 1000 + " Kb");

		if (Runtime.getRuntime().freeMemory() < 500000) {
			if (LOGD)
				PersonaLog.d(LOG_TAG, "force gargabe collecting below 500kb");
	...
	class ItemViewClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			try {
				...
				Intent intent = new Intent(PersonaLauncherIntent.Action.ACTION_VIEW_CLICK);
				intent.setComponent(mAppWidgetProvider);
				intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mAppWidgetId).putExtra(
						PersonaLauncherIntent.Extra.EXTRA_APPWIDGET_ID, mAppWidgetId);
				intent.putExtra(PersonaLauncherIntent.Extra.EXTRA_VIEW_ID, v.getId());
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			// if (LOGD)
			// PersonaLog.d(LOG_TAG, "*** new view");

		} else {
			holder = (ViewHolder) convertView.getTag();
			// if (LOGD)
			// PersonaLog.d(LOG_TAG, "*** recycled view");
		}
	...
	public void notifyToRegenerate() {
		if (LOGD)
			PersonaLog.d(LOG_TAG, "notifyToRegenerate widgetId = " + mAppWidgetId);
    ...
    private class MyQueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            PersonaLog.d("LAUNCHER", "API v1 QUERY COMPLETE");
            rowsElementsList.clear();
            final int size = mItemMappings.length;
