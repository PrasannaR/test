...
public class PersonaSecurityProfileUpdate extends FragmentActivity implements
	...
	private void updateUI() {
		//if (iResult == SUCCESS) {
		deleteLocalData(getApplicationContext());
		updateDB();
		killAllActivitiesInStack(mContext);
	...
	private void updateDB() {
		...
		values.put(PersonaDatabaseProvider.COL1, mCol_S);
		values.put(PersonaDatabaseProvider.COL2, mCol_SA);
		if(mAction ==  PersonaConstants.WIPE_APP) {
			values.put(PersonaDatabaseProvider.COL5,"N0");
		}
		getContentResolver().update(PersonaDatabaseProvider.ROUTINE_CHECK_CONTENT_URI, values,where, null);
	...
	//	if(!dir.getName().equals(TruBoxDatabase.getHashValue(PersonaDatabaseProvider.DB_NAME, ctx)) && !dir.getName().equals("encrypted.db3"))
		if(!dir.getName().equals(TruBoxDatabase.getHashValue(PersonaDatabaseProvider.DB_NAME, ctx)))
