...
public class PersonaSearch extends LinearLayout 
    ...
    // These four are passed to PersonaLauncher.startSearch() when the search widget
     ...
     */
    public PersonaSearch(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void startVoiceSearch() {
        try {
            ...
            PersonaLog.w(TAG, "Could not find voice search activity");
        }
    }
   ...
   /* private void showSearchDialog() {
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        if (!event.isSystem() && 
            ...
            // Forward key events to PersonaLauncher, which will forward text 
            // to search dialog
    ...
    protected void onFinishInflate() {
		...
		if(themeResources!=null){
	        int resource_id=0;
			//Action Buttons
			PersonaLauncher.loadThemeResource(themeResources,themePackage,"btn_search_dialog_voice",mVoiceButton,PersonaLauncher.THEME_ITEM_BACKGROUND);
			PersonaLauncher.loadThemeResource(themeResources,themePackage,"search_button_voice",mVoiceButton,PersonaLauncher.THEME_ITEM_FOREGROUND);
			PersonaLauncher.loadThemeResource(themeResources,themePackage,"textfield_searchwidget",mSearchText,PersonaLauncher.THEME_ITEM_BACKGROUND);
			View search_bg=findViewById(R.id.search_plate);
			PersonaLauncher.loadThemeResource(themeResources,themePackage,"search_floater",search_bg,PersonaLauncher.THEME_ITEM_BACKGROUND);
			...
			if(resource_id!=0){
				...
				googlePlaceholder=getContext().getResources().getDrawable(R.drawable.pr_placeholder_google);
			}
		}else{
			googlePlaceholder=getContext().getResources().getDrawable(R.drawable.pr_placeholder_google);
		}
    ...
    /**
     * Sets the {@link PersonaLauncher} that this gadget will call on to display the search dialog. 
     */
