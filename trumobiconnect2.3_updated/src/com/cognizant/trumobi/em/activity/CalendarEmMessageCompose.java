...
public class CalendarEmMessageCompose extends Activity implements OnClickListener, OnFocusChangeListener,OnTouchListener {
    ...
    public static void actionCompose(Context context, long accountId) {
       try {
           Intent i = new Intent(context, CalendarEmMessageCompose.class);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        event = CalendarDatabaseHelper.getEventDetails(getIntent()
				.getLongExtra("event_id", -1));
		...
		CalendarLog.d(CalendarConstants.Tag, "event obtained from cal compose "
    ...
    private void initViews() {
        ...
        mSubjectView.setText("Fwd: "+event.title);
        mMessageContentView = (TruMobBaseEditText)findViewById(R.id.message_content);
        mMessageContentView.setText(event.description);
...
//        mQuotedText.setVisibility(View.GONE);
		displayQuotedText(
				EmCalendarUtilities.buildMessageTextFromEventValues(
						getApplicationContext(), event, new StringBuilder()),
				null);
    ...
    private class LoadMessageTask extends AsyncTask<Long, Void, Object[]> {
        ...
        protected Object[] doInBackground(Long... messageIds) {
            ...
            Message message = Message.restoreMessageWithId(CalendarEmMessageCompose.this, messageIds[0]);
            if (message == null) {
                return new Object[] {null, null};
            }
            long accountId = message.mAccountKey;
            Account account = Account.restoreAccountWithId(CalendarEmMessageCompose.this, accountId);
            try {
                // Body body = Body.restoreBodyWithMessageId(MessageCompose.this, message.mId);
                message.mHtml = Body.restoreBodyHtmlWithMessageId(CalendarEmMessageCompose.this, message.mId);
                message.mText = Body.restoreBodyTextWithMessageId(CalendarEmMessageCompose.this, message.mId);
                boolean isEditDraft = ACTION_EDIT_DRAFT.equals(mAction);
                // the reply fields are only filled/used for Drafts.
                if (isEditDraft) {
                    message.mHtmlReply =
                        Body.restoreReplyHtmlWithMessageId(CalendarEmMessageCompose.this, message.mId);
                    message.mTextReply =
                        Body.restoreReplyTextWithMessageId(CalendarEmMessageCompose.this, message.mId);
                    message.mIntroText =
                        Body.restoreIntroTextWithMessageId(CalendarEmMessageCompose.this, message.mId);
                    message.mSourceKey = Body.restoreBodySourceKey(CalendarEmMessageCompose.this,
                                                                   message.mId);
                } else {
        ...
        protected void onPostExecute(Object[] messageAndAccount) {
            ...
            if (message == null && account == null) {
                ...
                // Bail out.
                Toast.makeText(CalendarEmMessageCompose.this, R.string.error_loading_message_body,
                               Toast.LENGTH_LONG).show();
                finish();
                return;
            }
            ...
            if (ACTION_EDIT_DRAFT.equals(mAction)) {
                ...
                mLoadAttachmentsTask = new AsyncTask<Long, Void, Attachment[]>() {
                    ...
                    protected Attachment[] doInBackground(Long... messageIds) {
                        return Attachment.restoreAttachmentsWithMessageId(CalendarEmMessageCompose.this,
                                messageIds[0]);
                    }
                    @Override
    ...
    public void updateMessageFromView(Message message) {
		...
		if (mSource != null && mQuotedText.getVisibility() == View.VISIBLE) {
			message.mSourceKey = mSource.mId;
			// If the quote bar is visible; this must either be a reply or
			// forward
    ...
    */
        ...
        mSaveMessageTask = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                synchronized (mDraft) {
                    if (mDraft.isSaved()) {
                        ...
                        Body.updateBodyWithMessageId(CalendarEmMessageCompose.this, mDraft.mId, values);
                    } else {
                    ...
                    for (Attachment attachment : attachments) {
                        if (!attachment.isSaved()) {
                            ...
                            attachment.save(CalendarEmMessageCompose.this);
                        }
                    }
            ...
            protected void onPostExecute(Void dummy) {
                ...
                if (!send && (getChangingConfigurations() & ActivityInfo.CONFIG_ORIENTATION) == 0) {
                    Toast.makeText(CalendarEmMessageCompose.this, R.string.message_saved_toast,
                            Toast.LENGTH_LONG).show();
                }
            }
        }.execute();
    ...
    private void onSend() {
        if (!isAddressAllValid()) {
			...
			try {
				...
				EmAddress[] addresses = EmAddress.parse(mToView
						.getText().toString().trim());
				ArrayList<Attendee> attendees = new ArrayList<Attendee>();
				for (EmAddress address : addresses) {
					Attendee attendee = new Attendee();
					attendee.attendeeEmail = address.getAddress();
					attendees.add(attendee);
				}
				event.attendees = attendees;
				event.title = mSubjectView.getText().toString();
				event.description =mMessageContentView.getText().toString();
				msg = EmCalendarUtilities.createMessageForEvent(event,
						account, Message.FLAG_OUTGOING_MEETING_INVITE,false);

				updateMessageFromView(msg);
			} catch (Exception e) {
			...
			if (msg != null) {
				...
				CalendarLog.d("mesage to addresses are ", msg.mTo);
				CalendarLog.d(CalendarConstants.Tag,
						"msg obtained from cal compose "
								+ msg.toContentValues().toString());
				
				msg.save(getApplicationContext());
				CalendarDatabaseHelper.insertMessageID((int) event._id,
						(int) msg.mId);
				setDraftNeedsSaving(false);
    ...
    private void addAttachment() {
    	...
    	if ((EmCalendarUtilities.getICSBytesForEvent(event, CalendarDatabaseHelper.getAccount()).length + getAttachedAttachmentsSizeFromUI()) > Email.MAX_ATTACHMENT_UPLOAD_SIZE) {	//NaGa, dont add attachment if the attachments together exceeds max limit
			Toast.makeText(
					this,
        ...
        nameView.setText("invite.ics");
//       ImageButton delButton=(ImageButton)view.findViewById(R.id.attachment_delete);       
