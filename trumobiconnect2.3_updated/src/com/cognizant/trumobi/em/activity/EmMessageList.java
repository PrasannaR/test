 ...
 * STRING_CHANGES		367712		For Implementing/Changing Error Messages
 */

public class EmMessageList extends TruMobiBaseListActivity implements OnItemClickListener, OnClickListener, OnTouchListener,
        AnimationListener {
    ...
    private static final String EXTRA_MAILBOX_TYPE = "com.cognizant.trumobi.em.activity.MAILBOX_TYPE";
    private static final String EXTRA_MAILBOX_ID = "com.cognizant.trumobi.em.activity.MAILBOX_ID";
    private static final String STATE_SELECTED_ITEM_TOP =
        "com.cognizant.trumobi.em.activity.MessageList.selectedItemTop";
    private static final String STATE_SELECTED_POSITION =
        "com.cognizant.trumobi.em.activity.MessageList.selectedPosition";
    private static final String STATE_CHECKED_ITEMS =
        "com.cognizant.trumobi.em.activity.MessageList.checkedItems";
    ...
    private int unreadMessageCount = 0;	//NaGa, Unread Count
    ...
    private final EmEmController mController = EmEmController.getInstance(getApplication());
    ...
    public final static String[] MAILBOX_FIND_INBOX_PROJECTION = new String[] {
        EmEmailContent.RECORD_ID, MailboxColumns.TYPE, MailboxColumns.FLAG_VISIBLE
    };
    ...
    private static final String MESSAGE_READ_SELECTION =
    ...
    private static final String ID_SELECTION = EmEmailContent.RECORD_ID + "=?";
    ...
    private static boolean isInbox = false, disableBackKey = false;
    
    

    /* package */ static final String[] MESSAGE_PROJECTION = new String[] {
        EmEmailContent.RECORD_ID, MessageColumns.MAILBOX_KEY, MessageColumns.ACCOUNT_KEY,
        ...
        MessageColumns.FLAGS, MessageColumns.TO_LIST,
    };
    ...
    public static void actionHandleMailbox(Context context, long id) {
        context.startActivity(createIntent(context, -1, id, -1, false));
    }
    
    public static void actionHandleMailbox(Context context, long id, String mailboxName) {
    	
    	isInbox = false;
    	isInbox = mailboxName.equalsIgnoreCase(context.getString(R.string.mailbox_name_display_inbox));
    	Log.e("mailboxNamemailboxName", "mailboxName : "+isInbox);
        context.startActivity(createIntent(context, -1, id, -1, false));
    }
     ...
     */
    public static void actionHandleAccount(Context context, long accountId, int mailboxType, boolean isfromAdmin) {
        context.startActivity(createIntent(context, accountId, -1, mailboxType, isfromAdmin));
    }
    ...
    public static void actionOpenAccountInboxUuid(Context context, String accountUuid) {
        Intent i = createIntent(context, -1, -1, Mailbox.TYPE_INBOX, false);
    ...
    public static Intent createIntent(Context context, long accountId, long mailboxId,
            int mailboxType, boolean fromadminsecurity) {
        Intent intent = new Intent(context, EmMessageList.class);
        if(isInbox)
        	intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);        
        if(!fromadminsecurity && (isApplicationSentToBackground(context) || getCurrentTopActivity(context))) {
        	Intent i = new Intent(context, PersonaMainActivity.class);
        	i.putExtra("newemail_notification", true);
    ...
    public static Intent createAccountIntentForShortcut(Context context, Account account,
            int mailboxType) {
        Intent i = createIntent(context, -1, -1, mailboxType, false);
    ...
    public void onCreate(Bundle icicle) {
        ...
        setContentView(R.layout.em_message_list);
        ...
        unreadCountView = (TextView) findViewById(R.id.unreadcount);
      ...
      //NaGa, back button functionality -->
        ...
        disableBackKey = true;
        Log.e("isInboxisInboxisInbox", "isInboxisInboxisInbox : "+isInbox);
        if (!isInbox && mMailboxId != -1)
        {
        ...
        EmSyncManager.setnetworkCallBack(mConnCB);
    ...
    public void onBackPressed() {
    	...
    	//PIN_SCREEN_ISSUE, -->
		boolean nefp = new SharedPreferences(this).getBoolean(
				"notifyEmailFrmPersona", false);
		if (nefp) {
			...
			new SharedPreferences(this).edit()
					.putBoolean("notifyEmailFrmPersona", false).commit();
	...
	private ConnectivityCallback mConnCB = new ConnectivityCallback() {

		@Override
		public void onWaiting() {
			// TODO Auto-generated method stub
			EmailLog.e("onWaitingonWaitingonWaiting",
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					// TODO Auto-generated method stub
					if (EmSyncManager.sConnectivityHold && mListFooterText != null)
						setListFooterText(false, false);
				}
			});
		...
		public void onConnected() {
			// TODO Auto-generated method stub
			EmailLog.e("onConnectedonConnectedonConnected",
					"Welcomeeeeeeeeeeeeeeeeeeeee");
			runOnUiThread(new Runnable() {
				...
				public void run() {
					// TODO Auto-generated method stub
					if (mListFooterText != null)
						setListFooterText(false, true);
				}
			});
		}
	};
    ...
    private void selectAccountAndMailbox(Intent intent) {
        ...
        if (mMailboxId != -1) {
            ...
            Log.v("mMailboxIdmMailboxId", "mMailboxIdmMailboxId "+mMailboxId);
            if(mMailboxId != Mailbox.QUERY_ALL_FAVORITES && mMailboxId != Mailbox.QUERY_ALL_INBOXES)	//NaGa, removing footer for starred and combined folders
    ...
    public void onResume() {
        ...
        notificationManager.cancel(EmMailService.NOTIFICATION_ID_NEW_MESSAGES);
      ...
      //NaGa, refreshing, in case if accounts removed -->
        int numAccounts = EmEmailContent.count(this,Account.CONTENT_URI, null, null);
        if (numAccounts == 0) {
        	EmWelcome.actionStart(this);
            finish();
        }
    ...
    protected void onDestroy() {
        super.onDestroy();
        //Log.e("onDestroyonDestroy", "onDestroyonDestroy : "+isInbox);
        //isInbox = false;
        EmUtility.cancelTaskInterrupt(mLoadMessagesTask);
        mLoadMessagesTask = null;
        EmUtility.cancelTaskInterrupt(mFindMailboxTask);
        mFindMailboxTask = null;
        EmUtility.cancelTaskInterrupt(mSetTitleTask);
        mSetTitleTask = null;
        EmUtility.cancelTaskInterrupt(mSetFooterTask);
        mSetFooterTask = null;
        
        if(mListAdapter != null)	//NaGa
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if (view != mListFooterView) {
            EmMessageListItem itemView = (EmMessageListItem) view;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_multi_delete:
                ...
                break;
            /*case R.id.account_title_button:	//NaGa
                onAccounts();
                break;*/
        }
    }
    ...
    class setContextMenuColor implements LayoutInflater.Factory {
                ...
                * object */
                new Handler().post( new Runnable() {  
					...
					public void run () {  
                    	...
                    	view.setBackgroundDrawable(getResources().getDrawable(R.drawable.em_message_list_item_background_read));
                        // sets the text color              
                        //((TextView) view).setTextColor(R.color.default_text_color);
                        // sets the text size              
    ...
    protected void setMenuBackground(){
        ...
        getLayoutInflater().setFactory( new setContextMenuColor()); }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        //setMenuBackground();
        if (mMailboxId < 0) {
            getMenuInflater().inflate(R.menu.em_message_list_option_smart_folder, menu);
        } else {
            getMenuInflater().inflate(R.menu.em_message_list_option, menu);
        }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
    	...
    	menu.clear();	//NaGa, all options are not visible
    	if (mMailboxId < 0) {
            getMenuInflater().inflate(R.menu.em_message_list_option_smart_folder, menu);
        } else {
            getMenuInflater().inflate(R.menu.em_message_list_option, menu);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.refresh:
            	//STRING_CHANGES, Show dialog when device offline -->
            	if(isOffLine())
            		showOffLineWarn();
            	else
            	//STRING_CHANGES, <--
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        ...
        EmMessageListItem itemView = (EmMessageListItem) info.targetView;
        ...
        switch (mailbox.mType) {
            case EmEmailContent.Mailbox.TYPE_DRAFTS:
                getMenuInflater().inflate(R.menu.em_message_list_context_drafts, menu);
                break;
            case EmEmailContent.Mailbox.TYPE_OUTBOX:
                getMenuInflater().inflate(R.menu.em_message_list_context_outbox, menu);
                break;
            case EmEmailContent.Mailbox.TYPE_TRASH:
                getMenuInflater().inflate(R.menu.em_message_list_context_trash, menu);
                break;
            default:
                getMenuInflater().inflate(R.menu.em_message_list_context, menu);
    ...
    AlertDialog OffLineAlert = null;
	private void showOffLineWarn() {
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
		alertDialogBuilder.setTitle(this
				.getString(R.string.account_offline_warn_dlg_title));
		// set dialog message
		alertDialogBuilder
				.setMessage(this.getString(R.string.account_offline_warn_dlg_msg))
				.setPositiveButton(this.getString(R.string.okay_action),
		...
		OffLineAlert = alertDialogBuilder.create();
		OffLineAlert.show();
	}
	//STRING_CHANGES, <--
    ...
    private void onFolders() {
        if (mMailboxId >= 0) {
            ...
            Mailbox mailbox = Mailbox.restoreMailboxWithId(this, mMailboxId);
            if (mailbox != null) {
                EmMailboxList.actionHandleAccount(this, mailbox.mAccountKey);
                //finish();	//NaGa, bug fix, #518
            }
        }
    }
    ...
    private long lookupAccountIdFromMailboxId(long mailboxId) {
        ...
        EmEmailContent.Mailbox mailbox =
            EmEmailContent.Mailbox.restoreMailboxWithId(this, mailboxId);
        if (mailbox == null) {
            return -2;
        }
    ...
    private void onCompose() {
        ...
        if (accountKey > -2) {
            EmMessageCompose.actionCompose(this, accountKey);
        } else {
            finish();
        }
    }
    ...
    private void onEditAccount() {
        long accountKey = lookupAccountIdFromMailboxId(mMailboxId);
        if (accountKey > -2) {
            EmAccountSettings.actionSettings(this, accountKey);
        } else {
            finish();
        }
    }
    ...
    private void onOpenMessage(long messageId, long mailboxId) {
        // TODO: Should not be reading from DB in UI thread
        EmEmailContent.Mailbox mailbox = EmEmailContent.Mailbox.restoreMailboxWithId(this, mailboxId);
        if (mailbox == null) {
            return;
        }

        if (mailbox.mType == EmEmailContent.Mailbox.TYPE_DRAFTS) {
            EmMessageCompose.actionEditDraft(this, messageId);
        } else {
            final boolean disableReply = (mailbox.mType == EmEmailContent.Mailbox.TYPE_TRASH);
            ...
            EmMessageView.actionView(this, messageId, mMailboxId, disableReply);
        }
    }

    private void onReply(long messageId) {
        EmMessageCompose.actionReply(this, messageId, false);
    }

    private void onReplyAll(long messageId) {
        EmMessageCompose.actionReply(this, messageId, true);
    }

    private void onForward(long messageId) {
        EmMessageCompose.actionForward(this, messageId);
    }
    ...
    private boolean isnetworkActive(Context context)	//NaGa, Error Banner
    {
    ...
    private void showMultiPanel(boolean show) {
        if (show && mMultiSelectPanel.getVisibility() != View.VISIBLE) {
            ...
            Animation animation = AnimationUtils.loadAnimation(this, R.anim.em_footer_appear);
            ...
            mMultiSelectPanel.startAnimation(
                        AnimationUtils.loadAnimation(this, R.anim.em_footer_disappear));
        }
        if (show) {
    ...
    private class SetFooterTask extends AsyncTask<Long, Void, Integer> {
        ...
        protected Integer doInBackground(Long... params) {
            ...
            if (accountId != -1) {
                ...
                Account account = Account.restoreAccountWithId(EmMessageList.this, accountId);
                if (account != null) {
                    mPushModeMailbox = account.mSyncInterval == Account.CHECK_INTERVAL_PUSH;
                    if (EmMessageList.this.mController.isMessagingController(account)) {
    ...
    private void finishFooterView(int listFooterMode) {
    	...
    	Log.v("mMailboxIdmMailboxId", "2222222222222222222222222 "+mMailboxId);
        mListFooterMode = listFooterMode;
        if (mListFooterMode != LIST_FOOTER_MODE_NONE || (mListFooterMode == LIST_FOOTER_MODE_NONE && mMailboxId == Mailbox.NO_MAILBOX)) 
        {
            mListFooterView = ((LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE))
                    .inflate(R.layout.em_message_list_item_footer, mListView, false);
     ...
     */
    private void setListFooterText(boolean active, boolean networkConnected) {
        ...
        {
        	if(networkConnected)
        		mListFooterText.setText((!isAdminActive())  ? R.string.eas_waiting_for_sync_security_message
        				: R.string.eas_waiting_for_sync);	//NaGa, change text for eas account
        	else
        		mListFooterText.setText("Waiting for connectivity");
        }
    }
    ...
    private class FindMailboxTask extends AsyncTask<Void, Void, Long> {
        ...
        protected Long doInBackground(Void... params) {
            ...
            long mailboxId = Mailbox.findMailboxOfType(EmMessageList.this, mAccountId, mMailboxType);
            if (mailboxId == Mailbox.NO_MAILBOX) {
                // Mailbox not found.  Does the account really exists?
                final boolean accountExists = Account.isValidId(EmMessageList.this, mAccountId);
        ...
        protected void onPostExecute(Long mailboxId) {
            if (showSecurityActivity) {
                ...
                Intent i = EmAccountSecurity.actionUpdateSecurityIntent(
                        EmMessageList.this, mAccountId);
                EmMessageList.this.startActivityForResult(i, REQUEST_SECURITY);
                return;
            }
            if (showWelcomeActivity) {
                // Let the Welcome activity show the default screen.
                EmWelcome.actionStart(EmMessageList.this);
                finish();
                return;
            }
    ...
    private boolean isSecurityHold(long accountId) {
        Cursor c = EmMessageList.this.getContentResolver().query(
    ...
    private class LoadMessagesTask extends AsyncTask<Void, Void, Cursor> {
        ...
        protected Cursor doInBackground(Void... params) {
            String selection =
                EmUtility.buildMailboxIdSelection(EmMessageList.this.mResolver, mMailboxKey);
            Cursor c = EmMessageList.this.managedQuery(
                    EmEmailContent.Message.CONTENT_URI, MESSAGE_PROJECTION,
                    selection, null, EmEmailContent.MessageColumns.TIMESTAMP + " DESC");
            if (isCancelled()) {
                c.close();
                c = null;
            }
        ...
        protected void onPostExecute(Cursor cursor) {
            ...
            EmMessageList.this.mListAdapter.changeCursor(cursor);
            ...
            if (mMailboxKey == Mailbox.QUERY_ALL_INBOXES) {
                EmMailService.resetNewMessageCount(EmMessageList.this, -1);
            } else if (mMailboxKey >= 0 && mAccountKey != -1) {
                EmMailService.resetNewMessageCount(EmMessageList.this, mAccountKey);
            }
            if(!isnetworkActive(getApplicationContext()))
            {
            	 showOffLineWarn();	//STRING_CHANGES
            	 mHandler.showErrorBanner(getString(R.string.status_network_error));
            	 //setTitleAccountName("",false);
            }
            //else	mSetTitleTask.execute();
        }
    }
    ...
    private class SetTitleTask extends AsyncTask<Void, Void, Object[]> {
        ...
        protected Object[] doInBackground(Void... params) {
            ...
            Cursor c = EmMessageList.this.mResolver.query(Mailbox.CONTENT_URI,
                    MAILBOX_NAME_PROJECTION, ID_SELECTION,
                    new String[] { Long.toString(mMailboxKey) }, null);
            try {
                if (c.moveToFirst()) {
                    mailboxName = EmUtility.FolderProperties.getInstance(EmMessageList.this)
            ...
            if (accountKey != null) {
                c = EmMessageList.this.mResolver.query(Account.CONTENT_URI,
            ...
            int nAccounts = EmEmailContent.count(EmMessageList.this, Account.CONTENT_URI, null, null);
        ...
        protected void onPostExecute(Object[] result) {
            ...
            if (result[0] != null) {
                setTitleAccountName(!isnetworkActive(getApplicationContext())? "":(String) result[0], nAccounts > 1);
                
                //NaGa, removing footer after sync, for eas -->
    ...
    class MessageListHandler extends Handler {
        ...
        public void handleMessage(android.os.Message msg) {
        	...
        	//NaGa, show loading for eas sync -->
			showProgress = (/*EmAccountSecurity.policyRequired*/ !isAdminActive()
					| (getListView().getCount() == 0 | getListView().getCount() == 1)) && (mMailboxId == Mailbox.NO_MAILBOX);
			if (!showProgress && !(msg.arg1 != 0)) {
				if (mListFooterView != null
						&& mListFooterMode == LIST_FOOTER_MODE_NONE) // NaGa
				{
					getListView().removeFooterView(mListFooterView);
			...
			if (mListFooterText != null && mListFooterMode == LIST_FOOTER_MODE_NONE)
				mListFooterText
						.setText((!isAdminActive())  ? R.string.eas_waiting_for_sync_security_message
								: R.string.eas_waiting_for_sync);	//NaGa, change text for eas account
			//NaGa, <--
            switch (msg.what) {
			case MSG_PROGRESS:
				...
				if (visible || showProgress) {
					...
					//if(mListFooterMode == LIST_FOOTER_MODE_NONE)
					//mListFooterView = ((LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE))
		                     //.inflate(R.layout.eas_loading, mListView, false);
                        ...
                        if (!isVisible) {
                            ...
                            mErrorBanner.startAnimation(
                                    AnimationUtils.loadAnimation(
                                            EmMessageList.this, R.anim.em_header_appear));
                        }
                    } else {
                        if (isVisible) {
                            ...
                            mErrorBanner.startAnimation(
                                    AnimationUtils.loadAnimation(
                                            EmMessageList.this, R.anim.em_header_disappear));
                        }
                    }
                    break;
    ...
    AlertDialog NetworkAlert = null;
    private boolean fromDialog = false;
	public void showNetworkErrorDiaolg(final EmMessagingException result,
            final long accountKey, final int progress) {
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(
				EmMessageList.context);
		alertDialogBuilder.setIcon(android.R.attr.alertDialogIcon);
		alertDialogBuilder.setTitle(EmMessageList.context
				...
				.setPositiveButton("Try again",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int id) {
								...
								// activity
								fromDialog = true;
								mControllerCallback.updateMailboxListCallback(result, accountKey, progress);
							}
						})
		...
		NetworkAlert = alertDialogBuilder.create();
		NetworkAlert.show();
	}
    ...
    private class ControllerResults implements EmEmController.Result {
        ...
        EmMessagingException mSendMessageException;
        ...
        public synchronized void updateMailboxListCallback(EmMessagingException result,
...
//        	EmailLog.e("updateMailboxListCallback", "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX *"+result.getExceptionType()+"*");
//            updateBanner(result, progress, mMailboxId);
        	if(result != null && result.getExceptionType() == EmMessagingException.IOERROR)	//367712, added "result != null" UI not getting refreshed after Sync 
//        		showNetworkErrorDiaolg(result, accountKey, progress);
        		updateBanner(result, progress, mMailboxId);
        	else
        	{
        		fromDialog = false;
         ...
         * @param progress
         */
        private void updateBanner(EmMessagingException result, int progress, long mailboxKey) {
            ...
            if (result != null) {
                ...
                if (result instanceof EmAuthenticationFailedException) {
                    ...
                    switch (result.getExceptionType()) {
                        ...
                        case EmMessagingException.SECURITY_POLICIES_REQUIRED:
                            id = R.string.account_setup_failed_security;
                            break;
                    }
                }
                //STRING_CHANGES, -->
                if(id == R.string.status_network_error)
                	showOffLineWarn();
                //STRING_CHANGES, <--
    ...
    /* package */ class MessageListAdapter extends CursorAdapter {
        ...
        public static final int COLUMN_TO_LIST = 10;	//NaGa
        ...
        public MessageListAdapter(Context context) {
            ...
            mAttachmentIcon = resources.getDrawable(R.drawable.em_ic_mms_attachment_small);
            mInvitationIcon = resources.getDrawable(R.drawable.em_ic_calendar_event_small);
            mFavoriteIconOn = resources.getDrawable(R.drawable.em_btn_star_on_normal_email_holo_light);	//NaGa
            mFavoriteIconOff = resources.getDrawable(R.drawable.em_btn_star_off_normal_email_holo_light);
            mSelectedIconOn = resources.getDrawable(R.drawable.em_btn_check_on_normal_holo_light);
            mSelectedIconOff = resources.getDrawable(R.drawable.em_btn_check_off_normal_holo_light);	//NaGa
        ...
        public void bindView(View view, Context context, Cursor cursor) {
			...
			boolean isSentBox = (mLeftTitle.getText().toString())
					.equalsIgnoreCase("Sent");

			if (isSentBox) {
				text = EmAddress.toFriendly(EmAddress.unpack(cursor
						.getString(COLUMN_TO_LIST)));
			}
			// else
            ...
            if (itemView.mRead) {
                ...
                subjectView.setTextColor(getResources().getColor(R.color.subject_text_color_read));
                fromView.setTypeface(Typeface.DEFAULT);
                //fromView.setTextColor(mTextColorSecondary);
                subjectView.setTextColor(getResources().getColor(R.color.senders_text_color_read));
                view.setBackgroundDrawable(context.getResources().getDrawable(
                        R.drawable.em_message_list_item_background_read));
            } else {
                subjectView.setTypeface(Typeface.DEFAULT_BOLD);
                subjectView.setTextColor(getResources().getColor(R.color.subject_text_color_unread));
                fromView.setTypeface(Typeface.DEFAULT_BOLD);
                //fromView.setTextColor(mTextColorPrimary);
                subjectView.setTextColor(getResources().getColor(R.color.senders_text_color_unread));
                view.setBackgroundDrawable(context.getResources().getDrawable(
                        R.drawable.em_message_list_item_background_unread));
                
                int unreadMessageCount = EmEmailContent.count(context, Message.CONTENT_URI,
                		MESSAGE_READ_SELECTION,
        				new String[] { String.valueOf(0) });
                
                if(unreadCountView == null)	
                	unreadCountView = (TextView) findViewById(R.id.unreadcount);
                if(unreadCountView != null && unreadMessageCount > 0)
                	unreadCountView.setText(""+unreadMessageCount);
                
//                Log.e("unreadMessageCount", "unreadMessageCountunreadMessageCount : "+unreadMessageCount);
			...
			if (itemView.mSelected) // NaGa, change highlight images
			{
				int selected_Drawable = (itemView.mRead) ? R.drawable.em_message_list_item_background_read_checked
						: R.drawable.em_message_list_item_background_unread_checked;

				itemView.setBackgroundDrawable(context.getResources()
						.getDrawable(selected_Drawable));
			}
        ...
        public View newView(Context context, Cursor cursor, ViewGroup parent) {
            return mInflater.inflate(R.layout.em_message_list_item, parent, false);
        }
         ...
         */
        public void updateSelected(EmMessageListItem itemView, boolean newSelected) {
            ...
            if(newSelected)
            	itemView.setBackgroundDrawable(getResources().getDrawable(R.drawable.em_list_selected_holo));
            else
            {
            	if(itemView.mRead)
            	{
            		Log.v("itemView.mRead", "itemView.mReaditemView.mRead");
            		itemView.setBackgroundDrawable(getResources().getDrawable(R.drawable.em_list_read_holo));
            	}
            	else
            		itemView.setBackgroundDrawable(getResources().getDrawable(R.drawable.em_list_unread_holo));
            }
            EmMessageList.this.showMultiPanel(mChecked.size() > 0);
        }
         ...
         */
        public void updateFavorite(EmMessageListItem itemView, boolean newFavorite) {
