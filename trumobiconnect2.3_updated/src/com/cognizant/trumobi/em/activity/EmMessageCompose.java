...
public class EmMessageCompose extends TruMobiBaseActivity implements OnClickListener, OnFocusChangeListener,OnTouchListener {
    private static final String ACTION_REPLY = "com.cognizant.trumobi.em.intent.action.REPLY";
    private static final String ACTION_REPLY_ALL = "com.cognizant.trumobi.em.intent.action.REPLY_ALL";
    private static final String ACTION_FORWARD = "com.cognizant.trumobi.em.intent.action.FORWARD";
    private static final String ACTION_EDIT_DRAFT = "com.cognizant.trumobi.em.intent.action.EDIT_DRAFT";

    private static final String EXTRA_ACCOUNT_ID = "account_id";
    private static final String EXTRA_MESSAGE_ID = "message_id";
    private static final String STATE_KEY_CC_SHOWN =
        "com.cognizant.trumobi.em.activity.MessageCompose.ccShown";
    private static final String STATE_KEY_BCC_SHOWN =
        "com.cognizant.trumobi.em.activity.MessageCompose.bccShown";
    private static final String STATE_KEY_QUOTED_TEXT_SHOWN =
        "com.cognizant.trumobi.em.activity.MessageCompose.quotedTextShown";
    private static final String STATE_KEY_SOURCE_MESSAGE_PROCED =
        "com.cognizant.trumobi.em.activity.MessageCompose.stateKeySourceMessageProced";
    private static final String STATE_KEY_DRAFT_ID =
        "com.cognizant.trumobi.em.activity.MessageCompose.draftId";
    ...
    private TruMobBaseEditText mSubjectView;
    private TruMobBaseEditText mMessageContentView;
    ...
    private EmEmailAddressAdapter mAddressAdapterCc;
    private EmEmailAddressAdapter mAddressAdapterBcc;
    ...
    public static void actionCompose(Context context, long accountId) {
       try {
           Intent i = new Intent(context, EmMessageCompose.class);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.em_message_compose);
        ...
        if (Intent.ACTION_VIEW.equals(mAction)
            ...
            Log.v("SaveMessageeeeeeeeeeeeeee", "55555555555555555555");
            setDraftNeedsSaving(true);
            mMessageLoaded = true;
            mSourceMessageProcessed = true;
        } else if( (intent.getStringExtra("ContactsAppHandshake") != null) && intent.getStringExtra("ContactsAppHandshake").equals("ContactsAppHandshake")) {	//NaGa, for Contacts HandShake
        	setAccount(intent);
        	initFromIntent(intent);
        	setDraftNeedsSaving(true);
            mMessageLoaded = true;
        ...
        if (ACTION_REPLY.equals(mAction))
        	barTitle.setText(R.string.reply_action);
        else if (ACTION_REPLY_ALL.equals(mAction))
        	barTitle.setText(R.string.reply_all_action);
        else
        	barTitle.setText(R.string.compose_title);
        //NaGa, <--
        updateTitle();
    }
    ...
    protected void onStop() {
    	...
    	super.onStop();
    	//STRING_CHANGES, -->
    	if(showWarningAlert != null)
    		showWarningAlert.dismiss();
    	//STRING_CHANGES, <--
    }
    ...
    private void initViews() {
        ...
        mSubjectView = (TruMobBaseEditText)findViewById(R.id.subject);
        mMessageContentView = (TruMobBaseEditText)findViewById(R.id.message_content);
        ...
        TextWatcher watcher = new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start,
            	...
            	Log.v("SaveMessageeeeeeeeeeeeeee", "6666666666666666");
                setDraftNeedsSaving(true);
            }
    ...
    private void setupAddressAdapters() {
        ...
        if (true) {
            ...
            mAddressAdapterCc = new EmGalEmailAddressAdapter(this);
            mAddressAdapterBcc = new EmGalEmailAddressAdapter(this);
        } else {
            /* EXCHANGE-REMOVE-SECTION-END */
            mAddressAdapterTo = new EmEmailAddressAdapter(this);
            mAddressAdapterCc = new EmEmailAddressAdapter(this);
            mAddressAdapterBcc = new EmEmailAddressAdapter(this);
            /* EXCHANGE-REMOVE-SECTION-START */
        }
        /* EXCHANGE-REMOVE-SECTION-END */
    }
    ...
    private class LoadMessageTask extends AsyncTask<Long, Void, Object[]> {
        ...
        protected Object[] doInBackground(Long... messageIds) {
            ...
            Message message = Message.restoreMessageWithId(EmMessageCompose.this, messageIds[0]);
            if (message == null) {
                return new Object[] {null, null};
            }
            long accountId = message.mAccountKey;
            Account account = Account.restoreAccountWithId(EmMessageCompose.this, accountId);
            try {
                // Body body = Body.restoreBodyWithMessageId(MessageCompose.this, message.mId);
                message.mHtml = Body.restoreBodyHtmlWithMessageId(EmMessageCompose.this, message.mId);
                message.mText = Body.restoreBodyTextWithMessageId(EmMessageCompose.this, message.mId);
                boolean isEditDraft = ACTION_EDIT_DRAFT.equals(mAction);
                // the reply fields are only filled/used for Drafts.
                if (isEditDraft) {
                    message.mHtmlReply =
                        Body.restoreReplyHtmlWithMessageId(EmMessageCompose.this, message.mId);
                    message.mTextReply =
                        Body.restoreReplyTextWithMessageId(EmMessageCompose.this, message.mId);
                    message.mIntroText =
                        Body.restoreIntroTextWithMessageId(EmMessageCompose.this, message.mId);
                    message.mSourceKey = Body.restoreBodySourceKey(EmMessageCompose.this,
                                                                   message.mId);
                } else {
        ...
        protected void onPostExecute(Object[] messageAndAccount) {
            ...
            if (message == null && account == null) {
                ...
                // Bail out.
                Toast.makeText(EmMessageCompose.this, R.string.error_loading_message_body,
                               Toast.LENGTH_LONG).show();
                finish();
                return;
            }
            ...
            if (ACTION_EDIT_DRAFT.equals(mAction)) {
                ...
                mLoadAttachmentsTask = new AsyncTask<Long, Void, Attachment[]>() {
                    ...
                    protected Attachment[] doInBackground(Long... messageIds) {
                        return Attachment.restoreAttachmentsWithMessageId(EmMessageCompose.this,
                                messageIds[0]);
                    }
                    @Override
    ...
    private void addAddresses(MultiAutoCompleteTextView view, EmAddress[] addresses) {
        if (addresses == null) {
            return;
        }
        for (EmAddress address : addresses) {
    ...
    private String getPackedAddresses(TextView view) {
        EmAddress[] addresses = EmAddress.parse(view.getText().toString().trim());
        return EmAddress.pack(addresses);
    }

    private EmAddress[] getAddresses(TextView view) {
        EmAddress[] addresses = EmAddress.parse(view.getText().toString().trim());
        return addresses;
    }
    ...
    private String makeDisplayName(String packedTo, String packedCc, String packedBcc) {
        EmAddress first = null;
        int nRecipients = 0;
        for (String packed: new String[] {packedTo, packedCc, packedBcc}) {
            EmAddress[] addresses = EmAddress.unpack(packed);
    ...
    private void updateMessage(Message message, Account account, boolean hasAttachments) {
        if (message.mMessageId == null || message.mMessageId.length() == 0) {
            message.mMessageId = EmUtility.generateMessageId();
        }
        message.mTimeStamp = System.currentTimeMillis();
        message.mFrom = new EmAddress(account.getEmailAddress(), account.getSenderName()).pack();
        ...
        if (mSource != null && mQuotedTextBar.getVisibility() == View.VISIBLE) {
            ...
            String fromAsString = EmAddress.unpackToString(mSource.mFrom);
            if (ACTION_FORWARD.equals(mAction)) {
                message.mFlags |= Message.FLAG_TYPE_FORWARD;
                String subject = mSource.mSubject;
                String to = EmAddress.unpackToString(mSource.mTo);
                String cc = EmAddress.unpackToString(mSource.mCc);
                message.mIntroText =
    ...
    private int getAttachedAttachmentsSizeFromUI() {	//NaGa, to get the attached files size
    	int totalSize = 0;
		try {
			int count = mAttachments.getChildCount();
			Attachment[] attachments = new Attachment[count];
			for (int i = 0; i < count; ++i) {
				attachments[i] = (Attachment) mAttachments.getChildAt(i)
						.getTag();
				totalSize += attachments[i].mSize;
			}
			Log.e("totalSizetotalSize", "totalSizetotalSize "+totalSize);
			return totalSize;
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    */
    private long getOrCreateDraftId() {
        synchronized (mDraft) {
            ...
            mController.saveToMailbox(mDraft, EmEmailContent.Mailbox.TYPE_DRAFTS);
            return mDraft.mId;
        }
    }
        ...
        mSaveMessageTask = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                synchronized (mDraft) {
                    if (mDraft.isSaved()) {
                        ...
                        Body.updateBodyWithMessageId(EmMessageCompose.this, mDraft.mId, values);
                    } else {
                    ...
                    for (Attachment attachment : attachments) {
                        if (!attachment.isSaved()) {
                            ...
                            attachment.save(EmMessageCompose.this);
                        }
                    }
            ...
            protected void onPostExecute(Void dummy) {
                ...
                if (!send && (getChangingConfigurations() & ActivityInfo.CONFIG_ORIENTATION) == 0) {
                    Toast.makeText(EmMessageCompose.this, R.string.message_saved_toast,
                            Toast.LENGTH_LONG).show();
                }
            }
        }.execute();
    ...
    /* package */ boolean isAddressAllValid() {
        for (TextView view : new TextView[]{mToView, mCcView, mBccView}) {
            ...
            if (!EmAddress.isAllValid(addresses)) {
	...
	boolean isToAddressValid() {
		String addresses = mToView.getText().toString().trim();
		if (!EmAddress.isAllValid(addresses))
			return false;
		return true;
	}
    
	AlertDialog showWarningAlert;
	private void showEmailInvalidWarn(String emailId) {
		showWarningAlert = null;
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
		alertDialogBuilder.setTitle(this
				.getString(R.string.account_send_message_warn_dlg_title));
		// set dialog message
		alertDialogBuilder
				.setMessage(this.getString(R.string.account_send_message_warn_dlg_message, emailId))
		...
		showWarningAlert = alertDialogBuilder.create();
	...
	private void showOfflineSendWarn() {
		showWarningAlert = null;
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
		alertDialogBuilder.setTitle(this
				.getString(R.string.account_offline_send_message_dlg_title));
		// set dialog message
		alertDialogBuilder
				.setMessage(this.getString(R.string.account_offline_send_message_dlg_message))
				.setPositiveButton(this.getString(R.string.okay_action),
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int id) {
								// if this button is clicked, close current
    ...
    private void onSend() {
    	//STRING_CHANGES, show warning on invalid To address-->
    	if(!isToAddressValid())
    		showEmailInvalidWarn(mToView.getText().toString());
    	else
    	//STRING_CHANGES, <--
        ...
        else if(mSubjectView.getText().toString().length() == 0)
        {
        	AlertDialog noSubjectAlert = null;
        	noSubjectAlert = showDialogView("Do you want to send this message without a subject?");
        	noSubjectAlert.show();
        }
        else {
        	if(isOffLine())
    ...
    public AlertDialog  showDialogView(String dialogTxt){
    ...
    //STRING_CHANGES, -->
	private void showDiscardWarn() {
		showWarningAlert = null;
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
		alertDialogBuilder.setTitle(this.getString(R.string.delete_action));
		// set dialog message
		alertDialogBuilder
				.setMessage(
						this.getString(R.string.account_discard_warn_dlg_msg))
				.setPositiveButton(this.getString(R.string.cancel_action),
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int id) {
								// if this button is clicked, close current
								// activity
				...
				.setNegativeButton(this.getString(R.string.delete_action),
						new DialogInterface.OnClickListener() {
    ...
    private void addAttachment(Attachment attachment) {
    	...
    	if ((attachment.mSize + getAttachedAttachmentsSizeFromUI()) > Email.MAX_ATTACHMENT_UPLOAD_SIZE) {	//NaGa, dont add attachment if the attachments together exceeds max limit
			Toast.makeText(
					this,
					(getAttachedAttachmentsSizeFromUI() == 0) ? R.string.message_compose_attachment_size
							: R.string.message_compose_attachment_size_multi,
					Toast.LENGTH_LONG).show();
			return;
        }

        View view = getLayoutInflater().inflate(R.layout.em_message_compose_attachment,
                mAttachments, false);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.v("SaveMessageeeeeeeeeeeeeee", "111111111111111");
        setDraftNeedsSaving(true);
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.discard:
                //onDiscard();
            	showDiscardWarn();	//STRING_CHANGES
                break;
            ...
            case R.id.quoted_text_delete:
                ...
                mDraft.mSourceKey = 0;
                Log.v("SaveMessageeeeeeeeeeeeeee", "22222222222222");
                setDraftNeedsSaving(true);
                break;
        }
    }
    ...
    private void onDeleteAttachment(View delButtonView) {
        ...
        Log.v("SaveMessageeeeeeeeeeeeeee", "33333333333333333");
        setDraftNeedsSaving(true);
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.em_message_compose_option, menu);
        return true;
    }
...
//     }
    ...
    /* package */ void initFromIntent(Intent intent) {
        ...
        if (Intent.ACTION_SEND.equals(mAction) && intent.hasExtra(Intent.EXTRA_STREAM)) {
            ...
            if (stream != null && type != null) {
                if (EmMimeUtility.mimeTypeMatches(type,
        ...
        if ((intent.getStringExtra("ContactsAppHandshake") != null) && 
        		intent.getStringExtra("ContactsAppHandshake").equals("ContactsAppHandshake")
            ...
            if (list != null) {
                for (Parcelable parcelable : list) {
                    ...
                    if (uri != null) {
                        ...
                        if (EmMimeUtility.mimeTypeMatches(attachment.mMimeType,
                                Email.ACCEPTABLE_ATTACHMENT_SEND_INTENT_TYPES)) {
                            addAttachment(attachment);
                        }
                    }
    ...
    private void initializeFromMailTo(String mailToString) {
        ...
        try {
            ...
            Log.e("initializeFromMailTo", "initializeFromMailTo : "+to);
    ...
    private void displayQuotedText(String textBody, String htmlBody) {
        ...
        if (text != null) {
            text = plainTextFlag ? EmEmailHtmlUtil.escapeCharacterToDisplay(text) : text;
            ...
            if (mQuotedText != null) {
                ...
                //Log.e("rrrrrrrrrrrrrrrrrr",""+mQuotedText.getUrl());
                //openWebApp(mQuotedText.getUrl());
            }
        }
    }
    private void openWebApp(String binary_source_path) {
        ...
        Intent openWebAppIntent = getPackageManager().getLaunchIntentForPackage("com.cognizant.trumobi.securebrowser");
        if (openWebAppIntent != null) {
               ...
               AlertDialog.Builder alertSBNotFound = new AlertDialog.Builder(this);
               alertSBNotFound.setTitle("Alert");
               alertSBNotFound.setMessage("Please install secure browser.");
               alertSBNotFound.setCancelable(true);
               alertSBNotFound.setNeutralButton(android.R.string.ok,
                    new DialogInterface.OnClickListener() {
                             public void onClick(DialogInterface dialog, int id) {
                                   dialog.cancel();
                }
            });
               AlertDialog installAlert = alertSBNotFound.create();
               installAlert.show();
        }
 }
    ...
    private boolean safeAddAddresses(String addrs, String ourAddress,
            MultiAutoCompleteTextView view, ArrayList<EmAddress> addrList) {
        boolean added = false;
        for (EmAddress address : EmAddress.unpack(addrs)) {
    ...
    /*package*/ void setupAddressViews(Message message, Account account,
         ...
         * or sender address.
         */
        EmAddress[] replyToAddresses = EmAddress.unpack(message.mReplyTo);
        if (replyToAddresses.length == 0) {
            replyToAddresses = EmAddress.unpack(message.mFrom);
        }
        addAddresses(mToView, replyToAddresses);

        if (replyAll) {
            // Keep a running list of addresses we're sending to
            ArrayList<EmAddress> allAddresses = new ArrayList<EmAddress>();
            String ourAddress = account.mEmailAddress;

            for (EmAddress address: replyToAddresses) {
    ...
    void processSourceMessage(Message message, Account account) {
    	Log.v("SaveMessageeeeeeeeeeeeeee", "444444444444444444444");
        setDraftNeedsSaving(true);
...
//                 }
            ...
            mSubjectView.setText(subject);
            addAddresses(mToView, EmAddress.unpack(message.mTo));
    ...
    private class Listener implements EmEmController.Result {
        public void updateMailboxListCallback(EmMessagingException result, long accountId,
                int progress) {
        }

        public void updateMailboxCallback(EmMessagingException result, long accountId,
