...
/**
 * 
 * This class implements a launcher shortcut for directly accessing a single
 * account.
 * 
 * This is simply a lightweight version of Accounts, and should almost certainly
 * be merged with it (or, one could be a base class of the other).
 */
public class EmAccountShortcutPicker extends TruMobiBaseListActivity implements
		OnItemClickListener {
	...
	private final static String[] FROM_COLUMNS = new String[] {
			EmEmailContent.AccountColumns.DISPLAY_NAME,
			EmEmailContent.AccountColumns.EMAIL_ADDRESS,
			EmEmailContent.RECORD_ID };
	private final static int[] TO_IDS = new int[] { R.id.description,
			R.id.email, R.id.new_message_count };
	...
	public void onCreate(Bundle icicle) {
		...
		// TODO: lightweight projection with only those columns needed for this
		// display
		// TODO: query outside of UI thread
		Cursor c = this.managedQuery(EmEmailContent.Account.CONTENT_URI,
		...
		requestWindowFeature(Window.FEATURE_CUSTOM_TITLE); // NaGa, adding title
															// Bar
		setContentView(R.layout.em_accounts);
		...
		AccountsAdapter a = new AccountsAdapter(this,
				R.layout.em_accounts_item, c, FROM_COLUMNS, TO_IDS);
	...
	private static class AccountsAdapter extends SimpleCursorAdapter {
		...
		public AccountsAdapter(Context context, int layout, Cursor c,
				String[] from, int[] to) {
		...
		/**
		 * This is only used for the unread messages count. Most of the views
		 * are handled normally by SimpleCursorAdapter.
		 */
		private static class MyViewBinder implements
			...
			public boolean setViewValue(View view, Cursor cursor,
				...
				if (view.getId() == R.id.new_message_count) {
					...
					int unreadMessageCount = 0; // TODO get unread count from
												// Account
	...
	private void setupShortcut(Account account) {
		...
		Intent shortcutIntent = EmMessageList.createAccountIntentForShortcut(
