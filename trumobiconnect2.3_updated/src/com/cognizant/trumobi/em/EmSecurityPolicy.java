...
/**
 * Utility functions to support reading and writing security policies, and
 * handshaking the device into and out of various security states.
 */
public class EmSecurityPolicy {

	private static EmSecurityPolicy sInstance = null;
	public static boolean skip_Admin_Policy = true;
	private Context mContext;
	private DevicePolicyManager mDPM;
	private ComponentName mAdminName;
	private PolicySet mAggregatePolicy;

	/* package */static final PolicySet NO_POLICY_SET = new PolicySet(0,
			PolicySet.PASSWORD_MODE_NONE, 0, 0, false);
	...
	private static final String[] ACCOUNT_SECURITY_PROJECTION_EAS = new String[] {
			AccountColumns.ID, AccountColumns.SECURITY_FLAGS,
			AccountColumns.EMAIL_ADDRESS };
	private static final int ACCOUNT_SECURITY_COLUMN_FLAGS = 1;
	// Note, this handles the NULL case to deal with older accounts where the
	// column was added
	private static final String WHERE_ACCOUNT_SECURITY_NONZERO = Account.SECURITY_FLAGS
			+ " IS NOT NULL AND " + Account.SECURITY_FLAGS + "!=0";
	private static final String WHERE_ACCOUNT_SECURITY_ZERO = Account.SECURITY_FLAGS
			+ " IS NOT NULL AND " + Account.SECURITY_FLAGS + "=0";

	/**
	 * This projection on Account is for clearing the "security hold" column.
	 * Also includes the security flags column, so we can use it for selecting.
	 */
	...
	private static final int ACCOUNT_FLAGS_COLUMN_EMAIL = 2;
	private static final String TAG = EmSecurityPolicy.class.getSimpleName();

	/**
	 * Get the security policy instance
	 */
	public synchronized static EmSecurityPolicy getInstance(Context context) {
		if (sInstance == null) {
			sInstance = new EmSecurityPolicy(context);
		}
		return sInstance;
	}
	 ...
	 */
	private EmSecurityPolicy(Context context) {
		mContext = context;
		mDPM = null;
	...
	/**
	 * Compute the aggregate policy for all accounts that require it, and record
	 * it.
	 * 
	 * The business logic is as follows: min password length take the max
	 * password mode take the max (strongest mode) max password fails take the
	 * min max screen lock time take the min require remote wipe take the max
	 * (logical or)
	 * 
	 * @return a policy representing the strongest aggregate. If no policy sets
	 *         are defined, a lightweight "nothing required" policy will be
	 *         returned. Never null.
	 */
	/* package */PolicySet computeAggregatePolicy() {
		...
		Cursor c = mContext.getContentResolver().query(Account.CONTENT_URI,
				ACCOUNT_SECURITY_PROJECTION, WHERE_ACCOUNT_SECURITY_NONZERO,
				null, null);
		try {
			while (c.moveToNext()) {
				...
				if (flags != 0) {
					...
					minPasswordLength = Math.max(p.mMinPasswordLength,
							minPasswordLength);
					passwordMode = Math.max(p.mPasswordMode, passwordMode);
					if (p.mMaxPasswordFails > 0) {
						maxPasswordFails = Math.min(p.mMaxPasswordFails,
								maxPasswordFails);
					}
					if (p.mMaxScreenLockTime > 0) {
						maxScreenLockTime = Math.min(p.mMaxScreenLockTime,
								maxScreenLockTime);
					}
		...
		if (policiesFound) {
			...
			// (not specified)
			if (minPasswordLength == Integer.MIN_VALUE)
				minPasswordLength = 0;
			if (passwordMode == Integer.MIN_VALUE)
				passwordMode = 0;
			if (maxPasswordFails == Integer.MAX_VALUE)
				maxPasswordFails = 0;
			if (maxScreenLockTime == Integer.MAX_VALUE)
				maxScreenLockTime = 0;

			return new PolicySet(minPasswordLength, passwordMode,
					maxPasswordFails, maxScreenLockTime, requireRemoteWipe);
		} else {
	...
	/**
	 * Get the dpm. This mainly allows us to make some utility calls without it,
	 * for testing.
	 */
	...
	/**
	 * API: Report that policies may have been updated due to rewriting values
	 * in an Account.
	 * 
	 * @param accountId
	 *            the account that has been updated, -1 if unknown/deleted
	 */
	...
	/**
	 * API: Report that policies may have been updated *and* the caller vouches
	 * that the change is a reduction in policies. This forces an immediate
	 * change to device state. Typically used when deleting accounts, although
	 * we may use it for server-side policy rollbacks.
	 */
	public void reducePolicies() {
		if(!skip_Admin_Policy)	//367712, Admin Disable
		{
			updatePolicies(-1);
			setActivePolicies();
		}
	...
	/**
	 * API: Query used to determine if a given policy is "active" (the device is
	 * operating at the required security level).
	 * 
	 * This can be used when syncing a specific account, by passing a specific
	 * set of policies for that account. Or, it can be used at any time to
	 * compare the device state against the aggregate set of device policies
	 * stored in all accounts.
	 * 
	 * This method is for queries only, and does not trigger any change in
	 * device state.
	 * 
	 * @param policies
	 *            the policies requested, or null to check aggregate stored
	 *            policies
	 * @return true if the policies are active, false if not active
	 */
	public boolean isActive(PolicySet policies) {
		
		if(skip_Admin_Policy)	return true;	//367712, Admin Disable
		...
		if (dpm.isAdminActive(mAdminName)) {
			...
			if (policies.mPasswordMode > 0) {
				if (dpm.getPasswordQuality(mAdminName) < policies
						.getDPManagerPasswordQuality()) {
					return false;
				}
			...
			// no check required for remote wipe (it's supported, if we're the
			// admin)
	 ...
	 * If the set is empty, we release our device administration. If the set is
	 * non-empty, we only proceed if we are already active as an admin.
	 */
	public void setActivePolicies() {
		...
		if (policies == NO_POLICY_SET) {
			...
			dpm.setPasswordQuality(mAdminName,
					policies.getDPManagerPasswordQuality());
			dpm.setPasswordMinimumLength(mAdminName,
					policies.mMinPasswordLength);
			// screen lock time
			dpm.setMaximumTimeToLock(mAdminName,
					policies.mMaxScreenLockTime * 1000);
			// local wipe (failed passwords limit)
			dpm.setMaximumFailedPasswordsForWipe(mAdminName,
					policies.mMaxPasswordFails);
		}
	}

	/**
	 * API: Set/Clear the "hold" flag in any account. This flag serves a dual
	 * purpose: Setting it gives us an indication that it was blocked, and
	 * clearing it gives EAS a signal to try syncing again.
	 */
	...
	/**
	 * Clear all account hold flags that are set. This will trigger watchers,
	 * and in particular will cause EAS to try and resync the account(s).
	 */
	public void clearAccountHoldFlags() {
		ContentResolver resolver = mContext.getContentResolver();
		Cursor c = resolver.query(Account.CONTENT_URI,
				ACCOUNT_FLAGS_PROJECTION, WHERE_ACCOUNT_SECURITY_NONZERO, null,
				null);
		if(c != null) {
		try {
			while (c.moveToNext()) {
	...
	/**
	 * API: Sync service should call this any time a sync fails due to
	 * isActive() returning false. This will kick off the
	 * notify-acquire-admin-state process and/or increase the security level.
	 * The caller needs to write the required policies into this account before
	 * making this call. Should not be called from UI thread - uses DB lookups
	 * to prepare new notifications
	 * 
	 * @param accountId
	 *            the account for which sync cannot proceed
	 */
	public void policiesRequired(long accountId) {
		
		if(skip_Admin_Policy)	return;	//367712, Admin Disable
		
		EmailLog.d(TAG, ">>>>>>>>>>>>>>>>>>>> Entering policies required ");
		Account account = EmEmailContent.Account.restoreAccountWithId(mContext,
				accountId);
		...
		String tickerText = mContext.getString(
				R.string.security_notification_ticker_fmt,
				account.getDisplayName());
		String contentTitle = mContext
				.getString(R.string.security_notification_content_title);
		String contentText = account.getDisplayName();
		String ringtoneString = account.getRingtone();
		Uri ringTone = (ringtoneString == null) ? null : Uri
				.parse(ringtoneString);
		...
		Intent intent = EmAccountSecurity.actionUpdateSecurityIntent(mContext,
				accountId);
		PendingIntent pending = PendingIntent.getActivity(mContext, 0, intent,
				PendingIntent.FLAG_UPDATE_CURRENT);

		Notification notification = new Notification(
				R.drawable.pr_connect_header_icon, tickerText,
				System.currentTimeMillis());
		notification.setLatestEventInfo(mContext, contentTitle, contentText,
				pending);
		notification.icon = R.drawable.pr_connect_header_icon;
		
		// Use the account's notification rules for sound & vibrate (but always
		// notify)
		...
		notificationManager.notify(
				EmMailService.NOTIFICATION_ID_SECURITY_NEEDED, notification);
		
		EmailLog.d(TAG, "<<<<<<<<<<<<<<<<<<<<<< Leaving policies required ");
	}

	/**
	 * Called from the notification's intent receiver to register that the
	 * notification can be cleared now.
	 */
	public void clearNotification(long accountId) {
		NotificationManager notificationManager = (NotificationManager) mContext
				.getSystemService(Context.NOTIFICATION_SERVICE);
		notificationManager
				.cancel(EmMailService.NOTIFICATION_ID_SECURITY_NEEDED);
	}

	/**
	 * API: Remote wipe (from server). This is final, there is no confirmation.
	 * It will only return to the caller if there is an unexpected failure.
	 */
	public void remoteWipe() {
		...
		if (dpm.isAdminActive(mAdminName)) {
			...
			Log.d(Email.LOG_TAG,
					"Could not remote wipe because not device admin.");
		}
	}
	...
	public static class PolicySet {
		 ...
		 *            (0=not enforced)
		 * @param passwordMode
		 * @param maxPasswordFails
		 *            (0=not enforced)
		 * @param maxScreenLockTime
		 *            in seconds (0=not enforced)
		 * @param requireRemoteWipe
		 * @throws IllegalArgumentException
		 *             for illegal arguments.
		 */
		public PolicySet(int minPasswordLength, int passwordMode,
				int maxPasswordFails, int maxScreenLockTime,
				boolean requireRemoteWipe) throws IllegalArgumentException {
			...
			if (passwordMode == PASSWORD_MODE_NONE) {
				...
				// The next value has a hard limit which cannot be supported if
				// exceeded.
		...
		public PolicySet(int flags) {
			mMinPasswordLength = (flags & PASSWORD_LENGTH_MASK) >> PASSWORD_LENGTH_SHIFT;
			mPasswordMode = (flags & PASSWORD_MODE_MASK);
			mMaxPasswordFails = (flags & PASSWORD_MAX_FAILS_MASK) >> PASSWORD_MAX_FAILS_SHIFT;
			mMaxScreenLockTime = (flags & SCREEN_LOCK_TIME_MASK) >> SCREEN_LOCK_TIME_SHIFT;
		 ...
		 * 
		 * @param account
		 *            to write the values mSecurityFlags and mSecuritySyncKey
		 * @param syncKey
		 *            the value to write into the account's mSecuritySyncKey
		 * @param update
		 *            if true, also writes the account back to the provider
		 *            (updating only the fields changed by this API)
		 * @param context
		 *            a context for writing to the provider
		 * @return true if the actual policies changed, false if no change
		 *         (note, sync key does not affect this)
		 */
		public boolean writeAccount(Account account, String syncKey,
			...
			if (update) {
				if (account.isSaved()) {
					...
					cv.put(AccountColumns.SECURITY_SYNC_KEY,
		...
		public String toString() {
			return "{ " + "pw-len-min=" + mMinPasswordLength + " pw-mode="
					+ mPasswordMode + " pw-fails-max=" + mMaxPasswordFails
					+ " screenlock-max=" + mMaxScreenLockTime
					+ " remote-wipe-req=" + mRequireRemoteWipe + "}";
		}
	}
	...
	/**
	 * Internal handler for enabled->disabled transitions. Resets all security
	 * keys forcing EAS to resync security state.
	 */
	/* package */void onAdminEnabled(boolean isEnabled) {
		if (!isEnabled) {
			Log.v("Policyyyyyyyyyyyyy", "removeddddddddddd");
			// transition to disabled state
			// Response: clear *all* security state information from the
			// accounts, forcing
			// them back to the initial configurations requiring policy
			// administration
			boolean isAdminDisabledAfterWipe = new SharedPreferences(mContext)
			.getBoolean("AdminDisabledAfterWipe", false);
			TruBoxDatabase.resetAllValues(mContext);
			ContentValues cv = new ContentValues();
			cv.put(AccountColumns.SECURITY_FLAGS, 0);
			cv.putNull(AccountColumns.SECURITY_SYNC_KEY);
			mContext.getContentResolver().update(Account.CONTENT_URI, cv, null,
					null);
			updatePolicies(-1);
			long accountId = EmEmailContent.Account.getDefaultAccountId(mContext);
			if(accountId > -1) {
    		EmEmailContent.Account account = EmEmailContent.Account.restoreAccountWithId(mContext, accountId);
			if (account != null) {
				PersonaSecurityProfileUpdate.emailDataWipe(mContext, account.toString(), true);
			}
			}
		
		//	removeAccount();
		
			PersonaSecurityProfileUpdate.deleteLocalData(mContext);
			PersonaSecurityProfileUpdate.killAllActivitiesInStack(mContext);
			TruMobiTimerClass.userInteractedStopTimer();
			ExternalAdapterRegistrationClass.setInstance();
			if (!isAdminDisabledAfterWipe) {
	...
	private void removeAccount() // NaGa
	{
		Cursor c = mContext.getContentResolver().query(Account.CONTENT_URI,
				ACCOUNT_SECURITY_PROJECTION_EAS, WHERE_ACCOUNT_SECURITY_ZERO,
				null, null);
		// Log.v("ffffffffffffffffffff", "CCCCCCCCCCCCCCCC "+c.getCount());
		if (c != null) {
			try {
				while (c.moveToNext()) {
					...
					Uri uri = ContentUris.withAppendedId(
							...
							c.getLong(ACCOUNT_FLAGS_COLUMN_ID));
					String email = c.getString(ACCOUNT_FLAGS_COLUMN_EMAIL);
					//Log.v("emailemailemailemail", "emailemail " + email);
					{
						mContext.getContentResolver().delete(uri, null, null);
						EmAccountBackupRestore.backupAccounts(mContext);
					}
				}
	...
	/**
	 * Device Policy administrator. This is primarily a listener for device
	 * state changes. Note: This is instantiated by incoming messages. Note: We
	 * do not implement onPasswordFailed() because the default behavior of the
	 * DevicePolicyManager - complete local wipe after 'n' failures - is
	 * sufficient.
	 */
	public static class PolicyAdmin extends DeviceAdminReceiver {
		...
		public void onEnabled(Context context, Intent intent) {
			EmSecurityPolicy.getInstance(context).onAdminEnabled(true);
		}
		...
		public void onDisabled(Context context, Intent intent) {
			EmSecurityPolicy.getInstance(context).setContext(context);
			EmSecurityPolicy.getInstance(context).onAdminEnabled(false);
		}
		...
		public void onPasswordChanged(Context context, Intent intent) {
			EmSecurityPolicy.getInstance(context).clearAccountHoldFlags();
		}
	}
}
