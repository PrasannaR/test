...
import com.cognizant.trumobi.em.mail.transport.EmMailTransport;
...
public class EmPop3Store extends EmStore {
    ...
    private static final EmFlag[] PERMANENT_FLAGS = { EmFlag.DELETED };

    private EmTransport mTransport;
    private String mUsername;
    private String mPassword;
    private final HashMap<String, EmFolder> mFolders = new HashMap<String, EmFolder>();
...
//    private int mThroughputKbS = Integer.MAX_VALUE;
     ...
     * @param _uri
     */
    private EmPop3Store(String _uri) throws EmMessagingException {
        URI uri;
        try {
            uri = new URI(_uri);
        } catch (URISyntaxException use) {
            throw new EmMessagingException("Invalid Pop3Store URI", use);
        }
        ...
        mTransport = new EmMailTransport("POP3");
    ...
    public EmFolder getFolder(String name) throws EmMessagingException {
        EmFolder folder = mFolders.get(name);
        if (folder == null) {
     ...
     *
     * @throws EmMessagingException if there was some problem with the account
     */
    @Override
    ...
    class Pop3Folder extends EmFolder {
         ...
         *
         * @throws EmMessagingException if the account is not going to be useable
         */
        public void checkSettings() throws EmMessagingException {
            if (!mCapabilities.uidl) {
                try {
                    ...
                    mTransport.close();
                    throw new EmMessagingException(null, ioe);
                }
            }
        }
        ...
        public synchronized void open(OpenMode mode, PersistentDataCallbacks callbacks)
            ...
            if (!mName.equalsIgnoreCase("INBOX")) {
                throw new EmMessagingException("Folder does not exist");
            }
            ...
            try {
                ...
                try {
                    ...
                    throw new EmAuthenticationFailedException(null, me);
                }
            } catch (IOException ioe) {
                mTransport.close();
                if (Config.LOGD && Email.DEBUG) {
            ...
            if (statException != null) {
                ...
                throw new EmMessagingException("POP3 STAT", statException);
            }
            mUidToMsgMap.clear();
        ...
        public EmMessage getMessage(String uid) throws EmMessagingException {
            if (mUidToMsgNumMap.size() == 0) {
                try {
                    ...
                    throw new EmMessagingException("getMessages", ioe);
                }
            }
        ...
        public EmMessage[] getMessages(int start, int end, MessageRetrievalListener listener)
            ...
            if (start < 1 || end < 1 || end < start) {
                throw new EmMessagingException(String.format("Invalid message set %d %d",
                        start, end));
            }
            try {
            ...
            return messages.toArray(new EmMessage[messages.size()]);
        }
        ...
        private void indexMsgNums(int start, int end)
                throws EmMessagingException, IOException {
        ...
        public void fetch(EmMessage[] messages, EmFetchProfile fp, MessageRetrievalListener listener)
            ...
            try {
                indexUids(uids);
                if (fp.contains(EmFetchProfile.Item.ENVELOPE)) {
                ...
                throw new EmMessagingException("fetch", ioe);
            }
            for (int i = 0, count = messages.length; i < count; i++) {
                EmMessage message = messages[i];
                if (!(message instanceof Pop3Message)) {
                    throw new EmMessagingException("Pop3Store.fetch called with non-Pop3 Message");
                }
                ...
                try {
                    ...
                    else if (fp.contains(EmFetchProfile.Item.BODY_SANE)) {
                        /*
                    ...
                    else if (fp.contains(EmFetchProfile.Item.STRUCTURE)) {
                        /*
                    ...
                    throw new EmMessagingException("Unable to fetch message", ioe);
                }
            }
        }

        private void fetchEnvelope(EmMessage[] messages,
                MessageRetrievalListener listener)  throws IOException, EmMessagingException {
        ...
        public void setFlags(EmMessage[] messages, EmFlag[] flags, boolean value)
                throws EmMessagingException {
            if (!value || !EmUtility.arrayContains(flags, EmFlag.DELETED)) {
                /*
            ...
            try {
                ...
                throw new EmMessagingException("setFlags()", ioe);
            }
        }
...
//        }
        ...
        private Pop3Capabilities getCapabilities() throws IOException, EmMessagingException {
        ...
        private String executeSensitiveCommand(String command, String sensitiveReplacement)
            ...
            if (response.length() > 1 && response.charAt(0) == '-') {
                throw new EmMessagingException(response);
            }
    ...
    class Pop3Message extends EmMimeMessage {
        public Pop3Message(String uid, Pop3Folder folder) throws EmMessagingException {
            mUid = uid;
            mFolder = folder;
            mSize = -1;
        }
        ...
        public void setFlag(EmFlag flag, boolean set) throws EmMessagingException {
            ...
            mFolder.setFlags(new EmMessage[] { this }, new EmFlag[] { flag }, set);
        }
    }
