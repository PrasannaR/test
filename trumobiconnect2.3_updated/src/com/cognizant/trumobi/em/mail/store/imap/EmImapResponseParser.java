...
import com.cognizant.trumobi.em.EmPeekableInputStream;
import com.cognizant.trumobi.em.mail.EmMessagingException;
import com.cognizant.trumobi.em.mail.transport.EmDiscourseLogger;
import com.cognizant.trumobi.em.mail.transport.EmLoggingInputStream;
 ...
 */
public class EmImapResponseParser {
    ...
    /** Input stream */
    private final EmPeekableInputStream mIn;
     ...
     */
    private final EmDiscourseLogger mDiscourseLogger;
    ...
    /**
     * We store all {@link EmImapResponse} in it.  {@link #destroyResponses()} must be called from
     * time to time to destroy them and clear it.
     */
    private final ArrayList<EmImapResponse> mResponsesToDestroy = new ArrayList<EmImapResponse>();
     ...
     */
    public EmImapResponseParser(InputStream in, EmDiscourseLogger discourseLogger) {
     ...
     */
    EmImapResponseParser(InputStream in, EmDiscourseLogger discourseLogger,
            int literalKeepInMemoryThreshold) {
        if (DEBUG_LOG_RAW_STREAM && Config.LOGD && Email.DEBUG) {
            in = new EmLoggingInputStream(in);
        }
        mIn = new EmPeekableInputStream(in);
    ...
    /**
     * Destroy all the {@link EmImapResponse}s stored in the internal storage and clear it.
     *
     * @see #readResponse()
     */
    public void destroyResponses() {
        for (EmImapResponse r : mResponsesToDestroy) {
            r.destroy();
        }
     ...
     * {@link EmImapResponse} object that represents it.
     *
     * <p>When this method successfully returns an {@link EmImapResponse}, the {@link EmImapResponse}
     * is stored in the internal storage.  When the {@link EmImapResponse} is no longer used
     * {@link #destroyResponses} should be called to destroy all the responses in the array.
     *
     * @return the parsed {@link EmImapResponse} object.
     * @exception ByeException when detects BYE.
     */
    public EmImapResponse readResponse() throws IOException, EmMessagingException {
        EmImapResponse response = null;
        try {
        ...
        if (response.is(0, EmImapConstants.BYE)) {
     ...
     * throw {@link EmMessagingException}.
     */
     ...
     */
    /* package */ String readUntilEol() throws IOException, EmMessagingException {
     ...
     */
    private EmImapResponse parseResponse() throws IOException, EmMessagingException {
        ...
        EmImapResponse responseToDestroy = null;
        final EmImapResponse responseToReturn;
        ...
        try {
            ...
            if (ch == '+') { // Continuation request
                readByte(); // skip +
                expect(' ');
                responseToDestroy = new EmImapResponse(null, true);

                // If it's continuation request, we don't really care what's in it.
                responseToDestroy.add(new EmImapSimpleString(readUntilEol()));
                ...
                responseToDestroy = new EmImapResponse(tag, false);

                final EmImapString firstString = parseBareString();
                ...
                if (peek() == ' ') {
                    ...
                    if (responseToDestroy.isStatusResponse()) { // It's a status response
                        ...
                        if (!TextUtils.isEmpty(rest)) {
                            ...
                            responseToDestroy.add(new EmImapSimpleString(rest));
                        }
    ...
    private EmImapElement parseElement() throws IOException, EmMessagingException {
        final int next = peek();
        switch (next) {
            case '(':
                return parseList('(', ')');
            ...
            case '"':
                readByte(); // Skip "
                return new EmImapSimpleString(readUntil('"'));
            case '{':
                return parseLiteral();
            case '\r':  // CR
                readByte(); // Consume \r
     ...
     */
    private EmImapString parseBareString() throws IOException, EmMessagingException {
        ...
        for (;;) {
            ...
            if (ch == '(' || ch == ')' || ch == '{' || ch == ' ' ||
                ...
                if (mParseBareString.length() == 0) {
                    throw new EmMessagingException("Expected string, none found.");
                }
                String s = mParseBareString.toString();

                // NIL will be always converted into the empty string.
                if (EmImapConstants.NIL.equalsIgnoreCase(s)) {
                    return EmImapString.EMPTY;
                }
                return new EmImapSimpleString(s);
            } else if (ch == '[') {
                // Eat all until next ']'
    ...
    private void parseElements(EmImapList list, char end)
        ...
        for (;;) {
            ...
            final EmImapElement el = parseElement();
            if (el == null) { // EOL
                return;
            }
            list.add(el);
    ...
    private EmImapList parseList(char opening, char closing)
            throws IOException, EmMessagingException {
        expect(opening);
        final EmImapList list = new EmImapList();
        parseElements(list, closing);
        expect(closing);
        return list;
    }

    private EmImapString parseLiteral() throws IOException, EmMessagingException {
        expect('{');
        final int size;
        try {
            size = Integer.parseInt(readUntil('}'));
        } catch (NumberFormatException nfe) {
            throw new EmMessagingException("Invalid length in literal");
        }
        expect('\r');
        expect('\n');
        EmFixedLengthInputStream in = new EmFixedLengthInputStream(mIn, size);
        if (size > mLiteralKeepInMemoryThreshold) {
            return new EmImapTempFileLiteral(in);
        } else {
            return new EmImapMemoryLiteral(in);
        }
    }
}
