...
public class EmRfc822Output {
    ...
    public static void writeTo(Context context, Message message, OutputStream out,
            boolean appendQuotedText, boolean sendBcc) throws IOException, EmMessagingException {     
        if (message == null) {
            // throw something?
            return;
        }
        ...
        Uri uri = ContentUris.withAppendedId(Attachment.MESSAGE_ID_URI, message.mId);
        ...
        try {
            ...
            if (!multipart) {
                ...
                multipartBoundary = "--_com.cognizant.trumobi.em_" + System.nanoTime();
    ...
    private static void writeOneAttachment(Context context, Writer writer, OutputStream out,
            Attachment attachment) throws IOException, EmMessagingException {
        writeHeader(writer, "Content-Type",
    ...
    private static void writeEncodedHeader(Writer writer, String name, String value)
        ...
        if (value != null && value.length() > 0) {
            writer.append(name);
            writer.append(": ");
            writer.append(EmMimeUtility.foldAndEncode2(value, name.length() + 2));
            writer.append("\r\n");
        }
    }
    ...
    private static void writeAddressHeader(Writer writer, String name, String value)
        ...
        if (value != null && value.length() > 0) {
            writer.append(name);
            writer.append(": ");
            writer.append(EmMimeUtility.fold(EmAddress.packedToHeader(value), name.length() + 2));
            writer.append("\r\n");
        }
    }
