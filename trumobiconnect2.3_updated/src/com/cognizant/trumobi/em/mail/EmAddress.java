...
public class EmAddress {
    ...
    private static final EmAddress[] EMPTY_ADDRESS_ARRAY = new EmAddress[0];
    ...
    public EmAddress(String address, String personal) {
        setAddress(address);
        setPersonal(personal);
    }

    public EmAddress(String address) {
        setAddress(address);
    }
     ...
     */
    public static EmAddress[] parse(String addressList) {
        if (addressList == null || addressList.length() == 0) {
            return EMPTY_ADDRESS_ARRAY;
        }
        Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(addressList);
        ArrayList<EmAddress> addresses = new ArrayList<EmAddress>();
        for (int i = 0, length = tokens.length; i < length; ++i) {
            Rfc822Token token = tokens[i];
            String address = token.getAddress();
            if (!TextUtils.isEmpty(address)) {
                if (isValidAddress(address)) {
                    ...
                    addresses.add(new EmAddress(address, name));
                }
            }
        }
        return addresses.toArray(new EmAddress[] {});
    }
    ...
    public boolean equals(Object o) {
        if (o instanceof EmAddress) {
            ...
            return getAddress().equals(((EmAddress) o).getAddress());
        }
    ...
    public String toString() {
        if (mPersonal != null) {
            if (mPersonal.matches(".*[\\(\\)<>@,;:\\\\\".\\[\\]].*")) {
                return EmUtility.quoteString(mPersonal) + " <" + mAddress + ">";
            } else {
     ...
     */
    public static String toString(EmAddress[] addresses) {
     ...
     */
    public static String toHeader(EmAddress[] addresses) {
        if (addresses == null || addresses.length == 0) {
            return null;
        }
        if (addresses.length == 1) {
     ...
     * was null or empty.
     */
    public static String toFriendly(EmAddress[] addresses) {
        if (addresses == null || addresses.length == 0) {
            return null;
        }
        if (addresses.length == 1) {
    ...
    public static String parseAndPack(String textList) {
        return EmAddress.pack(EmAddress.parse(textList));
    }
     ...
     */
    public static EmAddress unpackFirst(String packedList) {
        EmAddress[] array = unpack(packedList);
     ...
     */
    public static EmAddress[] unpack(String addressList) {
        if (addressList == null || addressList.length() == 0) {
            return EMPTY_ADDRESS_ARRAY;
        }
        ArrayList<EmAddress> addresses = new ArrayList<EmAddress>();
        ...
        */
        ...
        while (pairStartIndex < length) {
            ...
            if (addressEndIndex == -1 || pairEndIndex <= addressEndIndex) {
                ...
                address = new EmAddress(addressList.substring(pairStartIndex, pairEndIndex), null);
            } else {
                address = new EmAddress(addressList.substring(pairStartIndex, addressEndIndex),
     ...
     */
    public static String pack(EmAddress[] addresses) {
        ...
        for (int i = 0; i < nAddr; i++) {
            ...
            final EmAddress address = addresses[i];
     ...
     * @See unpack()
     */
    public static EmAddress[] legacyUnpack(String addressList) {
        if (addressList == null || addressList.length() == 0) {
            return new EmAddress[] { };
        }
        ArrayList<EmAddress> addresses = new ArrayList<EmAddress>();
        int length = addressList.length();
        int pairStartIndex = 0;
        ...
        while (pairStartIndex < length) {
            ...
            if (addressEndIndex == -1 || addressEndIndex > pairEndIndex) {
                address =
                    EmUtility.fastUrlDecode(addressList.substring(pairStartIndex, pairEndIndex));
            }
            else {
                address =
                    EmUtility.fastUrlDecode(addressList.substring(pairStartIndex, addressEndIndex));
                personal = 
                    EmUtility.fastUrlDecode(addressList.substring(addressEndIndex + 1, pairEndIndex));
            }
            addresses.add(new EmAddress(address, personal));
            pairStartIndex = pairEndIndex + 1;
        }
        return addresses.toArray(new EmAddress[] { });
    }
     ...
     * @See unpack()
     */
    public static String legacyPack(EmAddress[] addresses) {
        if (addresses == null) {
            return null;
