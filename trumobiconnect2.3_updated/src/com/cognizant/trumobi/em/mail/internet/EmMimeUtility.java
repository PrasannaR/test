...
public class EmMimeUtility {
    ...
    public static EmPart findFirstPartByMimeType(EmPart part, String mimeType)
            throws EmMessagingException {
        if (part.getBody() instanceof EmMultipart) {
            EmMultipart multipart = (EmMultipart)part.getBody();
            for (int i = 0, count = multipart.getCount(); i < count; i++) {
                EmBodyPart bodyPart = multipart.getBodyPart(i);
                EmPart ret = findFirstPartByMimeType(bodyPart, mimeType);
                if (ret != null) {
                    return ret;
                }
            }
    ...
    public static EmPart findPartByContentId(EmPart part, String contentId) throws Exception {
        if (part.getBody() instanceof EmMultipart) {
            EmMultipart multipart = (EmMultipart)part.getBody();
            for (int i = 0, count = multipart.getCount(); i < count; i++) {
                EmBodyPart bodyPart = multipart.getBodyPart(i);
                EmPart ret = findPartByContentId(bodyPart, contentId);
                if (ret != null) {
                    return ret;
                }
            }
     ...
     */
    public static String getTextFromPart(EmPart part) {
        try {
            if (part != null && part.getBody() != null) {
                InputStream in = part.getBody().getInputStream();
                String mimeType = part.getMimeType();
                if (mimeType != null && EmMimeUtility.mimeTypeMatches(mimeType, "text/*")) {
                    /*
     ...
     */
    public static EmBody decodeBody(InputStream in, String contentTransferEncoding)
            throws IOException {
        /*
        ...
        if (contentTransferEncoding != null) {
            contentTransferEncoding =
                EmMimeUtility.getHeaderParameter(contentTransferEncoding, null);
        ...
        EmBinaryTempFileBody tempBody = new EmBinaryTempFileBody();
     ...
     * @throws EmMessagingException
     */
    public static void collectParts(EmPart part, ArrayList<EmPart> viewables,
            ArrayList<EmPart> attachments) throws EmMessagingException {
        String disposition = part.getDisposition();
        String dispositionType = null;
        String dispositionFilename = null;
        if (disposition != null) {
            dispositionType = EmMimeUtility.getHeaderParameter(disposition, null);
            dispositionFilename = EmMimeUtility.getHeaderParameter(disposition, "filename");
        }
         ...
         */
        else if (part.getBody() instanceof EmMessage) {
            EmMessage message = (EmMessage)part.getBody();
