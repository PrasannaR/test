 ...
 * based code.
 */
public class EmEmController {

    private static EmEmController sInstance;
    private final Context mContext;
    private Context mProviderContext;
    private final EmMessagingController mLegacyController;
    ...
    protected EmEmController(Context _context) {
        mContext = _context;
        mProviderContext = _context;
        mLegacyController = EmMessagingController.getInstance(mContext);
     ...
     */
    public synchronized static EmEmController getInstance(Context _context) {
        if (sInstance == null) {
            sInstance = new EmEmController(_context);
        }
        return sInstance;
    }
    ...
    public void serviceLogging(int debugEnabled) {
        IEmailService service = EmExchangeUtils.getExchangeEmailService(mContext, mServiceCallback);
        try {
...
//            }
        ...
        if (service != null) {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    Account account =
                        EmEmailContent.Account.restoreAccountWithId(mProviderContext, accountId);
                    Mailbox mailbox =
                        EmEmailContent.Mailbox.restoreMailboxWithId(mProviderContext, mailboxId);
     ...
     */
    public void saveToMailbox(final EmEmailContent.Message message, final int mailboxType) {
    ...
    /* package */ long createMailbox(long accountId, int mailboxType) {
        ...
        box.mSyncInterval = EmEmailContent.Account.CHECK_INTERVAL_NEVER;
    ...
    public void sendMessage(long messageId, long accountId) {
        ...
        ContentValues cv = new ContentValues();
        cv.put(EmEmailContent.MessageColumns.MAILBOX_KEY, outboxId);

        // does this need to be SYNCED_CONTENT_URI instead?
        Uri uri = ContentUris.withAppendedId(EmEmailContent.Message.CONTENT_URI, messageId);
	...
	public boolean setOofSettings(Context context, long accountId, boolean enable,	//Oof
			String replyMessage, String startDate, String endDate) throws RemoteException {
		
		IEmailService service = getServiceForAccount(accountId);
		
		if (service != null) {
			return service.sendOofSettings(accountId, enable, replyMessage, startDate, endDate);
		}
		return false;
//		EmOofSettings.sendOofSettings(context, accountId, enable, replyMessage,
//				startDate, endDate);
	}
	public boolean getOofSettings(long accountId, Context context) throws RemoteException {
		
		IEmailService service = getServiceForAccount(accountId);
		
		if (service != null) {
			return service.getOofSettings(accountId);
		}
		return false;
//		EmOofSettings.sendOofSettings(context, accountId, enable, replyMessage,
//				startDate, endDate);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if (info != null && info.mVisibleLimitIncrement > 0) {
                    ...
                    ContentValues cv = new ContentValues();;
                    cv.put(EmEmailContent.FIELD_COLUMN_NAME, MailboxColumns.VISIBLE_LIMIT);
                    cv.put(EmEmailContent.ADD_COLUMN_NAME, info.mVisibleLimitIncrement);
    ...
    private long lookupAccountForMessage(long messageId) {
        ...
        Cursor c = resolver.query(EmEmailContent.Message.CONTENT_URI,
                                  MESSAGEID_TO_ACCOUNTID_PROJECTION, EmEmailContent.RECORD_ID + "=?",
    ...
    public void deleteMessage(long messageId, long accountId) {
        ...
        Cursor c = resolver.query(EmEmailContent.Message.CONTENT_URI,
                MESSAGEID_TO_MAILBOXID_PROJECTION, EmEmailContent.RECORD_ID + "=?",
                new String[] { Long.toString(messageId) }, null);
        ...
        EmAttachmentProvider.deleteAllAttachmentFiles(mProviderContext, accountId, messageId);

        Uri uri = ContentUris.withAppendedId(EmEmailContent.Message.SYNCED_CONTENT_URI, messageId);
        ...
        if (sourceMailboxId == trashMailboxId) {
            ...
            // 5b. Move to trash
            ContentValues cv = new ContentValues();
            cv.put(EmEmailContent.MessageColumns.MAILBOX_KEY, trashMailboxId);
            resolver.update(uri, cv, null, null);
        }
    ...
    public void setMessageRead(final long messageId, boolean isRead) {
        ContentValues cv = new ContentValues();
        cv.put(EmEmailContent.MessageColumns.FLAG_READ, isRead);
        Uri uri = ContentUris.withAppendedId(
                EmEmailContent.Message.SYNCED_CONTENT_URI, messageId);
        mProviderContext.getContentResolver().update(uri, cv, null, null);
    ...
    public void setMessageFavorite(final long messageId, boolean isFavorite) {
        ContentValues cv = new ContentValues();
        cv.put(EmEmailContent.MessageColumns.FLAG_FAVORITE, isFavorite);
        Uri uri = ContentUris.withAppendedId(
                EmEmailContent.Message.SYNCED_CONTENT_URI, messageId);
        mProviderContext.getContentResolver().update(uri, cv, null, null);
    ...
    public void loadAttachment(final long attachmentId, final long messageId, final long mailboxId,
        ...
        File saveToFile = EmAttachmentProvider.getAttachmentFilename(mProviderContext,
                accountId, attachmentId);
        Attachment attachInfo = Attachment.restoreAttachmentWithId(mProviderContext, attachmentId);
        if (attachInfo == null) {
            return;
        }
        Log.i("EMMEMEMEM","-------->  "+attachmentId+"  "+messageId+"   "+mailboxId+"   "+accountId);
Log.i("NEW","-------->  "+(saveToFile.exists() && attachInfo.mContentUri != null));
        if (saveToFile.exists() && attachInfo.mContentUri != null) {
        	...
        	Log.i("NEW","-------->  "+attachInfo.mContentUri);
            synchronized (mListeners) {
                for (Result listener : mListeners) {
        ...
        if (service != null) {
            ...
            try {
                service.loadAttachment(attachInfo.mId, saveToFile.getAbsolutePath(),
                        EmAttachmentProvider.getAttachmentUri(accountId, attachmentId).toString());
            } catch (RemoteException e) {
                // TODO Change exception handling to be consistent with however this method
                // is implemented for other protocols
                Log.e("onDownloadAttachment", "RemoteException", e);
    ...
    private IEmailService getServiceForAccount(long accountId) {
        ...
        Account account = EmEmailContent.Account.restoreAccountWithId(mProviderContext, accountId);
        if (account == null || isMessagingController(account)) {
            return null;
        } else {
            return EmExchangeUtils.getExchangeEmailService(mContext, mServiceCallback);
        }
    }
     ...
     * TODO this should use a cache because we'll be doing this a lot
     */
    public boolean isMessagingController(EmEmailContent.Account account) {
        if (account == null) return false;
        EmStore.StoreInfo info =
            EmStore.StoreInfo.getStoreInfo(account.getStoreUri(mProviderContext), mContext);
    ...
    public interface Result {
         ...
         */
        public void sendMailCallback(EmMessagingException result, long accountId,
     ...
     * out of scope.
     */
    private class LegacyListener extends EmMessagingListener {
        ...
        public void listFoldersFailed(long accountId, String message) {
            synchronized (mListeners) {
                for (Result l : mListeners) {
                    l.updateMailboxListCallback(new EmMessagingException(message), accountId, 0);
                }
            }
        }
        ...
        public void synchronizeMailboxFailed(long accountId, long mailboxId, Exception e) {
            EmMessagingException me;
            if (e instanceof EmMessagingException) {
                me = (EmMessagingException) e;
            } else {
                me = new EmMessagingException(e.toString());
            }
            synchronized (mListeners) {
                for (Result l : mListeners) {
        ...
        public void loadMessageForViewFailed(long messageId, String message) {
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    listener.loadMessageForViewCallback(new EmMessagingException(message),
                            messageId, 0);
                }
            }
        }
        ...
        public void loadAttachmentFailed(long accountId, long messageId, long attachmentId,
            ...
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    listener.loadAttachmentCallback(new EmMessagingException(reason),
        ...
        synchronized public void sendPendingMessagesFailed(long accountId, long messageId,
                Exception reason) {
            EmMessagingException me;
            if (reason instanceof EmMessagingException) {
                me = (EmMessagingException) reason;
            } else {
                me = new EmMessagingException(reason.toString());
            }
            synchronized (mListeners) {
                for (Result listener : mListeners) {
    ...
    private class ServiceCallback extends IEmailServiceCallback.Stub {
        ...
        public void loadAttachmentStatus(long messageId, long attachmentId, int statusCode,
                int progress) {
            EmMessagingException result = mapStatusToException(statusCode);
            switch (statusCode) {
                case EmEmailServiceStatus.SUCCESS:
                    progress = 100;
                    break;
                case EmEmailServiceStatus.IN_PROGRESS:
                    if (DEBUG_FAIL_DOWNLOADS && progress > 75) {
                        result = new EmMessagingException(
                                String.valueOf(EmEmailServiceStatus.CONNECTION_ERROR));
                    }
...
//                    + " progress=" + progress);
        ...
        private EmMessagingException mapStatusToException(int statusCode) {
            switch (statusCode) {
                case EmEmailServiceStatus.SUCCESS:
                case EmEmailServiceStatus.IN_PROGRESS:
                    return null;

                case EmEmailServiceStatus.LOGIN_FAILED:
                    return new EmAuthenticationFailedException("");

                case EmEmailServiceStatus.CONNECTION_ERROR:
                    return new EmMessagingException(EmMessagingException.IOERROR);

                case EmEmailServiceStatus.SECURITY_FAILURE:
                	Log.v("SECURITY_FAILURE", "SECURITY_FAILURESECURITY_FAILURE");
                    return new EmMessagingException(EmMessagingException.SECURITY_POLICIES_REQUIRED);

                case EmEmailServiceStatus.MESSAGE_NOT_FOUND:
                case EmEmailServiceStatus.ATTACHMENT_NOT_FOUND:
                case EmEmailServiceStatus.FOLDER_NOT_DELETED:
                case EmEmailServiceStatus.FOLDER_NOT_RENAMED:
                case EmEmailServiceStatus.FOLDER_NOT_CREATED:
                case EmEmailServiceStatus.REMOTE_EXCEPTION:
                    // TODO: define exception code(s) & UI string(s) for server-side errors
                default:
                    return new EmMessagingException(String.valueOf(statusCode));
            }
        }
    }
}
