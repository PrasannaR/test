...
public class EmUtility {
	...
	public final static String readInputStream(InputStream in, String encoding)
			throws IOException {
	 ...
	 * 
	 * TODO: Rename this, because "quoteString()" can mean so many different
	 * things.
	 * 
	...
	// TODO: unit test this
	public static String buildMailboxIdSelection(ContentResolver resolver,
			long mailboxId) {
		// Setup default selection & args, then add to it as necessary
		StringBuilder selection = new StringBuilder(MessageColumns.FLAG_LOADED
				+ " IN (" + Message.FLAG_LOADED_PARTIAL + ","
				+ Message.FLAG_LOADED_COMPLETE + ") AND ");
		if (mailboxId == Mailbox.QUERY_ALL_INBOXES
			...
			Cursor c = resolver.query(Mailbox.CONTENT_URI,
					EmEmailContent.ID_PROJECTION, MailboxColumns.TYPE
			...
			if(c != null) {
				while (c.moveToNext()) {
					...
					inboxes.append(c.getLong(EmEmailContent.ID_PROJECTION_COLUMN));
				}
				c.close();
			}
		...
		private FolderProperties(Context context) {
			...
			for (int i = 0; i < mSpecialMailbox.length; ++i) {
				if ("".equals(mSpecialMailbox[i])) {
					// there is no localized name, so use the display name from
					// the server
					mSpecialMailbox[i] = null;
				}
			}
			mSpecialMailboxDrawable = context.getResources().obtainTypedArray(
					R.array.mailbox_display_icons);
			mDefaultMailboxDrawable = context.getResources().getDrawable(
					R.drawable.em_ic_list_folder);
			mSummaryStarredMailboxDrawable = context.getResources()
					.getDrawable(R.drawable.em_ic_menu_star_holo_light_list); // NaGa
			mSummaryCombinedInboxDrawable = context.getResources().getDrawable(
					R.drawable.em_ic_list_combined_inbox);
		}
	 ...
	 *            this account Id will not trigger (when editing an existing
	 *            account)
	 * @param hostName
	 *            the server
	 * @param userLogin
	 ...
	 */
	public static String findDuplicateAccount(Context context,
			long allowAccountId, String hostName, String userLogin) {
		ContentResolver resolver = context.getContentResolver();
		Cursor c = resolver.query(HostAuth.CONTENT_URI, HostAuth.ID_PROJECTION,
				HOSTAUTH_WHERE_CREDENTIALS,
				new String[] { hostName, userLogin }, null);
		if(c != null) {
			try {
				while (c.moveToNext()) {
					...
					// display name
					Cursor c2 = resolver.query(Account.CONTENT_URI,
							Account.ID_PROJECTION, ACCOUNT_WHERE_HOSTAUTH,
							new String[] { Long.toString(hostAuthId) }, null);
					if(c2 != null) {
						try {
							while (c2.moveToNext()) {
								long accountId = c2
										.getLong(Account.ID_PROJECTION_COLUMN);
	 ...
	 * 
	 * @param DateTime
	 *            date string in format 20090211T180303Z (rfc2445, iCalendar).
	 ...
	 * 
	 * @param Email
	 *            style DateTime string in format 2010-02-23T16:00:00.000Z (ISO
	 *            8601, rfc3339)
	 * @return the time in milliseconds (since Jan 1, 1970)
	 */
	public static long parseEmailDateTimeToMillis(String date) {
		GregorianCalendar cal = new GregorianCalendar(Integer.parseInt(date
				...
				Integer.parseInt(date.substring(8, 10)), Integer.parseInt(date
						.substring(11, 13)), Integer.parseInt(date.substring(
						14, 16)), Integer.parseInt(date.substring(17, 19)));
		cal.setTimeZone(TimeZone.getTimeZone("GMT"));
		return cal.getTimeInMillis();
	}
	...
	public static String getConsistentDeviceId(Context context) {
		...
		try {
			...
			Log.d(Email.LOG_TAG, "Error in TelephonyManager.getDeviceId(): "
					+ e.getMessage());
			return null;
		}
		...
		sha.update(EmUtility.toUtf8(deviceId));
	 ...
	 * 
	 * It fixes: - "Thu, 10 Dec 09 15:08:08 GMT-0700" to
	 * "Thu, 10 Dec 09 15:08:08 -0700" (4 digit zone value can't be preceded by
	 * "GMT") We got a report saying eBay sends a date in this format
	 */
	public static String cleanUpMimeDate(String date) {
		if (TextUtils.isEmpty(date)) {
			return date;
		}
		date = DATE_CLEANUP_PATTERN_WRONG_TIMEZONE.matcher(date).replaceFirst(
				"$1");
		return date;
	}
