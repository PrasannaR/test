...
public class EmEmailServiceProxy implements IEmailService {
    ...
    private Object mReturn = null, mResult = null;
    // Service call timeout (in seconds)
    private int mTimeout = 45;
    private boolean mDead = false;

    public EmEmailServiceProxy(Context _context, Class<?> _class) {
        this(_context, _class, null);
    }

    public EmEmailServiceProxy(Context _context, Class<?> _class, IEmailServiceCallback _callback) {
        mContext = _context;
        mClass = _class;
        mCallback = _callback;
    ...
    public EmEmailServiceProxy setTimeout(int secs) {
        mTimeout = secs;
        return this;
    }
	...
	@Override
	public boolean sendOofSettings(final long accountId, final boolean enable,
			final String replyMessage, final String startDate,
			final String endDate) throws RemoteException {
		// TODO Auto-generated method stub
		...
		setTask(new Runnable() {
			public void run() {
				try {
					...
					mResult = mService.sendOofSettings(accountId, enable, replyMessage,
							startDate, endDate);
				} catch (RemoteException e) {
				}
			}
		...
		else return (Boolean)mResult;
	}

	@Override
	public boolean getOofSettings(final long accountId) throws RemoteException {
		// TODO Auto-generated method stub
		...
		setTask(new Runnable() {
			public void run() {
				try {
					if (mCallback != null)
						mService.setCallback(mCallback);
					mResult = mService.getOofSettings(accountId);
				} catch (RemoteException e) {
				}
			}
		});
