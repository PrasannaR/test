...
import com.cognizant.trumobi.em.EmNotificationService;
 ...
 */
public class EmMailService extends Service {
    ...
    public static final int NOTIFICATION_ID_LOGIN_FAILED = 4;	//PWD_CHANGE_IMPL

    private static final String ACTION_CHECK_MAIL =
        "com.cognizant.trumobi.em.intent.action.MAIL_SERVICE_WAKEUP";
    private static final String ACTION_RESCHEDULE =
        "com.cognizant.trumobi.em.intent.action.MAIL_SERVICE_RESCHEDULE";
    private static final String ACTION_CANCEL =
        "com.cognizant.trumobi.em.intent.action.MAIL_SERVICE_CANCEL";
    private static final String ACTION_NOTIFY_MAIL =
        "com.cognizant.trumobi.em.intent.action.MAIL_SERVICE_NOTIFY";
    
    private static final String ACTION_NOTIFY_LOGIN_FAILED =
            "com.cognizant.trumobi.em.intent.action.LOGIN_FAIL_SERVICE_NOTIFY";	//PWD_CHANGE_IMPL

    private static final String EXTRA_CHECK_ACCOUNT = "com.cognizant.trumobi.em.intent.extra.ACCOUNT";
    private static final String EXTRA_ACCOUNT_INFO = "com.cognizant.trumobi.em.intent.extra.ACCOUNT_INFO";
    private static final String EXTRA_DEBUG_WATCHDOG = "com.cognizant.trumobi.em.intent.extra.WATCHDOG";
    ...
    private final EmEmController.Result mControllerCallback = new ControllerResults();
    ...
    public static void actionReschedule(Context context) {
        Intent i = new Intent();
        i.setClass(context, EmMailService.class);
        i.setAction(EmMailService.ACTION_RESCHEDULE);
    ...
    public static void actionCancel(Context context)  {
        Intent i = new Intent();
        i.setClass(context, EmMailService.class);
        i.setAction(EmMailService.ACTION_CANCEL);
        context.startService(i);
    }
    ...
    //PWD_CHANGE_IMPL, -->
    public static void actionNotifyLoginFailed(Context context, long accountId) { 
        Intent i = new Intent(ACTION_NOTIFY_LOGIN_FAILED);
        i.setClass(context, EmMailService.class);
        i.putExtra(EXTRA_CHECK_ACCOUNT, accountId);
        context.startService(i);
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //PWD_CHANGE_IMPL, -->
        else if(ACTION_NOTIFY_LOGIN_FAILED.equals(action)) {
        	long accountId = intent.getLongExtra(EXTRA_CHECK_ACCOUNT, -1);
        	notifyLoginFail(accountId);
        	stopSelf(startId);
        }
        //PWD_CHANGE_IMPL, <--
    ...
    /* package */ PendingIntent createAlarmIntent(long checkId, long[] accountInfo,
            boolean isWatchdog) {
        Intent i = new Intent();
        i.setClass(this, EmMailService.class);
     ...
     */
    private boolean syncOneAccount(EmEmController controller, long checkAccountId, int startId) {
    ...
    class ControllerResults implements EmEmController.Result {
        ...
        public void updateMailboxCallback(EmMessagingException result, long accountId,
            ...
            if (result != null || progress == 100) {
                ...
                long inboxId = Mailbox.findMailboxOfType(EmMailService.this,
        ...
        public void serviceCheckMailCallback(EmMessagingException result, long accountId,
            ...
            if (result != null || progress == 100) {
                ...
                AlarmManager alarmManager = (AlarmManager)getSystemService(Context.ALARM_SERVICE);
                reschedule(alarmManager);
                int serviceId = EmMailService.this.mStartId;
                if (tag != 0) {
                    serviceId = (int) tag;
                }
                stopSelf(serviceId);
    ...
    private void notifyNewMessages(long accountId) {
        ...
        String reportTitle;
        Context local  = this;
        local = (local == null)? EmSyncManager.getContext():local;	//NaGa
        if (accountsWithNewMessages == 1) {
            // Prepare a report for a single account
            // "12 unread (gmail)"
			reportTitle = numNewMessages
					+ " "
					+ getResources()
							.getString(
									(numNewMessages > 1) ? R.string.notification_new_title_multi
											: R.string.notification_new_title);
            		/*getResources().getQuantityString(
                    R.plurals.notification_new_one_account_fmt, numNewMessages,
                    numNewMessages, reportName);*/
            intent = EmMessageList.createIntent(this, accountId, -1, Mailbox.TYPE_INBOX, false);
        } else {
            // Prepare a report for multiple accounts
            // "4 accounts"
        	reportTitle = accountsWithNewMessages + " " + getResources().getString(R.string.notification_new_title_multi);
            ...
            intent = EmMessageList.createIntent(this, -1, Mailbox.QUERY_ALL_INBOXES, -1, false);
        }
        
        //PIN_SCREEN_ISSUE, -->
        intent = new Intent(this, EmNotificationService.class);
...
//            PendingIntent.getActivity(this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        Notification notification = new Notification(
                R.drawable.pr_connect_header_icon,
    ...
    //PWD_CHANGE_IMPL, -->
    private void notifyLoginFail(long accountId) {
    	
    	Account nAccount = Account.restoreAccountWithId(getApplicationContext(), accountId);
    	if(nAccount == null)	return;
        ...
        intent = new Intent(getApplicationContext(), EmAccountSettings.class);
        intent.putExtra("account_id", accountId);
        intent.putExtra("login_warn", true);

        PendingIntent pending =
              PendingIntent.getActivity(this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        
        String nTitle = getString(R.string.login_failed_title);
        reportTitle = nTitle;
        reportString = getString(R.string.login_failed_ticker, nAccount.mDisplayName);
        Notification notification = new Notification(
                R.drawable.pr_connect_header_icon,
                nTitle,
                System.currentTimeMillis());
        ...
        NotificationManager notificationManager =
            (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        notificationManager.notify(NOTIFICATION_ID_LOGIN_FAILED, notification);
    }
    //PWD_CHANGE_IMPL, <--
}
