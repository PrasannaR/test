...
import com.cognizant.trumobi.em.mail.EmFolder.FolderType;
import com.cognizant.trumobi.em.mail.EmFolder.MessageRetrievalListener;
import com.cognizant.trumobi.em.mail.EmFolder.OpenMode;
 ...
 */
public class EmMessagingController implements Runnable {
    ...
    private static final EmFlag[] FLAG_LIST_SEEN = new EmFlag[] { EmFlag.SEEN };
    private static final EmFlag[] FLAG_LIST_FLAGGED = new EmFlag[] { EmFlag.FLAGGED };
    ...
    private static EmMessagingController sInstance = null;
     ...
     */
    private final EmGroupMessagingListener mListeners = new EmGroupMessagingListener();
    private boolean mBusy;
    private final Context mContext;

    protected EmMessagingController(Context _context) {
        mContext = _context;
     ...
     * @return
     */
    public synchronized static EmMessagingController getInstance(Context _context) {
        if (sInstance == null) {
            sInstance = new EmMessagingController(_context);
        }
        return sInstance;
    }
     ...
     */
    public static void injectMockController(EmMessagingController mockController) {
    ...
    private void put(String description, EmMessagingListener listener, Runnable runnable) {
        try {
    ...
    public void addListener(EmMessagingListener listener) {
        mListeners.addListener(listener);
    }

    public void removeListener(EmMessagingListener listener) {
        mListeners.removeListener(listener);
    }

    private boolean isActiveListener(EmMessagingListener listener) {
     ...
     */
    public void listFolders(final long accountId, EmMessagingListener listener) {
        final EmEmailContent.Account account =
                EmEmailContent.Account.restoreAccountWithId(mContext, accountId);
        if (account == null) {
            return;
        }
        ...
        put("listFolders", listener, new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    EmStore store = EmStore.getInstance(account.getStoreUri(mContext), mContext, null);

                    EmFolder[] remoteFolders = store.getPersonalNamespaces();
                    ...
                    localFolderCursor = mContext.getContentResolver().query(
                            ...
                            EmEmailContent.MailboxColumns.ACCOUNT_KEY + "=?",
                    ...
                    if (!remoteFolderNames.equals(localFolderNames)) {
                        ...
                        for (String localNameToDrop : localsToDrop) {
                            ...
                            switch (localInfo.mType) {
                                ...
                                default:
                                    ...
                                    EmAttachmentProvider.deleteAllMailboxAttachmentFiles(
                                    ...
                                    Uri uri = ContentUris.withAppendedId(
                                            EmEmailContent.Mailbox.CONTENT_URI, localInfo.mId);
                        ...
                        for (String remoteNameToAdd : remoteFolderNames) {
                            EmEmailContent.Mailbox box = new EmEmailContent.Mailbox();
                            box.mDisplayName = remoteNameToAdd;
                            // box.mServerId;
                            // box.mParentServerId;
                            box.mAccountKey = account.mId;
                            box.mType = EmLegacyConversions.inferMailboxTypeFromName(
     ...
     * @param listener
     */
    public void synchronizeMailbox(final EmEmailContent.Account account,
            final EmEmailContent.Mailbox folder, EmMessagingListener listener) {
        /*
         * We don't ever sync the Outbox.
         */
        if (folder.mType == EmEmailContent.Mailbox.TYPE_OUTBOX) {
            return;
        }
     ...
     * @param folder
     */
    private void synchronizeMailboxSynchronous(final EmEmailContent.Account account,
            final EmEmailContent.Mailbox folder) {
        mListeners.synchronizeMailboxStarted(account.mId, folder.mId);
        try {
            processPendingActionsSynchronous(account);

            EmStoreSynchronizer.SyncResults results;

            // Select generic sync or store-specific sync
            EmStore remoteStore = EmStore.getInstance(account.getStoreUri(mContext), mContext, null);
            EmStoreSynchronizer customSync = remoteStore.getMessageSynchronizer();
            if (customSync == null) {
    ...
    private void saveOrUpdate(EmEmailContent content) {
        if (content.isSaved()) {
     ...
     * @throws EmMessagingException
     */
    private EmStoreSynchronizer.SyncResults synchronizeMailboxGeneric(
            final EmEmailContent.Account account, final EmEmailContent.Mailbox folder)
            throws EmMessagingException {
        ...
        if (folder.mType == Mailbox.TYPE_DRAFTS || folder.mType == Mailbox.TYPE_OUTBOX) {
            int totalMessages = EmEmailContent.count(mContext, folder.getUri(), null, null);
            return new EmStoreSynchronizer.SyncResults(totalMessages, 0);
        }
        ...
        try {
            localUidCursor = resolver.query(
                    ...
                    EmEmailContent.MessageColumns.ACCOUNT_KEY + "=?" +
        ...
        int localUnreadCount = EmEmailContent.count(mContext, EmEmailContent.Message.CONTENT_URI,
                EmEmailContent.MessageColumns.ACCOUNT_KEY + "=?" +
                " AND " + MessageColumns.MAILBOX_KEY + "=?" +
        ...
        EmStore remoteStore = EmStore.getInstance(account.getStoreUri(mContext), mContext, null);
        EmFolder remoteFolder = remoteStore.getFolder(folder.mDisplayName);
        ...
        if (folder.mType == Mailbox.TYPE_TRASH || folder.mType == Mailbox.TYPE_SENT
            ...
            if (!remoteFolder.exists()) {
                if (!remoteFolder.create(FolderType.HOLDS_MESSAGES)) {
                    return new EmStoreSynchronizer.SyncResults(0, 0);
                }
            }
        }
        ...
        if (visibleLimit <= 0) {
            EmStore.StoreInfo info = EmStore.StoreInfo.getStoreInfo(account.getStoreUri(mContext),
                    mContext);
            visibleLimit = info.mVisibleLimitDefault;
        }

        // 7.  Create a list of messages to download
        EmMessage[] remoteMessages = new EmMessage[0];
        final ArrayList<EmMessage> unsyncedMessages = new ArrayList<EmMessage>();
        HashMap<String, EmMessage> remoteUidMap = new HashMap<String, EmMessage>();
        ...
        if (remoteMessageCount > 0) {
            ...
            for (EmMessage message : remoteMessages) {
         ...
         */
        final ArrayList<EmMessage> newMessages = new ArrayList<EmMessage>();
        ...
        if (unsyncedMessages.size() > 0) {
            EmFetchProfile fp = new EmFetchProfile();
            fp.add(EmFetchProfile.Item.FLAGS);
            fp.add(EmFetchProfile.Item.ENVELOPE);
            final HashMap<String, LocalMessageInfo> localMapCopy =
                new HashMap<String, LocalMessageInfo>(localMessageMap);

            remoteFolder.fetch(unsyncedMessages.toArray(new EmMessage[0]), fp,
                    new MessageRetrievalListener() {
                        public void messageRetrieved(EmMessage message) {
                            try {
                                // Determine if the new message was already known (e.g. partial)
                                // And create or reload the full message info
                                LocalMessageInfo localMessageInfo =
                                    localMapCopy.get(message.getUid());
                                EmEmailContent.Message localMessage = null;
                                if (localMessageInfo == null) {
                                    localMessage = new EmEmailContent.Message();
                                } else {
                                    localMessage = EmEmailContent.Message.restoreMessageWithId(
                                ...
                                if (localMessage != null) {
                                    try {
                                        ...
                                        EmLegacyConversions.updateMessageFields(localMessage,
                                                message, account.mId, folder.mId);
                                        // Commit the message to the local store
                                        saveOrUpdate(localMessage);
                                        // Track the "new" ness of the downloaded message
                                        if (!message.isSet(EmFlag.SEEN)) {
        ...
        for (EmFlag flag : remoteFolder.getPermanentFlags()) {
            if (flag == EmFlag.SEEN) {
                remoteSupportsSeen = true;
            }
            if (flag == EmFlag.FLAGGED) {
        ...
        if (remoteSupportsSeen || remoteSupportsFlagged) {
            for (EmMessage remoteMessage : remoteMessages) {
                LocalMessageInfo localMessageInfo = localMessageMap.get(remoteMessage.getUid());
                if (localMessageInfo == null) {
                    continue;
                }
                boolean localSeen = localMessageInfo.mFlagRead;
                boolean remoteSeen = remoteMessage.isSet(EmFlag.SEEN);
                boolean newSeen = (remoteSupportsSeen && (remoteSeen != localSeen));
                boolean localFlagged = localMessageInfo.mFlagFavorite;
                boolean remoteFlagged = remoteMessage.isSet(EmFlag.FLAGGED);
                boolean newFlagged = (remoteSupportsFlagged && (localFlagged != remoteFlagged));
                if (newSeen || newFlagged) {
                    Uri uri = ContentUris.withAppendedId(
                            EmEmailContent.Message.CONTENT_URI, localMessageInfo.mId);
                    ContentValues updateValues = new ContentValues();
                    updateValues.put(EmEmailContent.Message.FLAG_READ, remoteSeen);
                    updateValues.put(EmEmailContent.Message.FLAG_FAVORITE, remoteFlagged);
...
//        resolver.update(uri, updateValues, null, null);
        ...
        for (String uidToDelete : localUidsToDelete) {
            ...
            EmAttachmentProvider.deleteAllAttachmentFiles(mContext, account.mId, infoToDelete.mId);

            // Delete the message itself
            Uri uriToDelete = ContentUris.withAppendedId(
                    EmEmailContent.Message.CONTENT_URI, infoToDelete.mId);
            resolver.delete(uriToDelete, null, null);

            // Delete extra rows (e.g. synced or deleted)
            Uri syncRowToDelete = ContentUris.withAppendedId(
                    EmEmailContent.Message.UPDATED_CONTENT_URI, infoToDelete.mId);
        ...
        ArrayList<EmMessage> largeMessages = new ArrayList<EmMessage>();
        ArrayList<EmMessage> smallMessages = new ArrayList<EmMessage>();
        for (EmMessage message : unsyncedMessages) {
        ...
        fp = new EmFetchProfile();
        fp.add(EmFetchProfile.Item.BODY);
        remoteFolder.fetch(smallMessages.toArray(new EmMessage[smallMessages.size()]), fp,
                new MessageRetrievalListener() {
                    public void messageRetrieved(EmMessage message) {
                        // Store the updated message locally and mark it fully loaded
                        copyOneMessageToProvider(message, account, folder,
                                EmEmailContent.Message.FLAG_LOADED_COMPLETE);
                    }
        });
        ...
        fp.clear();
        fp.add(EmFetchProfile.Item.STRUCTURE);
        remoteFolder.fetch(largeMessages.toArray(new EmMessage[largeMessages.size()]), fp, null);
        for (EmMessage message : largeMessages) {
            if (message.getBody() == null) {
                ...
                // further download.
                fp.clear();
                fp.add(EmFetchProfile.Item.BODY_SANE);
                //  TODO a good optimization here would be to make sure that all Stores set
                //  the proper size after this fetch and compare the before and after size. If
                //  they equal we can mark this SYNCHRONIZED instead of PARTIALLY_SYNCHRONIZED
                remoteFolder.fetch(new EmMessage[] { message }, fp, null);

                // Store the partially-loaded message and mark it partially loaded
                copyOneMessageToProvider(message, account, folder,
                        EmEmailContent.Message.FLAG_LOADED_PARTIAL);
            } else {
        ...
        fp.clear();
        fp.add(EmFetchProfile.Item.STRUCTURE);
        remoteFolder.fetch(largeMessages.toArray(new EmMessage[largeMessages.size()]),
                fp, null);
        for (EmMessage message : largeMessages) {
            if (message.getBody() == null) {
                /*
...
//            }
         ...
         */
        EmStoreSynchronizer.SyncResults results = new EmStoreSynchronizer.SyncResults(
...
//        localFolder.close(false);
        ...
        return new EmStoreSynchronizer.SyncResults(remoteMessageCount, newMessages.size());
    }
     ...
     */
    private void copyOneMessageToProvider(EmMessage message, EmEmailContent.Account account,
            EmEmailContent.Mailbox folder, int loadStatus) {
        try {
            EmEmailContent.Message localMessage = null;
            Cursor c = null;
            try {
                c = mContext.getContentResolver().query(
                        EmEmailContent.Message.CONTENT_URI,
                        EmEmailContent.Message.CONTENT_PROJECTION,
                        EmEmailContent.MessageColumns.ACCOUNT_KEY + "=?" +
                        " AND " + MessageColumns.MAILBOX_KEY + "=?" +
                        ...
                        null);
                if (c.moveToNext()) {
                    localMessage = EmEmailContent.getContent(c, EmEmailContent.Message.class);
                }
            } finally {
                if (c != null) {
                    c.close();
            ...
            EmEmailContent.Body body = EmEmailContent.Body.restoreBodyWithMessageId(mContext,
                    localMessage.mId);
            if (body == null) {
                body = new EmEmailContent.Body();
            }
            try {
                // Copy the fields that are available into the message object
                EmLegacyConversions.updateMessageFields(localMessage, message, account.mId,
                        folder.mId);
                ...
                EmLegacyConversions.updateBodyFields(body, localMessage, viewables);
                ...
                EmLegacyConversions.updateAttachments(mContext, localMessage,
                        attachments, false);
                ...
                cv.put(EmEmailContent.MessageColumns.FLAG_ATTACHMENT, localMessage.mFlagAttachment);
                cv.put(EmEmailContent.MessageColumns.FLAG_LOADED, localMessage.mFlagLoaded);
                Uri uri = ContentUris.withAppendedId(EmEmailContent.Message.CONTENT_URI,
                        localMessage.mId);
    ...
    public void processPendingActions(final long accountId) {
        put("processPendingActions", null, new Runnable() {
            public void run() {
                try {
                    EmEmailContent.Account account =
                        EmEmailContent.Account.restoreAccountWithId(mContext, accountId);
                    if (account == null) {
                        return;
                    }
     ...
     * @throws EmMessagingException
     */
    private void processPendingActionsSynchronous(EmEmailContent.Account account)
           throws EmMessagingException {
        ContentResolver resolver = mContext.getContentResolver();
     ...
     * @param accountIdArgs
     */
    private void processPendingDeletesSynchronous(EmEmailContent.Account account,
            ContentResolver resolver, String[] accountIdArgs) {
        Cursor deletes = resolver.query(EmEmailContent.Message.DELETED_CONTENT_URI,
                EmEmailContent.Message.CONTENT_PROJECTION,
                EmEmailContent.MessageColumns.ACCOUNT_KEY + "=?", accountIdArgs,
                EmEmailContent.MessageColumns.MAILBOX_KEY);
        long lastMessageId = -1;
        try {
            // Defer setting up the store until we know we need to access it
            EmStore remoteStore = null;
            ...
            while (deletes.moveToNext()) {
                ...
                EmEmailContent.Message oldMessage =
                    EmEmailContent.getContent(deletes, EmEmailContent.Message.class);
                ...
                if (remoteStore == null && deleteFromTrash) {
                    remoteStore = EmStore.getInstance(account.getStoreUri(mContext), mContext, null);
                }
                ...
                Uri uri = ContentUris.withAppendedId(EmEmailContent.Message.DELETED_CONTENT_URI,
                        oldMessage.mId);
     ...
     * @param accountIdArgs
     */
    private void processPendingUploadsSynchronous(EmEmailContent.Account account,
            ContentResolver resolver, String[] accountIdArgs) throws EmMessagingException {
        ...
        try {
            ...
            while (mailboxes.moveToNext()) {
                ...
                Cursor upsyncs1 = resolver.query(EmEmailContent.Message.CONTENT_URI,
                        EmEmailContent.Message.ID_PROJECTION,
                        EmEmailContent.Message.MAILBOX_KEY + "=?"
                        + " and (" + EmEmailContent.Message.SERVER_ID + " is null"
                        + " or " + EmEmailContent.Message.SERVER_ID + "=''" + ")",
                        mailboxKeyArgs,
                        null);
                try {
                    while (upsyncs1.moveToNext()) {
                        ...
                        if (remoteStore == null) {
                            remoteStore =
                                EmStore.getInstance(account.getStoreUri(mContext), mContext, null);
                        }
                        ...
                        // upsync the message
                        long id = upsyncs1.getLong(EmEmailContent.Message.ID_PROJECTION_COLUMN);
                        lastMessageId = id;
                ...
                Cursor upsyncs2 = resolver.query(EmEmailContent.Message.UPDATED_CONTENT_URI,
                        EmEmailContent.Message.ID_PROJECTION,
                        EmEmailContent.MessageColumns.MAILBOX_KEY + "=?", mailboxKeyArgs,
                        null);
                try {
                    while (upsyncs2.moveToNext()) {
                        ...
                        // upsync the message
                        long id = upsyncs2.getLong(EmEmailContent.Message.ID_PROJECTION_COLUMN);
                        lastMessageId = id;
                        processUploadMessage(resolver, remoteStore, account, mailbox, id);
                    }
                } finally {
     ...
     * @param accountIdArgs
     */
    private void processPendingUpdatesSynchronous(EmEmailContent.Account account,
            ContentResolver resolver, String[] accountIdArgs) {
        Cursor updates = resolver.query(EmEmailContent.Message.UPDATED_CONTENT_URI,
                EmEmailContent.Message.CONTENT_PROJECTION,
                EmEmailContent.MessageColumns.ACCOUNT_KEY + "=?", accountIdArgs,
                EmEmailContent.MessageColumns.MAILBOX_KEY);
        long lastMessageId = -1;
        try {
            ...
            while (updates.moveToNext()) {
                ...
                EmEmailContent.Message oldMessage =
                    EmEmailContent.getContent(updates, EmEmailContent.Message.class);
                lastMessageId = oldMessage.mId;
                EmEmailContent.Message newMessage =
                    EmEmailContent.Message.restoreMessageWithId(mContext, oldMessage.mId);
                if (newMessage != null) {
                ...
                // Finally, delete the update
                Uri uri = ContentUris.withAppendedId(EmEmailContent.Message.UPDATED_CONTENT_URI,
                        oldMessage.mId);
                resolver.delete(uri, null, null);
            }
     ...
     * @param messageId
     */
    private void processUploadMessage(ContentResolver resolver, EmStore remoteStore,
            EmEmailContent.Account account, Mailbox mailbox, long messageId)
            throws EmMessagingException {
        EmEmailContent.Message newMessage =
            EmEmailContent.Message.restoreMessageWithId(mContext, messageId);
        ...
        if (deleteUpdate) {
            ...
            Uri uri = ContentUris.withAppendedId(EmEmailContent.Message.UPDATED_CONTENT_URI, messageId);
            resolver.delete(uri, null, null);
        }
    }
     ...
     * @param newMessage
     */
    private void processPendingFlagChange(EmStore remoteStore, Mailbox mailbox, boolean changeRead,
            boolean changeFlagged, EmEmailContent.Message newMessage) throws EmMessagingException {
        ...
        EmFolder remoteFolder = remoteStore.getFolder(mailbox.mDisplayName);
        if (!remoteFolder.exists()) {
            return;
        }
        remoteFolder.open(OpenMode.READ_WRITE, null);
        ...
        EmMessage remoteMessage = remoteFolder.getMessage(newMessage.mServerId);
        ...
        EmMessage[] messages = new EmMessage[] { remoteMessage };
        if (changeRead) {
     ...
     */
    private void processPendingMoveToTrash(EmStore remoteStore,
            EmEmailContent.Account account, Mailbox newMailbox, EmEmailContent.Message oldMessage,
            final EmEmailContent.Message newMessage) throws EmMessagingException {
        ...
        if (account.getDeletePolicy() == EmAccount.DELETE_POLICY_NEVER) {
            EmEmailContent.Message sentinel = new EmEmailContent.Message();
            sentinel.mAccountKey = oldMessage.mAccountKey;
            sentinel.mMailboxKey = oldMessage.mMailboxKey;
            sentinel.mFlagLoaded = EmEmailContent.Message.FLAG_LOADED_DELETED;
        ...
        EmFolder remoteFolder = remoteStore.getFolder(oldMailbox.mDisplayName);
        if (!remoteFolder.exists()) {
            return;
        }
        ...
        EmMessage remoteMessage = remoteFolder.getMessage(oldMessage.mServerId);
        if (remoteMessage == null) {
            remoteFolder.close(false);
            return;
        }
        ...
        EmFolder remoteTrashFolder = remoteStore.getFolder(newMailbox.mDisplayName);
        ...
        if (remoteTrashFolder.exists()) {
            ...
            remoteFolder.copyMessages(new EmMessage[] { remoteMessage }, remoteTrashFolder,
                    new EmFolder.MessageUpdateCallbacks() {
                public void onMessageUidChange(EmMessage message, String newUid) {
                    // update the UID in the local trash folder, because some stores will
                    // have to change it when copying to remoteTrashFolder
                    ContentValues cv = new ContentValues();
                    cv.put(EmEmailContent.Message.SERVER_ID, newUid);
                 ...
                 */
                public void onMessageNotFound(EmMessage message) {
        ...
        remoteMessage.setFlag(EmFlag.DELETED, true);
     ...
     */
    private void processPendingDeleteFromTrash(EmStore remoteStore,
            EmEmailContent.Account account, Mailbox oldMailbox, EmEmailContent.Message oldMessage)
            throws EmMessagingException {
        ...
        EmFolder remoteTrashFolder = remoteStore.getFolder(oldMailbox.mDisplayName);
        if (!remoteTrashFolder.exists()) {
            return;
        }
        ...
        EmMessage remoteMessage = remoteTrashFolder.getMessage(oldMessage.mServerId);
        if (remoteMessage == null) {
            remoteTrashFolder.close(false);
            return;
        }
     ...
     */
    private boolean processPendingAppend(EmStore remoteStore, EmEmailContent.Account account,
            Mailbox newMailbox, EmEmailContent.Message message)
            throws EmMessagingException {
        ...
        EmFolder remoteFolder = remoteStore.getFolder(newMailbox.mDisplayName);
        if (!remoteFolder.exists()) {
            if (!remoteFolder.canCreate(FolderType.HOLDS_MESSAGES)) {
                ...
                if (message.mServerId == null || message.mServerId.length() == 0) {
                    ...
                    Uri uri =
                        ContentUris.withAppendedId(EmEmailContent.Message.CONTENT_URI, message.mId);
                    ContentValues cv = new ContentValues();
                    cv.put(EmEmailContent.Message.SERVER_ID, message.mServerId);
                    mContext.getContentResolver().update(uri, cv, null, null);
                }
                return true;
            }
        ...
        if (remoteMessage == null) {
            ...
            EmMessage localMessage = EmLegacyConversions.makeMessage(mContext, message);

            // 3b. Upload it
            EmFetchProfile fp = new EmFetchProfile();
            fp.add(EmFetchProfile.Item.BODY);
            remoteFolder.appendMessages(new EmMessage[] { localMessage });
            ...
            EmFetchProfile fp = new EmFetchProfile();
            fp.add(EmFetchProfile.Item.ENVELOPE);
            remoteFolder.fetch(new EmMessage[] { remoteMessage }, fp, null);
        ...
        if (updateInternalDate && message.mServerId != null) {
            try {
                EmMessage remoteMessage2 = remoteFolder.getMessage(message.mServerId);
                if (remoteMessage2 != null) {
                    EmFetchProfile fp2 = new EmFetchProfile();
                    fp2.add(EmFetchProfile.Item.ENVELOPE);
                    remoteFolder.fetch(new EmMessage[] { remoteMessage2 }, fp2, null);
        ...
        if (deleteMessage || updateMessage) {
            Uri uri = ContentUris.withAppendedId(EmEmailContent.Message.CONTENT_URI, message.mId);
            ContentResolver resolver = mContext.getContentResolver();
            if (deleteMessage) {
                resolver.delete(uri, null, null);
                ...
                ContentValues cv = new ContentValues();
                cv.put(EmEmailContent.Message.SERVER_ID, message.mServerId);
                cv.put(EmEmailContent.Message.SERVER_TIMESTAMP, message.mServerTimeStamp);
     ...
     */
    public void loadMessageForView(final long messageId, EmMessagingListener listener) {
        ...
        put("loadMessageForViewRemote", listener, new Runnable() {
            public void run() {
                try {
                    ...
                    EmEmailContent.Message message =
                        EmEmailContent.Message.restoreMessageWithId(mContext, messageId);
                    if (message == null) {
                        mListeners.loadMessageForViewFailed(messageId, "Unknown message");
                        return;
                    }
                    if (message.mFlagLoaded == EmEmailContent.Message.FLAG_LOADED_COMPLETE) {
                    ...
                    EmEmailContent.Account account =
                        EmEmailContent.Account.restoreAccountWithId(mContext, message.mAccountKey);
                    EmEmailContent.Mailbox mailbox =
                        EmEmailContent.Mailbox.restoreMailboxWithId(mContext, message.mMailboxKey);
...
//                  localFolder.updateMessage((LocalMessage)message);
                    ...
                    EmMessage remoteMessage = remoteFolder.getMessage(message.mServerId);
                    EmFetchProfile fp = new EmFetchProfile();
                    fp.add(EmFetchProfile.Item.BODY);
                    remoteFolder.fetch(new EmMessage[] { remoteMessage }, fp, null);
    ...
    public void loadAttachment(final long accountId, final long messageId, final long mailboxId,
            final long attachmentId, EmMessagingListener listener) {
        ...
        put("loadAttachment", listener, new Runnable() {
            public void run() {
                try {
                    ...
                    File saveToFile = EmAttachmentProvider.getAttachmentFilename(mContext, accountId,
                            attachmentId);
                    ...
                    EmEmailContent.Account account =
                        EmEmailContent.Account.restoreAccountWithId(mContext, accountId);
                    EmEmailContent.Mailbox mailbox =
                        EmEmailContent.Mailbox.restoreMailboxWithId(mContext, mailboxId);
                    EmEmailContent.Message message =
                        EmEmailContent.Message.restoreMessageWithId(mContext, messageId);
                    ...
                    EmMessage storeMessage = remoteFolder.createMessage(message.mServerId);
                    EmMimeBodyPart storePart = new EmMimeBodyPart();
                    storePart.setSize((int)attachment.mSize);
                    storePart.setHeader(EmMimeHeader.HEADER_ANDROID_ATTACHMENT_STORE_DATA,
                            attachment.mLocation);
                    storePart.setHeader(EmMimeHeader.HEADER_CONTENT_TYPE,
                            String.format("%s;\n name=\"%s\"",
                            attachment.mMimeType,
                            attachment.mFileName));
                    // TODO is this always true for attachments?  I think we dropped the
                    // true encoding along the way
                    storePart.setHeader(EmMimeHeader.HEADER_CONTENT_TRANSFER_ENCODING, "base64");

                    EmMimeMultipart multipart = new EmMimeMultipart();
                    multipart.setSubType("mixed");
                    multipart.addBodyPart(storePart);

                    storeMessage.setHeader(EmMimeHeader.HEADER_CONTENT_TYPE, "multipart/mixed");
                    storeMessage.setBody(multipart);

                    // 4. Now ask for the attachment to be fetched
                    EmFetchProfile fp = new EmFetchProfile();
                    fp.add(storePart);
                    remoteFolder.fetch(new EmMessage[] { storeMessage }, fp, null);

                    // 5. Save the downloaded file and update the attachment as necessary
                    EmLegacyConversions.saveAttachmentBody(mContext, storePart, attachment,
                            accountId);
    ...
    /* package */ void pruneCachedAttachments(long accountId) {
        ContentResolver resolver = mContext.getContentResolver();
        File cacheDir = EmAttachmentProvider.getAttachmentDirectory(mContext, accountId);
     ...
     * @param listener
     */
    public void sendPendingMessages(final EmEmailContent.Account account, final long sentFolderId,
            EmMessagingListener listener) {
     ...
     * @param listener
     */
    public void sendPendingMessagesSynchronous(final EmEmailContent.Account account,
            long sentFolderId) {
        ...
        ContentResolver resolver = mContext.getContentResolver();
        Cursor c = resolver.query(EmEmailContent.Message.CONTENT_URI,
                EmEmailContent.Message.ID_COLUMN_PROJECTION,
                EmEmailContent.Message.MAILBOX_KEY + "=?", new String[] { Long.toString(outboxId) },
                null);
        if( c == null) return;
        try {
            // 2.  exit early
            ...
            EmSender sender = EmSender.getInstance(mContext, account.getSenderUri(mContext));
            EmStore remoteStore = EmStore.getInstance(account.getStoreUri(mContext), mContext, null);
            ...
            while (c.moveToNext()) {
                ...
                Uri syncedUri =
                    ContentUris.withAppendedId(EmEmailContent.Message.SYNCED_CONTENT_URI, messageId);
                if (requireMoveMessageToSentFolder) {
                    resolver.update(syncedUri, moveToSentValues, null, null);
                } else {
                    EmAttachmentProvider.deleteAllAttachmentFiles(mContext, account.mId, messageId);
                    Uri uri =
                        ContentUris.withAppendedId(EmEmailContent.Message.CONTENT_URI, messageId);
                    resolver.delete(uri, null, null);
     ...
     * @param listener
     */
    public void checkMail(final long accountId, final long tag, final EmMessagingListener listener) {
            ...
            public void run() {
                ...
                if (account != null) {
                    ...
                    if (inboxId != Mailbox.NO_MAILBOX) {
                        EmEmailContent.Mailbox mailbox =
                            EmEmailContent.Mailbox.restoreMailboxWithId(mContext, inboxId);
                        if (mailbox != null) {
    ...
    private static class Command {
        ...
        public EmMessagingListener listener;
