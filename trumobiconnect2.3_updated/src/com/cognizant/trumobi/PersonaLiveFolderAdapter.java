...
class PersonaLiveFolderAdapter extends CursorAdapter {
    ...
    private final PersonaLauncher mLauncher;
    private Typeface themeFont=null;
    PersonaLiveFolderAdapter(PersonaLauncher personaLauncher, PersonaLiveFolderInfo info, Cursor cursor) {
        super(personaLauncher, cursor, true);
        mIsList = info.displayMode == LiveFolders.DISPLAY_MODE_LIST;
        mInflater = LayoutInflater.from(personaLauncher);
    ...
    static Cursor query(Context context, PersonaLiveFolderInfo info) {
        try{
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        ...
        if (!mIsList) {
            view = mInflater.inflate(R.layout.pr_application_boxed, parent, false);
        } else {
            view = mInflater.inflate(R.layout.pr_application_list, parent, false);
    ...
    private Drawable loadIcon(Context context, Cursor cursor, ViewHolder holder) {
        ...
        if (data != null) {
            ...
            if (icon == null) {
                ...
                icon = new PersonaFastBitmapDrawable(PersonaUtilities.createBitmapThumbnail(bitmap, mLauncher));
            ...
            if (icon == null) {
                try {
                    ...
                    final int id = resources.getIdentifier(resource,
                            null, null);
                    icon = PersonaUtilities.createIconThumbnail(resources.getDrawable(id), mLauncher);
