...
import com.cognizant.trumobi.PersonaApplicationInfo;
import com.cognizant.trumobi.PersonaLauncherModel;
import com.cognizant.trumobi.catalogue.PersonaAppCatalogueFilters.Catalogue;

public class PersonaAppInfoMList extends ListActivity implements
		View.OnCreateContextMenuListener, View.OnClickListener,
		DialogInterface.OnCancelListener {
	private static final String TAG = "PersonaAppInfoMList";
	...
	private PersonaApplicationListAdapter mAppInfoAdapter;
	// list of task info
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int GroupIndex = intent.getIntExtra(EXTRA_CATALOGUE_INDEX,
				PersonaAppCatalogueFilters.getInstance().getDrawerFilter().getCurrentFilterIndex());
		mCatalogue = PersonaAppCatalogueFilters.getInstance().getCatalogue(GroupIndex);
		if (mCatalogue == null) {
		...
		setContentView(R.layout.pr_app_group_conf_list);
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE,
				R.layout.pr_custom_title1);

		List<PersonaAppListInfo> appInfos = new ArrayList<PersonaAppListInfo>();

		/* list info */
		mAppInfoAdapter = new PersonaApplicationListAdapter(this, appInfos);
	...
	public void onClick(View v) {
		if (v == mOkButton) {
			...
			//editor.clear();
			PersonaApplicationListAdapter adapter = (PersonaApplicationListAdapter) mAppInfoList.getAdapter();
			for (int i = 0; i < adapter.getCount(); i++) {
				PersonaAppListInfo tempAppListInfo = (PersonaAppListInfo) adapter.getItem(i);
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		PersonaApplicationListAdapter adapter = (PersonaApplicationListAdapter) mAppInfoList.getAdapter();
		PersonaAppListInfo tempAppInfo = (PersonaAppListInfo) adapter.getItem(position);
	...
	private void updateAppList() {
		...
		ArrayList<PersonaApplicationInfo> appInfos = PersonaLauncherModel.mApplicationsAdapter.allItems;
		/* app info */
		final List<PersonaAppListInfo> savedAppInfos = new ArrayList<PersonaAppListInfo>();
		...
		for (int i = 0; i < appInfos.size(); i++) {
			PersonaAppListInfo tempAppListInfo = new PersonaAppListInfo();
			/* get App info */
			PersonaApplicationInfo tempAppInfo = appInfos.get(i);
