...
public class PersonaDesktopIndicator extends ViewGroup implements AnimationListener {
	...
	private static final int INDICATOR_DEFAULT_COLOR=0x99000000;
	...
	public PersonaDesktopIndicator(Context context) {
		super(context);
		loadThemeColors(context);
		initIndicator(context);
	}

	public PersonaDesktopIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
		loadThemeColors(context);
		initIndicator(context);
	}

	public PersonaDesktopIndicator(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initIndicator(Context context){
		...
		case INDICATOR_TYPE_PAGER:
			mIndicator=new PersonaPreviewPager(context);
			((PersonaPreviewPager) mIndicator).setTotalItems(mItems);
			((PersonaPreviewPager) mIndicator).setCurrentItem(mCurrent);
			break;
	...
	public void indicate(float percent){
		...
		switch(mIndicatorType){
		case INDICATOR_TYPE_PAGER:
			((PersonaPreviewPager) mIndicator).setCurrentItem(position);
			break;
		case INDICATOR_TYPE_SLIDER_BOTTOM:
		case INDICATOR_TYPE_SLIDER_TOP:
	...
	private Runnable mAutoHide = new Runnable() {
		   public void run() {
			   if(mAnimation==null){
				   mAnimation=AnimationUtils.loadAnimation(getContext(), R.anim.pr_fade_out_fast);
				   mAnimation.setAnimationListener(PersonaDesktopIndicator.this);
			   }else{
				   try{
	...
	private void loadThemeColors(Context context){
    	...
    	String themePackage=PersonaAlmostNexusSettingsHelper.getThemePackageName(context, PersonaLauncher.THEME_DEFAULT);
