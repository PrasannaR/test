...
/**
 * Various utilities shared amongst the PersonaLauncher's classes.
 */

/**
 * 
 * @author rudhra bug id -2223	resolved
 *
 */
final class PersonaUtilities {
    ...
    static Drawable createIconThumbnail(Drawable icon, Context context) {
     ...
     /*  if (sIconWidth == -1) {
            ...
            sIconWidth  = (int) resources.getDimension(android.R.dimen.thumbnail_width);
            sIconHeight = (int) resources.getDimension(android.R.dimen.thumbnail_height);
        }

        PersonaLog.e("sIconWidth", ""+sIconWidth);
        PersonaLog.e("sIconHeight", ""+sIconHeight);
        ...
        PersonaLog.e("iconWidth", ""+iconWidth);
        PersonaLog.e("iconHeight", ""+iconHeight);
        PersonaLog.e("scale", ""+scale);
        
        if (width > 0 && height > 0) {
          //  if (width < iconWidth || height < iconHeight || scale != 1.0f) {
            	
            	PersonaLog.e("Inside if", "Inside if");
                final float ratio =     1.0f;    //(float) iconWidth / iconHeight;
                ...
                final Bitmap thumb = Bitmap.createBitmap(iconWidth, iconWidth, c);
                ...
                PersonaLog.e("icon.getBounds() 1", ""+icon.getBounds());
                final int x = (sIconWidth - width) / 2;
                final int y = (sIconHeight - height) / 2;
                
                
                icon.setBounds(x, y,  iconWidth, iconHeight);
                PersonaLog.e("icon.getBounds() 2", ""+icon.getBounds());
                icon.draw(canvas);
                icon.setBounds(sOldBounds);
                PersonaLog.e("icon.getBounds() 3", ""+icon.getBounds());
                icon = new PersonaFastBitmapDrawable(thumb);
         /*   } else if (iconWidth < width && iconHeight < height) {
            	
            	PersonaLog.e("Inside Else", "Inside Else");
                ...
                PersonaLog.e("sIconWidth", ""+sIconWidth);
                PersonaLog.e("sIconHeight", ""+sIconHeight);
                PersonaLog.e("icon.getBounds() else 1", ""+icon.getBounds());
    ...
    static Bitmap createBitmapThumbnail(Bitmap bitmap, Context context) {
    	...
    	PersonaLog.e("Inside createBitmapThumbnail","Inside createBitmapThumbnail");
        if (sIconWidth == -1) {
            final Resources resources = context.getResources();
        ...
        if (width > 0 && height > 0) {
            if (width < bitmapWidth || height < bitmapHeight) {
                ...
                paint.setDither(false);
                paint.setFilterBitmap(true);
                canvas.drawBitmap(bitmap, (sIconWidth - bitmapWidth)  ,
                        (sIconHeight - bitmapHeight) , paint);
                return thumb;
            }
        }
    ...
    static Drawable drawReflection(Drawable icon,Context context){
        ...
        try{
            ...
            return new PersonaFastBitmapDrawable(original);
        }
       ...
       try{
           return new PersonaFastBitmapDrawable(Bitmap.createScaledBitmap(bitmapWithReflection,Math.round((float)sIconWidth*ratio),sIconHeight,true));
       }catch(OutOfMemoryError e){
           return icon;
       }
    }
    ...
    static Drawable scaledDrawable(Drawable icon,Context context, boolean tint, float scale, int color){
        ...
        try{
        	Bitmap endImage=Bitmap.createScaledBitmap(original, (int)(width), (int)(height), true);
          	//Bitmap endImage=Bitmap.createScaledBitmap(original, (int)(width*1.2), (int)(height*1.2), true);

        	original.recycle();
        	return new PersonaFastBitmapDrawable(endImage);
        } catch (OutOfMemoryError e) {
            return icon;
        }
    }
