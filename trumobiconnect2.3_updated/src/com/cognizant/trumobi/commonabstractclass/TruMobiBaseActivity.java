...
import com.cognizant.trumobi.persona.PersonaLoginActivity;
...
public class TruMobiBaseActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new SharedPreferences(this).edit().putBoolean("appinBG", false)
	...
	protected void onResume() {
		...
		if (TruMobiBaseActivity.bkillActivity) {
			...
			PersonaLog.d(TAG,
					"=== PersonaLauncer - TruMobiBaseActivity finish ===");
			finish();

		} else if (requirePinScreenOnResume()) {
			PersonaLog.d(TAG,
					"===  - TruMobiBaseActivity launch main activity ===");
			TruMobiTimerClass.launchMainActivity(context);
		} else {
			PersonaLog.d(TAG,
					"===  - TruMobiBaseActivity Register for broadcast ===");
			mLocalBcMgr.registerReceiver(mMessageReceiver, new IntentFilter(
					"KILL_ACT"));
			if (TruMobiTimerClass.scheduledExecutorService == null
					|| TruMobiTimerClass.scheduledExecutorService.isShutdown()
					|| TruMobiTimerClass.scheduledExecutorService
							.isTerminated()) {
				PersonaLog
						.d(TAG,
								"===  - TruMobiBaseActivity Register for broadcast ===");
		...
		vulnarability();
		// mLocalBcMgr.registerReceiver(mMessageReceiver, new
		// IntentFilter("KILL_ACT"));
		// if(TruMobiTimerClass.scheduledExecutorService == null
		// ||TruMobiTimerClass.scheduledExecutorService.isShutdown() ||
		// TruMobiTimerClass.scheduledExecutorService.isTerminated())
		// {
		// TruMobiTimerClass.startTimerTrigger(context);
		// }
		// vulnarability();
	...
	public void onBackPressed() {
		...
		/*
		 * if (this instanceof EmAccountSecurity) {
		 * 
		 * } else {
		 */
		isBackPressed = true;
	...
	// IME_KEYPAD_TIMER_FIX : Added this method to handle Timer value during
	// Keypad show.
	@Override
