...
import com.cognizant.trumobi.persona.multitouch.PersonaMultiTouchController;
import com.cognizant.trumobi.persona.multitouch.PersonaMultiTouchController.MultiTouchObjectCanvas;
import com.cognizant.trumobi.persona.multitouch.PersonaMultiTouchController.PointInfo;
import com.cognizant.trumobi.persona.multitouch.PersonaMultiTouchController.PositionAndScale;
import com.cognizant.trumobi.persona.widgets.PersonaWidgetSpace;
 ...
 */
public class PersonaWorkspace extends PersonaWidgetSpace implements PersonaDropTarget, PersonaDragSource, PersonaDragScroller,
MultiTouchObjectCanvas<Object>, FlingListener {
	...
	private PersonaCustomScroller mScroller;
	private final PersonaFlingGesture mFlingGesture;

	/**
	 * CellInfo for the cell that is currently being dragged
	 */
	private PersonaCellLayout.CellInfo mDragInfo;
	 ...
	 */
	private PersonaCellLayout.CellInfo mVacantCache = null;
	...
	private PersonaMultiTouchController<Object> personaMultiTouchController;
	...
	private int mDesktopCacheType=PersonaAlmostNexusSettingsHelper.CACHE_LOW;
	...
	//MediaPlayer _shootMP= MediaPlayer.create(getContext(), Uri.parse("android.resource://com.cognizant.trumobi/" + R.raw.nokia));
	//AudioManager meng = (AudioManager) Context.getSystemService(Context.AUDIO_SERVICE);

	MediaPlayer _shootMP=new MediaPlayer();//= MediaPlayer.create(getContext(), Uri.parse("android.resource://com.cognizant.trumobi/" + R.raw.six));
	 ...
	 */
	public PersonaWorkspace(Context context, AttributeSet attrs) {
	 ...
	 *
	 * @param context The application's context.
	 * @param attrs The attributes set containing the PersonaWorkspace's customization values.
	 * @param defStyle Unused.
	 */
	public PersonaWorkspace(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		System.gc();
		mWallpaperManager = WallpaperManager.getInstance(context);

		PersonaLog.i("oncreate ^^^^^^^^^^^^^", "hello");
		/* Rogro82@xda Extended : Load the default and number of homescreens from the settings database */
		

		mHomeScreens = PersonaAlmostNexusSettingsHelper.getDesktopScreens(context);
		mDefaultScreen = PersonaAlmostNexusSettingsHelper.getDefaultScreen(context);

		if(mDefaultScreen>mHomeScreens-1) 
		{
			mDefaultScreen=0;
		}
		//ADW: create desired screens programmatically
		LayoutInflater layoutInflter=LayoutInflater.from(context);
		for(int i=0;i<mHomeScreens;i++){
			PersonaCellLayout screen=(PersonaCellLayout)layoutInflter.inflate(R.layout.pr_workspace_screen, this, false);

			if(PersonaLauncher.wsettings==3)
			{
				PersonaLog.i("Height &&&&&&&&&&&", screen.getCellHeight() + "" );
				PersonaLog.i("Width &&&&&&&&&&&", screen.getCellWidth() + "" );
			
				if(i==2)
				{	
					if(screen.getCellHeight() < screen.getCellWidth())
					{
					//	screen.setBackgroundResource(R.drawable.mathology);
					}
					else
					{
					//	screen.setBackgroundResource(R.drawable.krishna);
					}
		...
		mFlingGesture = new PersonaFlingGesture();
	...
	private void initWorkspace() {
		mScroller = new PersonaCustomScroller(getContext(), new PersonaElasticInterpolator(5f));
		mCurrentScreen = mDefaultScreen;
		PersonaLauncher.setScreen(mCurrentScreen);
		mPaint=new Paint();
		mPaint.setDither(false);
		final ViewConfiguration configuration = ViewConfiguration.get(getContext());
		mTouchSlop = configuration.getScaledTouchSlop();
		mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();
		//Wysie: Use PersonaMultiTouchController only for multitouch events
		personaMultiTouchController = new PersonaMultiTouchController<Object>(this, false);
		mDesktopRows=PersonaAlmostNexusSettingsHelper.getDesktopRows(getContext());
		mDesktopColumns=PersonaAlmostNexusSettingsHelper.getDesktopColumns(getContext());
		mDesktopCacheType=PersonaAlmostNexusSettingsHelper.getScreenCache(getContext());
	}

	@Override
	public void addView(View child, int index, LayoutParams params) {
		if (!(child instanceof PersonaCellLayout)) {
			throw new IllegalArgumentException("A PersonaWorkspace can only have PersonaCellLayout children.");
		}
	 ...
	 */
	PersonaFolder getOpenFolder() {
		PersonaCellLayout currentScreen = (PersonaCellLayout) getChildAt(mCurrentScreen);
		if(currentScreen==null)return null;
		int count = currentScreen.getChildCount();
		for (int i = 0; i < count; i++) {
			View child = currentScreen.getChildAt(i);
			PersonaCellLayout.LayoutParams lp = (PersonaCellLayout.LayoutParams) child.getLayoutParams();
			if (lp.cellHSpan == mDesktopColumns && lp.cellVSpan == mDesktopRows && child instanceof PersonaFolder) {
				return (PersonaFolder) child;
			}
		}
		return null;
	}

	ArrayList<PersonaFolder> getOpenFolders() {
		final int screens = getChildCount();
		ArrayList<PersonaFolder> personaFolders = new ArrayList<PersonaFolder>(screens);

		for (int screen = 0; screen < screens; screen++) {
			PersonaCellLayout currentScreen = (PersonaCellLayout) getChildAt(screen);
			int count = currentScreen.getChildCount();
			for (int i = 0; i < count; i++) {
				View child = currentScreen.getChildAt(i);
				PersonaCellLayout.LayoutParams lp = (PersonaCellLayout.LayoutParams) child.getLayoutParams();
				if (lp.cellHSpan == mDesktopColumns && lp.cellVSpan == mDesktopRows && child instanceof PersonaFolder) {
					personaFolders.add((PersonaFolder) child);
					break;
				}
			}
		}

		return personaFolders;
	}
	...
	void addInScreen(View child, int screen, int x, int y, int spanX, int spanY, boolean insert) {
		...
		final PersonaCellLayout group = (PersonaCellLayout) getChildAt(screen);
		PersonaCellLayout.LayoutParams lp = (PersonaCellLayout.LayoutParams) child.getLayoutParams();
		if (lp == null) {
			lp = new PersonaCellLayout.LayoutParams(x, y, spanX, spanY);
		} else {
			lp.cellX = x;
			lp.cellY = y;
			lp.cellHSpan = spanX;
		...
		if (!(child instanceof PersonaFolder)) {
			child.setOnLongClickListener(mLongClickListener);
		}
	}

	/*void addWidget(View view, PersonaWidget widget, boolean insert) {
        addInScreen(view, widget.screen, widget.cellX, widget.cellY, widget.spanX,
                widget.spanY, insert);
    }*/

	PersonaCellLayout.CellInfo findAllVacantCells(boolean[] occupied) {
		PersonaCellLayout group = (PersonaCellLayout) getChildAt(mCurrentScreen);
		if (group != null) {
	...
	PersonaCellLayout.CellInfo findAllVacantCellsFromModel() {
		PersonaCellLayout group = (PersonaCellLayout) getChildAt(mCurrentScreen);
		if (group != null) {
			int countX = group.getCountX();
			int countY = group.getCountY();
			boolean occupied[][] = new boolean[countX][countY];
			PersonaLauncher.getModel().findAllOccupiedCells(occupied, countX, countY, mCurrentScreen);
	...
	public void computeScroll() {
		if (mScroller.computeScrollOffset()) {
			...
			/*if(mLauncher.isScrollableAllowed()){
				...
				if (changedView instanceof PersonaWidgetCellLayout)
					((PersonaWidgetCellLayout) changedView).onViewportOut();
				}
				changedView = getChildAt(mCurrentScreen); // A screen get in
				if (changedView instanceof PersonaWidgetCellLayout)
				((PersonaWidgetCellLayout) changedView).onViewportIn();
			}*/
		}
	}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if(!mSensemode){
			...
			if (fastDraw) {
				...
				PersonaCellLayout currentView = (PersonaCellLayout) getChildAt(currentScreen);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthMode != MeasureSpec.EXACTLY) {
			throw new IllegalStateException("PersonaWorkspace can only be used in EXACTLY mode.");
		}
	...
	protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
		if(!mLauncher.isAllAppsVisible() && getChildCount()>0){
			final PersonaFolder openFolder = getOpenFolder();
			if (openFolder != null) {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (personaMultiTouchController.onTouchEvent(ev)) {
			return false;
		}
		...
		case MotionEvent.ACTION_UP:
			...
			if (mTouchState != TOUCH_STATE_SCROLLING && mTouchState != TOUCH_SWIPE_DOWN_GESTURE && mTouchState != TOUCH_SWIPE_UP_GESTURE) {
				final PersonaCellLayout currentScreen = (PersonaCellLayout) getChildAt(mCurrentScreen);
	...
	void enableChildrenCache(int fromScreen, int toScreen) {
		if(mDesktopCacheType!=PersonaAlmostNexusSettingsHelper.CACHE_DISABLED){
			...
			for (int i = fromScreen; i <= toScreen; i++) {
				PersonaCellLayout layout = (PersonaCellLayout) getChildAt(i);
				if(mDesktopCacheType==PersonaAlmostNexusSettingsHelper.CACHE_LOW)
	...
	void clearChildrenCache() {
		if(mDesktopCacheType!=PersonaAlmostNexusSettingsHelper.CACHE_DISABLED){
			final int count = getChildCount();
			for (int i = 0; i < count; i++) {
				final PersonaCellLayout layout = (PersonaCellLayout) getChildAt(i);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (deltaX1<0 && mCurrentScreen > 0) {

				mycurrentscreen = mCurrentScreen - 1;

				//PersonaLog.i("value delta < 0 *********", mycurrentscreen   + "");


			} else if (deltaX1>0 && mCurrentScreen < getChildCount() - 1) {
				...
				//PersonaLog.i("value *********","mycurrentscreen: " + mycurrentscreen + " nextScreen: " + nextScreen);
				mycurrentscreen = nextScreen;
			}
		...
		case MotionEvent.ACTION_UP:
			...
			clearVacantCache();
			enableChildrenCache(mCurrentScreen, mycurrentscreen);

			mycurrentscreen = Math.max(0, Math.min(mycurrentscreen, getChildCount() - 1));
			if(PersonaLauncher.wsettings==3)
			{
				if(mycurrentscreen==2)
				{
					...
					if (_shootMP != null)
						...
						/*	_shootMP.reset();
							_shootMP.setDataSource(getContext(), Uri.parse("android.resource://com.cognizant.trumobi/" + R.raw.six));
							_shootMP.prepare();
							_shootMP.start();*/
				...
				{
					if (_shootMP != null && _shootMP.isPlaying())
			...
			{
				//290778 commented for swipe up and down actions
	...
	public void OnFling(int Direction) {
		if (mTouchState == TOUCH_STATE_SCROLLING) {
			if (Direction == PersonaFlingGesture.FLING_LEFT && mCurrentScreen > 0) {
				snapToScreen(mCurrentScreen - 1);
			} else if (Direction == PersonaFlingGesture.FLING_RIGHT && mCurrentScreen < getChildCount() - 1) {
				snapToScreen(mCurrentScreen + 1);
			} else {
				final int screenWidth = getWidth();
				final int nextScreen = (getScrollX() + (screenWidth / 2)) / screenWidth;
	...
	void snapToScreen(int whichScreen) {
		...
		//PersonaLog.d("PersonaWorkspace", "whichScreen: "+whichScreen+"; mCurrentScreen: "+mCurrentScreen+"; getChildCount: "+(getChildCount()-1));
		if (screenDelta == 0) {
			durationOffset = 400;
			//PersonaLog.d("PersonaWorkspace", "Increasing duration by "+durationOffset+" times");
		}
	...
	void startDrag(PersonaCellLayout.CellInfo cellInfo) {
		View child = cellInfo.cell;

		// Make sure the drag was started by a long press as opposed to a long click.
		// Note that PersonaSearch takes focus when clicked rather than entering touch mode
		/*if (!child.isInTouchMode() && !(child instanceof PersonaSearch)) {
            return;
        }*/
		...
		PersonaCellLayout current = ((PersonaCellLayout) getChildAt(mCurrentScreen));
		/* final PersonaItemInfo info = (PersonaItemInfo)child.getTag();
        mLauncher.showActions(info, child); */ 

		current.onDragChild(child);
		mDragger.startDrag(child, this, child.getTag(), PersonaDragController.DRAG_ACTION_MOVE);
		invalidate();
		clearVacantCache();
	}
	...
	protected void onRestoreInstanceState(Parcelable state) {
		try{
			...
			PersonaLog.d("WORKSPACE","Google bug http://code.google.com/p/android/issues/detail?id=3981 found, bypassing...");
		}
	}

	void addApplicationShortcut(PersonaApplicationInfo info, PersonaCellLayout.CellInfo cellInfo,
			boolean insertAtFirst) {
		final PersonaCellLayout layout = (PersonaCellLayout) getChildAt(cellInfo.screen);
	...
	public void onDrop(PersonaDragSource source, int x, int y, int xOffset, int yOffset, Object dragInfo) {
		...
		final PersonaCellLayout personaCellLayout = getCurrentDropLayout();
		if (source != this) {
			onDropExternal(x - xOffset, y - yOffset, dragInfo, personaCellLayout);
		} else {
			// Move internally
			if (mDragInfo != null) {
				boolean moved=false;
				...
				if (index != mDragInfo.screen) {
					final PersonaCellLayout originalCellLayout = (PersonaCellLayout) getChildAt(mDragInfo.screen);
					originalCellLayout.removeView(cell);
					personaCellLayout.addView(cell);
					moved=true;
				}
				mTargetCell = estimateDropCell(x - xOffset, y - yOffset,
						mDragInfo.spanX, mDragInfo.spanY, cell, personaCellLayout, mTargetCell);
				personaCellLayout.onDropChild(cell, mTargetCell);
				if(mTargetCell[0]!=mDragInfo.cellX || mTargetCell[1]!=mDragInfo.cellY)
					moved=true;
				final PersonaItemInfo info = (PersonaItemInfo)cell.getTag();
				if(moved){
					PersonaCellLayout.LayoutParams lp = (PersonaCellLayout.LayoutParams) cell.getLayoutParams();
					PersonaLauncherModel.moveItemInDatabase(mLauncher, info,
							PersonaLauncherSettings.Favorites.CONTAINER_DESKTOP, index, lp.cellX, lp.cellY);
					//}else{
	...
	private void onDropExternal(int x, int y, Object dragInfo, PersonaCellLayout personaCellLayout) {
		onDropExternal(x, y, dragInfo, personaCellLayout, false);
	}
	...
	private void onDropExternal(int x, int y, Object dragInfo, PersonaCellLayout personaCellLayout,
		...
		case PersonaLauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
			...
			view = mLauncher.createShortcut(R.layout.pr_application, personaCellLayout,
					(PersonaApplicationInfo) info);
			break;
		case PersonaLauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER:
			view = PersonaFolderIcon.fromXml(R.layout.pr_folder_icon, mLauncher,
					(ViewGroup) getChildAt(mCurrentScreen), ((PersonaUserFolderInfo) info));
			break;
		case PersonaLauncherSettings.Favorites.ITEM_TYPE_LIVE_FOLDER:
			view = PersonaLiveFolderIcon.fromXml(
					R.layout.pr_live_folder_icon, mLauncher,
					(ViewGroup) getChildAt(mCurrentScreen),(PersonaLiveFolderInfo) info);
			break;
		default:
			throw new IllegalStateException("Unknown item type: " + info.itemType);
		}

		personaCellLayout.addView(view, insertAtFirst ? 0 : -1);
		view.setOnLongClickListener(mLongClickListener);
		mTargetCell = estimateDropCell(x, y, 1, 1, view, personaCellLayout, mTargetCell);
		personaCellLayout.onDropChild(view, mTargetCell);
		PersonaCellLayout.LayoutParams lp = (PersonaCellLayout.LayoutParams) view.getLayoutParams();
		...
		PersonaLauncherModel.addOrMoveItemInDatabase(mLauncher, info,
				PersonaLauncherSettings.Favorites.CONTAINER_DESKTOP, mCurrentScreen, lp.cellX, lp.cellY);
	}

	/**
	 * Return the current {@link PersonaCellLayout}, correctly picking the destination
	 * screen while a scroll is in progress.
	 */
	private PersonaCellLayout getCurrentDropLayout() {
		int index = mScroller.isFinished() ? mCurrentScreen : mNextScreen;
		final PersonaCellLayout layout = (PersonaCellLayout) getChildAt(index);
		if (layout!=null)
			return layout;
		else
			return (PersonaCellLayout) getChildAt(mCurrentScreen);
	}

	/**
	 * {@inheritDoc}
	 */
	public boolean acceptDrop(PersonaDragSource source, int x, int y,
			int xOffset, int yOffset, Object dragInfo) {
		final PersonaCellLayout layout = getCurrentDropLayout();
		final PersonaCellLayout.CellInfo cellInfo = mDragInfo;
	...
	private int[] estimateDropCell(int pixelX, int pixelY,
			int spanX, int spanY, View ignoreView, PersonaCellLayout layout, int[] recycle) {
	...
	public void onDropCompleted(View target, boolean success) {
		...
		if (success){
			if (target != this && mDragInfo != null) {
				final PersonaCellLayout personaCellLayout = (PersonaCellLayout) getChildAt(mDragInfo.screen);
				personaCellLayout.removeView(mDragInfo.cell);
				final Object tag = mDragInfo.cell.getTag();
				PersonaLauncher.getModel().removeDesktopItem((PersonaItemInfo) tag);
			}
		} else {
			if (mDragInfo != null) {
				final PersonaCellLayout personaCellLayout = (PersonaCellLayout) getChildAt(mDragInfo.screen);
				personaCellLayout.onDropAborted(mDragInfo.cell);
			}
		}
	 ...
	 */
	/*private PersonaSearch findSearchWidget(PersonaCellLayout screen) {
        final int count = screen.getChildCount();
        for (int i = 0; i < count; i++) {
            View v = screen.getChildAt(i);
            if (v instanceof PersonaSearch) {
                return (PersonaSearch) v;
            }
        }
        return null;
    }*/
	 ...
	 */
	/*public PersonaSearch findSearchWidgetOnCurrentScreen() {
        PersonaCellLayout currentScreen = (PersonaCellLayout)getChildAt(mCurrentScreen);
        return findSearchWidget(currentScreen);
    }*/

	public PersonaFolder getFolderForTag(Object tag) {
		...
		for (int screen = 0; screen < screenCount; screen++) {
			...
			for (int i = 0; i < count; i++) {
				...
				PersonaCellLayout.LayoutParams lp = (PersonaCellLayout.LayoutParams) child.getLayoutParams();
				if (lp.cellHSpan == mDesktopColumns && lp.cellVSpan == mDesktopRows && child instanceof PersonaFolder) {
					PersonaFolder f = (PersonaFolder) child;
					if (f.getInfo() == tag) {
						return f;
					}
				}
	 ...
	 * {@link PersonaLauncher} to accept or block dpad-initiated long-presses.
	 */
	...
	void removeShortcutsForPackage(String packageName) {
		...
		for (int i = 0; i < count; i++) {
			...
			for (int j = 0; j < childCount; j++) {
				...
				if (tag instanceof PersonaApplicationInfo) {
				...
				} else if (tag instanceof PersonaUserFolderInfo) {
					final PersonaUserFolderInfo info = (PersonaUserFolderInfo) tag;
					final ArrayList<PersonaApplicationInfo> contents = info.contents;
					final ArrayList<PersonaApplicationInfo> toRemove = new ArrayList<PersonaApplicationInfo>(1);
					final int contentsCount = contents.size();
					boolean removedFromFolder = false;

					for (int k = 0; k < contentsCount; k++) {
						final PersonaApplicationInfo appInfo = contents.get(k);
						...
						if (Intent.ACTION_MAIN.equals(intent.getAction()) &&
							...
							PersonaLauncherModel.deleteItemFromDatabase(mLauncher, appInfo);
					...
					if (removedFromFolder) {
						final PersonaFolder personaFolder = getOpenFolder();
						if (personaFolder != null) personaFolder.notifyDataSetChanged();
					}
				}
			}
	...
	void updateShortcutFromApplicationInfo(PersonaApplicationInfo info) {
		final int count = getChildCount();
		for (int i = 0; i < count; i++) {
			final PersonaCellLayout layout = (PersonaCellLayout) getChildAt(i);
			int childCount = layout.getChildCount();
			for (int j = 0; j < childCount; j++) {
				...
				Object tag = view.getTag();
				if (tag instanceof PersonaApplicationInfo) {
					PersonaApplicationInfo tagInfo = (PersonaApplicationInfo)tag;
					if (tagInfo.id == info.id)
					{
						tagInfo.assignFrom(info);

						View newview = mLauncher.createShortcut(R.layout.pr_application, layout, tagInfo);
					...
					final PersonaUserFolderInfo folderInfo = (PersonaUserFolderInfo) tag;
					final ArrayList<PersonaApplicationInfo> contents = folderInfo.contents;
					final int contentsCount = contents.size();
					for (int k = 0; k < contentsCount; k++) {
						final PersonaApplicationInfo appInfo = contents.get(k);
				...
				if (tag instanceof PersonaApplicationInfo) {
					...
					if ((info.itemType == PersonaLauncherSettings.Favorites.ITEM_TYPE_APPLICATION ||
						...
						if (icon != null && icon != info.icon) {
							...
							//info.icon = PersonaUtilities.createIconThumbnail(icon, mContext);
					...
					for (int k = 0; k < contentsCount; k++) {
						...
						final Intent intent = appInfo.intent;
						final ComponentName name = intent.getComponent();
						if ((appInfo.itemType == PersonaLauncherSettings.Favorites.ITEM_TYPE_APPLICATION ||
								appInfo.itemType == PersonaLauncherSettings.Favorites.ITEM_TYPE_SHORTCUT)&&
								Intent.ACTION_MAIN.equals(intent.getAction()) && name != null &&
								packageName.equals(name.getPackageName())) {
							...
							if (icon != null && icon != appInfo.icon) {
								...
								appInfo.icon = PersonaUtilities.createIconThumbnail(icon, mLauncher);
	...
	public void setBounceAmount(int amount){
		...
		mScroller.setInterpolator(new PersonaElasticInterpolator(mScrollingBounce/10));
	}
		...
		for(int i=0;i<getChildCount();i++){
			...
			if (tmp.contains(x+getScrollX(), y+getScrollY())){
				if(mCurrentScreen!=i){
					...
					mScroller.setInterpolator(new PersonaElasticInterpolator(0));
		...
		for (int i = 0; i < count; i++) {
			...
			if ( child !=null)
			{
				...
				if (cell_xy[0] >= left_cellmargin && cell_xy[0] < rigth_cellmargin && cell_xy[1] >= top_cellmargin && cell_xy[1] < botton_cellmargin)  {
					try {
						// Get PersonaWidget ID
						int id = ((AppWidgetHostView)child).getAppWidgetId();
						// Ask to PersonaWidgetSpace if the PersonaWidget identified itself when created as 'Scrollable'
	...
	public void unbindWidgetScrollableViewsForWidget(int widgetId) {
		PersonaLog.d("WORKSPACE", "trying to completely unallocate widget ID="+widgetId);
	...
	protected void removeScreen(int screen){
		final PersonaCellLayout layout = (PersonaCellLayout) getChildAt(screen);
		int childCount = layout.getChildCount();
		final PersonaLauncherModel model = PersonaLauncher.getModel();
		for (int j = 0; j < childCount; j++) {
			final View view = layout.getChildAt(j);
			...
			final PersonaItemInfo item = (PersonaItemInfo) tag;
			if (item!=null && item.container == PersonaLauncherSettings.Favorites.CONTAINER_DESKTOP) {
			...
			if (item!=null && item instanceof PersonaUserFolderInfo) {
			...
			} else if (item!=null && item instanceof PersonaLauncherAppWidgetInfo) {
		...
		if(getChildCount()<=mDefaultScreen){
			PersonaAlmostNexusSettingsHelper.setDefaultScreen(mLauncher, 0);
			mDefaultScreen=0;
		}
		if(mLauncher.getDesktopIndicator()!=null)mLauncher.getDesktopIndicator().setItems(getChildCount());
		indicatorLevels(mCurrentScreen);
		PersonaAlmostNexusSettingsHelper.setDesktopScreens(mLauncher, getChildCount());
	}
	protected PersonaCellLayout addScreen(int position){
	...
	protected void swapScreens(int screen_a, int screen_b){
		...
		PersonaCellLayout layout = (PersonaCellLayout) getChildAt(screen_a);
		...
		for (int j = 0; j < childCount; j++) {
			...
			final PersonaItemInfo item = (PersonaItemInfo) tag;
			if(item!=null && item.container==PersonaLauncherSettings.Favorites.CONTAINER_DESKTOP){
				PersonaLauncherModel.moveItemInDatabase(mLauncher, item, item.container, screen_b, item.cellX, item.cellY);
			}
		}
		layout = (PersonaCellLayout) getChildAt(screen_b);
		...
		for (int j = 0; j < childCount; j++) {
			...
			final PersonaItemInfo item = (PersonaItemInfo) tag;
			if(item!=null && item.container==PersonaLauncherSettings.Favorites.CONTAINER_DESKTOP){
				PersonaLauncherModel.moveItemInDatabase(mLauncher, item, item.container, screen_a, item.cellX, item.cellY);
			}
		}
		//swap the views
		PersonaCellLayout a=(PersonaCellLayout) getChildAt(screen_a);
	...
	private void moveItemPositions(int screen, int diff){
		...
		for (int i=screen+1;i<getChildCount();i++){
			...
			for (int j = 0; j < childCount; j++) {
				...
				final PersonaItemInfo item = (PersonaItemInfo) tag;
				if(item!=null && item.container==PersonaLauncherSettings.Favorites.CONTAINER_DESKTOP){
					PersonaLauncherModel.moveItemInDatabase(mLauncher, item, item.container, item.screen+diff, item.cellX, item.cellY);
				}
			}
		}
	}
			...
			for (int j = 0; j < childCount; j++) {
				...
				if (tag instanceof PersonaApplicationInfo) {
					...
					if ((info.itemType==PersonaLauncherSettings.Favorites.ITEM_TYPE_APPLICATION||
							...
							Intent.ACTION_MAIN.equals(intent.getAction()) && name != null &&
							packageName.equals(name.getPackageName())) {
						((PersonaBubbleTextView) view).setCounter(counter, color);
						view.invalidate();
	...
	private void preTransitionDraw(Canvas canvas, int mTransitionStyle, boolean isMovingRight, int xOffset, float transformAmount, int width )
	{
		if ( mTransitionStyle == 2 )  // rotate
		{
			...
			if ( mDesktopCacheType == PersonaAlmostNexusSettingsHelper.CACHE_AUTO )
			{
		...
		{
			PersonaCellLayout.mTransitionAmount = Math.abs( xOffset )/(float)width;
			PersonaCellLayout.mIsTransitionEnabled = true;
			PersonaCellLayout.mIsTransitionNegative = isMovingRight;
			canvas.translate(xOffset, 0);
		}
	}
	...
	private void postTransitionDraw(Canvas canvas, int mTransitionStyle, boolean isMovingRight, int xOffset, float transformAmount, int width )
	{
		...
		else if ( mTransitionStyle == 5 ) // scatter
		{
			...
			PersonaCellLayout.mIsTransitionOther = true;
			if( isMovingRight )
			{
				canvas.translate(xOffset - width, 0);
			}
			...
			PersonaCellLayout.mTransitionAmount = 1 - transformAmount;
		}
	}
	...
	private void finishTransitionDraw(Canvas canvas, int mTransitionStyle)
	{
		...
		{
			PersonaCellLayout.mIsTransitionEnabled = false;
			PersonaCellLayout.mIsTransitionOther = false;
			canvas.restore();
		}
		else
		{
