 ...
 */
public class SmsNumberPickerDialog extends AlertDialog implements
		OnClickListener {
	 ...
	 *            The initial number.
	 */
	public SmsNumberPickerDialog(Context context, OnNumberSetListener callBack,
			int number, int rangeMin, int rangeMax, int title) {
	 ...
	 */
	// @SuppressLint("NewApi")
	public SmsNumberPickerDialog(Context context, int theme,
			OnNumberSetListener callBack, int number, int rangeMin,
			int rangeMax, int title) {
		super(context, theme);
		mCallback = callBack;
		...
		setButton(DialogInterface.BUTTON_POSITIVE,
				context.getText(R.string.set), this);
		setButton(DialogInterface.BUTTON_NEGATIVE,
				context.getText(R.string.no), (OnClickListener) null);

		LayoutInflater inflater = (LayoutInflater) context
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		View view = inflater.inflate(R.layout.sms_number_picker_dialog, null);
		setView(view);
		...
		mNumberPicker.setOnLongPressUpdateInterval(100); // make the repeat rate
															// three times as
															// fast
															// as normal since
															// the range is so
